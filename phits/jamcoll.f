...
c***********************************************************************
      ...
      subroutine jamcoll
      use QMD_COOD2_MOD, only : coodrealloc, nqmdm
...
!$OMP THREADPRIVATE(/errorcom/)
      ...
      character rtype*3
      dimension indd(100)
! T.Sato 2014/11/13 for initialization. Not sure necessary or not
      indd(:)=0.0  ! initialization
...
c...Initialize time dependent analysis.
      if(mstc(3).eq.1.and.mstd(23).eq.1) call jamanat(1)
...
c...Scatter two-particles.
        call jamscatt
        if(ncascerr.le.-1) return ! T.Sato 2018/06/23, avoid infinite loop
...
*-----------------------------------------------------------------------
         if( mstc(190) .ne. 0 ) then
            ...
            call jam2qmd2(mste(21),mste(23))
            call caldis2R(mste(21),mste(23))
...
*-----------------------------------------------------------------------
      ...
      if( nv .ge. nnn * 0.8 .and. mstd(30) .ne. 0 ) then
      ...
      if(mstc(190) .ne. 0 .and. nv .ge. nqmdm - 100) call coodrealloc ! 2017/7/11 ogawa, extend array for particle data
...
*-----------------------------------------------------------------------
      ...
      if( mstc(190) .eq. 0 ) then
         ...
         if( pard(1) .lt. pare(1) ) then

            call qmdtrspt(pare(1))
...
c***********************************************************************
      ...
      use QMD_COOD2_MOD, only : rr2,rbij,pp2,rha, rhe, rhc
...
c...Avoid second collisions for the same pairs
      ...
      ibar1=k(9,i1)
      ibar2=k(9,i2)
      srt=sqrt(max(0d0,(p(4+mstc(190)*2,i1)+p(4+mstc(190)*2,i2))**2
...
cKN.---------------- for JAM of PHITS ------------
      ...
      dt=t02-t01
      dx12=dt**2-rsqare
      em1sq=p(4+mstc(190)*2,i1)**2-p(1,i1)**2-p(2,i1)**2-p(3,i1)**2
      em2sq=p(4+mstc(190)*2,i2)**2-p(1,i2)**2-p(2,i2)**2-p(3,i2)**2
      dxp1=dt*p(4+mstc(190)*2,i1)-dx*p(1,i1)-dy*p(2,i1)-dz*p(3,i1)
      dxp2=dt*p(4+mstc(190)*2,i2)-dx*p(1,i2)-dy*p(2,i2)-dz*p(3,i2)
      dp12=p(4+mstc(190)*2,i1)*p(4+mstc(190)*2,i2)-p(1,i1)*p(1,i2)
     $ -p(2,i1)*p(2,i2)-p(3,i1)*p(3,i2)
      ...
      dt1=-p(4+mstc(190)*2,i1)*(dxp1*em2sq-dxp2*dp12)/dn
      dt2=p(4+mstc(190)*2,i2)*(dxp2*em1sq-dxp1*dp12)/dn
      tcol1=t01+dt1
      tcol2=t02+dt2
...
!$OMP THREADPRIVATE(/coodid/)
      ...
      common /errorcom/ncascerr ! T.Sato 2018/06/23
!$OMP THREADPRIVATE(/errorcom/)
...
*-----------------------------------------------------------------------
         ...
         if( mstc(190) .ne. 0 ) then
          ...
          do i = min(i1,i2), max(i1,i2), abs(i1-i2)
      ...
      if(mstc(56).ne.0) then
        ...
        if( kf1.eq.2112 .or. kf1.eq.2212 ) then
            ...
            if( mstc(190) .eq. 0 ) then
               ...
               if( ifermi.eq.1 .and. p(4,i1)-p(5,i1).lt.efermi )
     &         ntag = 1

            else

               if( ntag .eq. 0 ) then
                  call pauliR( i1, ntag, phase )
               end if
        ...
        if(( kf2.eq.2112 .or. kf2.eq.2212) .and. (ntag.eq.0) ) then
            ...
            if( mstc(190) .eq. 0 ) then
               ...
               if( ifermi.eq.1 .and. p(4,i2)-p(5,i2).lt.efermi )
     &         ntag = 1

            else

               if( ntag .eq. 0 ) then
                  call pauliR( i2, ntag, phase )
               end if
...
c...Store particle information.
      if(nv .gt. nv_old .and. mstc(190) .ne. 0) then ! JAMQMD2.0
         call jam2qmd
         call caldisaR
...
c....Hard or soft
      if(srt.ge.parc(71).and.icltyp.le.4) then
        ...
        if(n_jet.eq.0) then
          ...
          if(kf1.eq.211.or.kf1.eq.-211.or.kf1.eq.321.or.kf1.eq.311
     &    .or.kf1.eq.-321) then ! hard collision for meson is not implemented
           goto 2200 ! T.Sato 2018/06/26
          else
           ichanel=6
          endif
...
c.....AntiB B annihilation.
      if(icltyp.eq.4) then
        ...
        nv=nv+1
        if(nv.gt.nnn) then
          call jamerrm(30,0,'(jamabsrb:)Particle too large [nnn]')
        endif
        i3=nv
        i4=0
...
c...MB/MM absorption.
      ...
      p(3,i3)=pcm(3)
      p(5,i3)=pcm(5)
      p(4,i3)=sqrt(pcm(1)**2+pcm(2)**2+pcm(3)**2+pcm(5)**2) ! consider potential sometime
      p(6,i3)=p(4,i3)
      mste(25)=i3
      mste(27)=i4
...
c....s-wave pion production.
      if(mste(3).le.3) then
        ...
        pd(1,5)=pymass(kd(1,2))
        pd(2,5)=pymass(kd(2,2))
        pd(3,5)=pymass(kd(3,2))
      endif
...
c...Kinematics.
      ...
      if(mste(3).le.3) then
        nv=nv+1
        nmeson=nmeson+1
        if(nv.gt.nnn) call jamerrm(30,0,'(jamscatm:)particle too large')
        i3=nv
        do j=1,5
         p(j,i1)=pd(1,j)
         p(j,i2)=pd(2,j)
...
c=================================================================*
      ...
      common /paraj/ mstz(300), parz(300) ! T.Sato 2021/05/18

      data  emnuc/0.938272046d0/  ! T.Sato 2022/11/03 use protom mass in param-physcnst.inc
...
c ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
c=================================================================*
c     Get angular distribution of np elastic scattering below 1 GeV
c-----------------------------------------------------------------*
c=================================================================*
      if( mstz(148) == 1 .and.  ! T.Sato 2021/05/18, further revised 2022/11/07
     &    ( (kf1 == 2112 .and. kf2 == 2212) .or.
     &    (kf1 == 2212 .and. kf2 == 2112) ) .and.
     &     snew < 2.3 ) then
        call hydrog(plab,angcm)
        if(angcm.ge.-100.0d0) then ! angcm is determined by hydrog, T.Sato 2022/11/03
         c1 = angcm
...
c...Backward scattering prob.
      if(kf1.ne.kf2.and.snew.le.3.0d0) then
        bprob=1.0d0
...
c...Purpose: to print collsioin/decay information.
      ...
      call pyname(kcp(2,1),chaf1)
      call pyname(kcp(2,2),chaf2)
      i1=mste(21)
      i2=mste(23)
      i3=mste(25)
      i4=mste(27)
...
c...Two-body collisions.
        ...
        call pyname(kcp(2,3),chaf3)
        call pyname(kcp(2,4),chaf4)
...
c...Purpose: to check possible errors.
      ...
      ih=mstc(38)
      if(mstc(8).ne.2) then
        write(ih,'(/,''  (jamcheck) nv nbary nmeson'',3(i5,1x))')
     $  nv,nbary,nmeson
        write(ih,*)echar
      endif
...
!$OMP THREADPRIVATE(/pq/)
c...Commonblock for LPC.
      common /jamlpc1/wa0,wr0,ww,rho0,ext,idens,icscde,trad
!$OMP THREADPRIVATE(/jamlpc1/)
      parameter (epsx=-20.0d0)
      logical first
      save prad2,pcount,numpau,first
!$OMP THREADPRIVATE(prad2,pcount,numpau,first)
      data first/.true./
...
c****************************************************************
     ...
     $       1.0d0*parj(32)+0.001d0+pymass(kfl1)+pymass(kfl2))
     ...
     $     ,1.0d0*parj(32)+pymass(kfl1)+pymass(kfl2))
...
c....In the case of gluon-gluon system.
          ...
          pv(jt,4)=p(4+mstc(190)*2,jp1)+p(4+mstc(190)*2,jp2)
...
c...Defiene cluster distance.
     ...
     $    .or.(kf1.eq.2112.and.kf2.eq.2212))then
        ...
        call jamrobo(0d0,0d0,-bex,-bey,-bez,gam,p1(1),p1(2),p1(3),p1(6))
        call jamrobo(0d0,0d0,-bex,-bey,-bez,gam,p2(1),p2(2),p2(3),p2(6))
        ...
        r2(4)=t
        do j=1,3
         r1(j)=r1(j)+t1*p1(j)/p1(6)
         r2(j)=r2(j)+t2*p2(j)/p2(6)
        end do
...
c...Form deuteron.
      ...
      dt2=t-coll(2,jent)
      do j=1,3
       r(j,i1)=r(j,i1)+dt1*p(j,i1)/p(6,i1)
       r(j,i2)=r(j,i2)+dt2*p(j,i2)/p(6,i2)
      end do
      ...
      p(6,i1)=p(6,i1)+p(6,i2)
      p(5,i1)=sqrt(p(6,i1)**2-p(1,i1)**2-p(2,i1)**2-p(3,i1)**2)
...
c...Initial values: global time,collision counter,etc.
      ...
      r(1,1)=pard(2)
      r(2,1)=0d0
      r(3,1)=-sqrt(abs(trad**2-pard(2)**2))
...
c...Too low velocity: quit cascading.
      if( ( kf1 .eq. 2212 .or. kf1 .eq. 2112 ) .and.
     &      v1 .lt. vlcut ) goto 6000

c...Too low energy: quit cascading.
      if( ( kf1 .eq. 2212 .or. kf1 .eq. 2112 ) .and.
     &      p1(4)-p1(5) .lt. 0.020 ) goto 6000
...
c...In case of |r1| > rad. Determine cascade is continue?
      rr1=sqrt(r1(1)**2+r1(2)**2+r1(3)**2)
      if(rr1.gt.trad*ext) goto 6000

c...Calculate proton and neutron density.
      if(idens.eq.1) then
        vol=4*paru(1)*trad**3/3
        rho=dble(mstd(5))/vol
        if(rr1.gt.trad) then
           rho=0.0d0
        endif
...
c...Loop over neuton and proton,
c   picking up nucleons from target nucleus.
...
c...Normal end.
        ...
        path=-patht*log(max(1.0d-15,rn(0)))
...
c...decay event
      ...
      if(ireac.eq.1.or.ireac.eq.2) then
        ...
        nv=nv+1
        nbary=nbary+1
        if(nv.gt.nnn) call jamerrm(30,0,'(jamfpath:)no more memory nnn')
        i2=nv
        call jamzero(i2)
        k(1,i2)=1
        k(2,i2)=kf2
...
c...Serch for next particle which will collied.
 ...
 6100 continue
      ...
      if(ncolp.ge.mstd(6)) goto 7000
      if(ncoln.ge.mstd(5)-mstd(6)) goto 7000
 ...
 7000 continue
      ...
      if( ncoln .lt. mstd(5)-mstd(6) ) then

         do i = 1, mstd(5)-mstd(6)-ncoln
