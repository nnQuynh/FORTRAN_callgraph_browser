...
*-----------------------------------------------------------------------
      ...
      common/cncalday/ncalday ! S.H. 2023.5.15
...
*-----------------------------------------------------------------------
         ...
         if( ic .lt. 1 .or. ic .gt. icpum ) then
            ...
            write(ErrCha,*) '*** Error at cputime(ic) : ic is too large'
            ErrID = 'L:53/R:cputime/F:utl01.f' !E00_016_001
...
*-----------------------------------------------------------------------
         ...
         if( ietm(ic) .eq. 0 ) then
          ...
          if ( ncalday .eq. 0 ) then
            stime(ic) = stime(ic) + sect_a(estime(ic))
            cputm(ic) = cputm(ic) + cput_a(ecputm(ic))
          else
            caltimsec = ncalday * 86400d0
            stime(ic) = stime(ic) + sect_a(estime(ic)) + caltimsec
            cputm(ic) = cputm(ic) + cput_a(ecputm(ic)) + caltimsec
          end if
...
************************************************************************
*                                                                      *
      subroutine timex(time3,time2,ic)
*                                                                      *
...
*-----------------------------------------------------------------------
            ...
            if( istart .eq. 0 ) then
               ...
               istart = 1
               ncalday = 0 ! S.H. 2023.5.15
               timefom  = cput_a( time0 ) ! S.H. 2024.3.18
               return
...
c$$$               time3 = time2 - time0
               time3 = time2 - timefom
c$$$               write(99,'(1p4d14.6)') time0,time2,time3,timefom
               if ( time3 .lt. 0d0 ) then ! S.H. 2024.3.20
                  ncaldayfom = int( dabs(time3) / 86400. ) + 1
                  time3 = time3 + 86400d0*ncaldayfom
                  time2 = timefom + time3
               end if
               timefom = time2
               return
...
*-----------------------------------------------------------------------
            ...
            time3 = time2 - time1
            time1 = time2
            if ( time3 .lt. 0d0 ) then ! S.H. 2023.5.15
               ncalday = int( dabs(time3) / 86400. ) + 1
               time3 = time3 + 86400d0*ncalday
               time1 = time1 + 86400d0*ncalday
            end if
