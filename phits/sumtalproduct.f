!***********************************************************************
!                                                                      *
      subroutine sumtal_read_tproduct(m,iax,ntf,ierr)
!                                                                      *
!     original routine is read_tproduct in restproduct.f               *
!                                                                      *
...
*-----------------------------------------------------------------------
     ...
     &      das_iterg(iterg(m)),das_ittrg(ittrg(m)),das_itarg(itarg(m)),
     &      trRES(irestalm(m)),
...
*-----------------------------------------------------------------------
  999   continue
      ...
      end subroutine sumtal_read_tproduct


!***********************************************************************
!                                                                      *
      subroutine sumtal_calc_stdev_productreg(m,ntf,
     &                             np,na,nt,ne,nr,
...
!-----------------------------------------------------------------------
      ...
      double precision  trSUMTAL(np,ne,nt,na,nr,2)
...
*-----------------------------------------------------------------------
     ...
     &         trRES(ip,ie,it,ia,ir,2) )
           ...
           if( trSUMTAL(ip,ie,it,ia,ir,1) > cmax )
     &          cmax = trSUMTAL(ip,ie,it,ia,ir,1)
           if( trSUMTAL(ip,ie,it,ia,ir,1) < cmin )
     &          cmin = trSUMTAL(ip,ie,it,ia,ir,1)
...
!-----------------------------------------------------------------------
      if( ntf > 1 ) then
       if( maxcasd /= maxcasres ) then
        ...
        ErrCha = ''
        ErrID = 'L:391/R:sumtal_calc_stdev_productreg/F:sumtalproduct.f' !E58_001_001


        write(*,'(/
     &       '' ***** Error in sumtal calc stdev productreg *****''/)')
...
!-----------------------------------------------------------------------
      ...
      end subroutine sumtal_calc_stdev_productreg



!***********************************************************************
!                                                                      *
      subroutine sumtal_calc_stdev_productrz(m,ntf,
     &                             np,na,nt,ne,nr,nz,
     &                             sumfactor,weightRate,
     &                             trRES,trSUMTAL,
     &                             resc2SUMTAL,resc3SUMTAL,
...
!-----------------------------------------------------------------------
      ...
      double precision  trSUMTAL(np,ne,nt,na,nr,nz,2)
      double precision  resc2SUMTAL
      double precision  resc3SUMTAL
...
*-----------------------------------------------------------------------
     ...
     &          trRES(ip,ie,it,ia,ir,iz,2) )
            ...
            if( trSUMTAL(ip,ie,it,ia,ir,iz,1) > cmax )
     &           cmax = trSUMTAL(ip,ie,it,ia,ir,iz,1)
            if( trSUMTAL(ip,ie,it,ia,ir,iz,1) < cmin )
     &           cmin = trSUMTAL(ip,ie,it,ia,ir,iz,1)
...
!-----------------------------------------------------------------------
      if( ntf > 1 ) then
       if( maxcasd /= maxcasres ) then
        ...
        m_err = 'maxcas is inconsistent.'
        ErrCha = ''
        ErrID = 'L:547/R:sumtal_calc_stdev_productrz/F:sumtalproduct.f' !E58_001_002


        write(*,'(/
     &       '' ***** Error in sumtal calc stdev productrz *****''/)')
        call ErrWrite(ErrID, ErrCha)
        write(*,'('' error = '',(a)/)') trim(m_err)
       end if
      end if
...
!-----------------------------------------------------------------------
      ...
      end subroutine sumtal_calc_stdev_productrz



!***********************************************************************
!                                                                      *
      subroutine sumtal_calc_stdev_productxyz(m,ntf,
...
!-----------------------------------------------------------------------
      ...
      double precision  trSUMTAL(np,ne,nt,na,nx*ny*nz,2)
      double precision  resc2SUMTAL
      double precision  resc3SUMTAL
...
*-----------------------------------------------------------------------
           ...
           do ia = 1, na
            do ip = 1, np
             ...
             if( ntf > 1 ) then
              ...
              ! sig_N1+N2 xi wi = F r1 sig_N1 xi wi
              !                 + F r2 sig_N2 xi wi
              trSUMTAL(ip,ie,it,ia,icf(ix,iy,iz),1) =
     &        trSUMTAL(ip,ie,it,ia,icf(ix,iy,iz),1) +
     &                ( sumfactor * weightRate(ntf) *
     &           trRES(ip,ie,it,ia,icf(ix,iy,iz),1) )
              ...
              ! sig_N1+N2(xiwi)**2 = F**2 r1**2 sig_N1(xiwi)**2
              !                    + F**2 r2**2 sig_N2(xiwi)**2
              trSUMTAL(ip,ie,it,ia,icf(ix,iy,iz),2) =
     &        trSUMTAL(ip,ie,it,ia,icf(ix,iy,iz),2) +
     &                ( sumfactor**2 * weightRate(ntf)**2 *
     ...
     &           trRES(ip,ie,it,ia,icf(ix,iy,iz),2) )
             ...
             if( trSUMTAL(ip,ie,it,ia,icf(ix,iy,iz),1) > cmax )
     &             cmax = trSUMTAL(ip,ie,it,ia,icf(ix,iy,iz),1)
             if( trSUMTAL(ip,ie,it,ia,icf(ix,iy,iz),1) < cmin )
     &             cmin = trSUMTAL(ip,ie,it,ia,icf(ix,iy,iz),1)
...
!-----------------------------------------------------------------------
      if( ntf > 1 ) then
       if( maxcasd /= maxcasres ) then
        ...
        m_err = 'maxcas is inconsistent.'
        ErrCha = ''
        ErrID = 'L:711/R:sumtal_calc_stdev_productxyz/F:sumtalproduct.f' !E58_001_003


        write(*,'(/
     &       '' ***** Error in sumtal calc stdev productxyz *****''/)')
        call ErrWrite(ErrID, ErrCha)
        write(*,'('' error = '',(a)/)') trim(m_err)
       end if
      end if
...
!-----------------------------------------------------------------------
      ...
      end subroutine sumtal_calc_stdev_productxyz



!***********************************************************************
!                                                                      *
      subroutine sumtal_calc_average_productreg(m,ntf,nfile,
     &                               np,na,nt,ne,nr,
         ...
         do it = 1, nt
          do ia = 1, na
           do ip = 1, np
            ...
            ! X_bar
            trSUMTAL(ip,ie,it,ia,ir,1) =
     &      trSUMTAL(ip,ie,it,ia,ir,1) * sumfactor/sumWR

            ! sig_x
            trSUMTAL(ip,ie,it,ia,ir,2) = sqrt(
     &      trSUMTAL(ip,ie,it,ia,ir,2)) * sumfactor/sumWR


            ! Sigma(xi wi)**2 = (sig_X**2 N(N-1)+N X_bar**2)(W/N)**2
            trSUMTAL(ip,ie,it,ia,ir,2) =
     &     (trSUMTAL(ip,ie,it,ia,ir,2)**2 *
     &               resc3SUMTAL * (resc3SUMTAL-1.0d0) +
     &               resc3SUMTAL *
     &      trSUMTAL(ip,ie,it,ia,ir,1)**2) *
     &              (resc2SUMTAL/resc3SUMTAL)**2

            ! Sigma xi wi = X_bar W
            trSUMTAL(ip,ie,it,ia,ir,1) =
     &      trSUMTAL(ip,ie,it,ia,ir,1) * resc2SUMTAL
...
!-----------------------------------------------------------------------
      ...
      end subroutine sumtal_calc_average_productreg



!***********************************************************************
!                                                                      *
      subroutine sumtal_calc_average_productrz(m,ntf,nfile,
     &                               np,na,nt,ne,nr,nz,
     &                               sumfactor,weightRate,sumWR,
     &                               trRES,trSUMTAL,
     &                               resc2SUMTAL,resc3SUMTAL,
          ...
          do it = 1, nt
           do ia = 1, na
            do ip = 1, np
             ...
             ! X_bar
             trSUMTAL(ip,ie,it,ia,ir,iz,1) =
     &       trSUMTAL(ip,ie,it,ia,ir,iz,1) * sumfactor/sumWR

             ! sig_x
             trSUMTAL(ip,ie,it,ia,ir,iz,2) = sqrt(
     &       trSUMTAL(ip,ie,it,ia,ir,iz,2)) * sumfactor/sumWR


             ! Sigma(xi wi)**2 = (sig_X**2 N(N-1)+N X_bar**2)(W/N)**2
             trSUMTAL(ip,ie,it,ia,ir,iz,2) =
     &      (trSUMTAL(ip,ie,it,ia,ir,iz,2)**2 *
     &                resc3SUMTAL * (resc3SUMTAL-1.0d0) +
     &                resc3SUMTAL *
     &       trSUMTAL(ip,ie,it,ia,ir,iz,1)**2) *
     &               (resc2SUMTAL/resc3SUMTAL)**2

             ! Sigma xi wi = X_bar W
             trSUMTAL(ip,ie,it,ia,ir,iz,1) =
     &       trSUMTAL(ip,ie,it,ia,ir,iz,1) * resc2SUMTAL
...
!-----------------------------------------------------------------------
      ...
      end subroutine sumtal_calc_average_productrz



!***********************************************************************
!                                                                      *
      subroutine sumtal_calc_average_productxyz(m,ntf,nfile,
     &                               np,na,nt,ne,nx,ny,nz,
     &                               sumfactor,weightRate,sumWR,
     &                               trRES,trSUMTAL,
     &                               resc2SUMTAL,resc3SUMTAL,
           ...
           do it = 1, nt
            do ia = 1, na
             do ip = 1, np
              ...
              ! X_bar
              trSUMTAL(ip,ie,it,ia,icf(ix,iy,iz),1) =
     &        trSUMTAL(ip,ie,it,ia,icf(ix,iy,iz),1) * sumfactor/sumWR

              ! sig_x
              trSUMTAL(ip,ie,it,ia,icf(ix,iy,iz),2) = sqrt(
     &        trSUMTAL(ip,ie,it,ia,icf(ix,iy,iz),2)) * sumfactor/sumWR


              ! Sigma(xi wi)**2 = (sig_X**2 N(N-1)+N X_bar**2)(W/N)**2
              trSUMTAL(ip,ie,it,ia,icf(ix,iy,iz),2) =
     &       (trSUMTAL(ip,ie,it,ia,icf(ix,iy,iz),2)**2 *
     &        resc3SUMTAL * (resc3SUMTAL-1.0d0) +
     &        resc3SUMTAL *
     &        trSUMTAL(ip,ie,it,ia,icf(ix,iy,iz),1)**2) *
     &        (resc2SUMTAL/resc3SUMTAL)**2

              ! Sigma xi wi = X_bar W
              trSUMTAL(ip,ie,it,ia,icf(ix,iy,iz),1) =
     &        trSUMTAL(ip,ie,it,ia,icf(ix,iy,iz),1) * resc2SUMTAL
...
!-----------------------------------------------------------------------
      ...
      end subroutine sumtal_calc_average_productxyz
