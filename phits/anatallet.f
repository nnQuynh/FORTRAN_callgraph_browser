!***********************************************************************
!                                                                      *
      subroutine anatal_calc_anova_letreg(m,ntf,
     &                             np,ne,nr,
...
!-----------------------------------------------------------------------
      return
      end subroutine anatal_calc_anova_letreg



!***********************************************************************
!                                                                      *
      subroutine anatal_calc_anova_letrz(m,ntf,
     &                             np,ne,nr,nz,
     &                             sumfactor,weightRate,
     &                             trRES,tranatal,nfile,
     &                             resc2SUMTAL,resc3SUMTAL,
...
!-----------------------------------------------------------------------
      return
      end subroutine anatal_calc_anova_letrz



!***********************************************************************
!                                                                      *
      subroutine anatal_calc_anova_letxyz(m,ntf,
     &                             np,ne,nx,ny,nz,
     &                             sumfactor,weightRate,
     &                             trRES,tranatal,nfile,
     &                             resc2SUMTAL,resc3SUMTAL,
...
!-----------------------------------------------------------------------
      ...
      end subroutine anatal_calc_anova_letxyz



************************************************************************
*                                                                      *
      subroutine anatal_pletreg(m,np,nr,mr,ne,kr,eb,tr,
     &                   nfile,weightRate,
     &                   nvl,ivl,rvl,
     &                   nx,ny,nz,xm,ym,zm,igsh,idasa,manatally)
*                                                                      *
*       output the LET tally in region mesh                            *
...
!      dimension   dlr(nr+1) !  integer lr(nr) ---> double dlr(nr)
      real(8),allocatable :: ew(:),vl(:),rdata(:,:),dlr(:) 
...
*-----------------------------------------------------------------------
      ...
      character hsunit(16)*33

      data hsunit( 1) / 'Track [cm/(keV/um)/source]       '/
      data hsunit( 2) / 'Dose [MeV/(keV/um)/source]       '/
      data hsunit( 3) / 'Track [cm/ln(keV/um)/source]     '/
      data hsunit( 4) / 'Dose [MeV/ln(keV/um)/source]     '/
      data hsunit( 5) / 'Track [cm/source]                '/
      data hsunit( 6) / 'Dose [MeV/source]                '/
      data hsunit( 7) / 'Track [1/cm^2/(keV/um)/source]   '/
      data hsunit( 8) / 'Dose [MeV/cm^3/(keV/um)/source]  '/
      data hsunit( 9) / 'Track [1/cm^2/ln(keV/um)/source] '/
      data hsunit(10) / 'Dose [MeV/cm^3/ln(keV/um)/source]'/
      data hsunit(11) / 'Track [1/cm^2/source]            '/
      data hsunit(12) / 'Dose [MeV/cm^3/source]           '/
      data hsunit(13) / 'L*f(L) [dimensionless]           '/
      data hsunit(14) / 'L*d(L) [keV/um]                  '/
...
!-----------------------------------------------------------------------
      ...
      allocate (ew(ne),vl(nr),lr(nr),rdata(2,nfile),dlr(nr+1))
...
*-----------------------------------------------------------------------
     ...
     &          itunt(m) .eq. 13) then  ! T.Sato 2017/11/21
     ...
     &          itunt(m) .eq. 14 ) then ! T.Sato 2017/11/21
...
*-----------------------------------------------------------------------
            ...
            if( itunt(m) .le. 6 .or. itunt(m) .ge. 13) then  ! T.Sato 2017/11/21
...
! sumover
               call pletreg_sumover_stdev_ntf(0,m,ntf,
     &             ip,ie,ir,
     &             rtfac(m),ew(ie),vl(ir),
     &             ew_sum,vl_sum)
...
!sumover
            do iax = 1,itaxn(m)
              ...
              if(ido_ana == 1) then
                 ...
                 call get_letreg_tr_sum_data(m,iax,nfile,
     &                 ip,ie,ir,rdata)
...
C rearrange data from anatalrst to anataldata
     ...
     &         cij,cijaxs,cijaxs2,nanataldata,ierr)
          ...
          if ( itaxs(m,iax) .eq. 1 ) cijaxs = 'LET [keV/um]'
...
*-----------------------------------------------------------------------
           ...
           if( lcy_txt(m) .gt. 0 ) then
            ...
            if( itayl(m) .eq. 0 ) then
               write(iot,'( "y:  ",a33)') hsunit(itunt(m))
            else
...
! T.Sato 2017/11/21 calculate normalization factor
     ...
     &              * delvol(ianataldata,ijaxs)
               ...
               do ip = 1, np
             if( itaxs(m,iax) .eq. 1 ) then ! let axis
                     if( itunt(m) .ne.  5 .and. itunt(m) .ne. 6 .and.
     &                   itunt(m) .ne. 11 .or. itunt(m) .ne. 12 ) then
                     tott(ip,1) = tott(ip,1) / voll
                     end if

             else
                  if( itunt(m) .gt. 6 .and. itunt(m) .lt. 13 ) then
                     tott(ip,1) = tott(ip,1) / voll
                  end if
             end if
               end do
...
! sumover
     ...
     &         1000(1pe13.4,0pf8.4))') (tott(ip,1),tott(ip,2),ip=1,np)   ! T.Sato 2017/11/21
                  ...
                  if( itunt(m) .gt. 6 .or. itunt(m) .lt. 13 ) then
...
*-----------------------------------------------------------------------
            ...
            call prestart(m,iot) !OBINATA(2012.9.5)
...
*-----------------------------------------------------------------------
      ...
      deallocate (ew,vl,lr,rdata,dlr)

      return
      end subroutine anatal_pletreg



************************************************************************
*                                                                      *
      subroutine anatal_pletrz(m,np,nr,nz,ne,rm,zm,eb,
     &                  tr,nfile,weightRate,
     &                  idasa,manatally)
*                                                                      *
*       output r-z scoring mesh LET tally                              *
*       created by T.Miura on 2021/09/13                               *
*                                                                      *
************************************************************************
      use sangelmod, only: itsans
...
! sumover
     ...
     &                          * ( zm(iz+1) - zm(iz) )
        ...
        else ! for L*f(L) or L*d(L), volume should be 1, T.Sato 2017/11/21
         vl(ir,iz,1)=1.0d0
        endif
       enddo
      enddo
...
! sumover
               call pletrz_sumover_stdev_ntf(0,m,ntf,
     &               ip,ie,ir,iz,
...
! sumover
            do iax = 1,itaxn(m)
              ...
              if(ido_ana == 1) then
                 ...
                 call get_letrz_tr_sum_data(m,iax,nfile,
     &                 ip,ie,ir,iz,rdata)
...
*-----------------------------------------------------------------------
           ...
           if( lcy_txt(m) .gt. 0 ) then
            ...
            if( itayl(m) .eq. 0 ) then
               write(iot,'( "y: ",a29)') hsunit(itunt(m))
            else
               write(iot,'( "y: ",200a1)') (itayt(m)(i:i),i=1,itayl(m))
            end if
           end if
...
*-----------------------------------------------------------------------
      ...
      return
      end subroutine anatal_pletrz



************************************************************************
*                                                                      *
      subroutine anatal_pletxyz(m,np,nl,lt,
     &                   nx,ny,nz,ne,xm,ym,zm,eb,tr,
     &                   nfile,weightRate,
     &                   igsh,idasa,manatally)
*                                                                      *
*       output xyz scoring mesh LET tally                              *
*       created by T.Miura on 2021/09/13                               *
*                                                                      *
************************************************************************
      use sangelmod, only: itsans
...
! sumover
      ...
      do ix=1,nx  ! T.Sato 2017/11/21
       do iy=1,ny
        do iz=1,nz
         if(itunt(m).le.12) then ! T.Sato 2017/11/21
...
! sumover
               call pletxyz_sumover_stdev_ntf(0,m,ntf,
     %               ip,ie,ix,iy,iz,
...
!sumover
            do iax = 1,itaxn(m)
              ...
              if(ido_ana == 1) then
                 ...
                 call get_letxyz_tr_sum_data(m,iax,nfile,
...
!***********************************************************************
!                                                                      *
      subroutine get_letreg_tr_sum_data(m,iax,nfile,
     &           ip,ie,ir,rdata)
!                                                                      *
!***********************************************************************
...
!$       end if
        ...
        call get_letreg_tr_sum_data_sub(p_sum,
...
!***********************************************************************
!                                                                      *
      subroutine get_letreg_tr_sum_data_sub(tr_sum,
...
!***********************************************************************
!                                                                      *
      subroutine get_letrz_tr_sum_data(m,iax,nfile,
     &           ip,ie,ir,iz,rdata)
!                                                                      *
!***********************************************************************
...
!$       end if
        ...
        call get_letrz_tr_sum_data_sub(p_sum,
     &     itpan_sum(m,iax),itenm_sum(m,iax),
...
!***********************************************************************
!                                                                      *
      subroutine get_letrz_tr_sum_data_sub(tr_sum,
...
!***********************************************************************
!                                                                      *
      subroutine get_letxyz_tr_sum_data(m,iax,nfile,
     &           ip,ie,ix,iy,iz,rdata)
!                                                                      *
!***********************************************************************
...
!$       end if
        ...
        call get_letxyz_tr_sum_data_sub(p_sum,
     &     itpan_sum(m,iax),itenm_sum(m,iax),
...
!***********************************************************************
!                                                                      *
      subroutine get_letxyz_tr_sum_data_sub(tr_sum,
