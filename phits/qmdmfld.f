...
!$OMP THREADPRIVATE(/caldis/)
      common /qmdswt/ ijmq, imosc, prat
!$OMP THREADPRIVATE(/qmdswt/)

      external erf
*-----------------------------------------------------------------------
* about p   p(1-3): x,y,z momentum
*           p(4)  : Sqrt[ p^2 + mass^2 + 2 mass * Potential] = Energy(pot included)
*           p(5)  : mass
*           p(6)  : Sqrt[ p^2 + mass^2 ] = Energy
*-----------------------------------------------------------------------
      do 100 j = 2, massal
            if( inds(j) .eq. 0 ) goto 100

         empj2 = p(4,j)**2 - p(1,j)**2 - p(2,j)**2 - p(3,j)**2
...
*-----------------------------------------------------------------------
            ...
            if(ijmq .eq. 1) then
             ...
             p(6,j) =sqrt(p(5,j)**2 + p(1,j)**2 + p(2,j)**2 + p(3,j)**2) ! in JAMQMD, p(6) is sometimes missing
            endif
            eij  = p(6,i) + p(6,j)
            empi2 = p(4,i)**2 - p(1,i)**2 - p(2,i)**2 - p(3,i)**2
...
*                                                                      *
************************************************************************
      use QMD_COOD2_MOD, only : rh3d, rr2,rbij,pp2,rha,rhe,rhc
...
!$OMP THREADPRIVATE(/gradu0/)

*-----------------------------------------------------------------------

      dimension bi(3), bij(3), cij(3), rij(3)
      dimension binds(massal)
      dimension ffr(3,nnn), ffp(3,nnn)
*-----------------------------------------------------------------------

               do j = 1, massal

                 binds(j) = min(inds(j),1)  ! control of rho3 (zero if inds(j) == 0. rha if inds(j) >= 1)
...
*-----------------------------------------------------------------------
               ...
               do 100 i = 1, massal
                     ...
                     rho3 = sum(binds(1:massal) * rha(1:massal,i))
...
*-----------------------------------------------------------------------
     ...
     &             +  cl  * rhc(j,i)
             ccpp =   ccpps * p(5,j) / p(4,j)
             ppcc =   ccpps * p(5,i) / p(4,i)

             grbb  = - rbij(j,i)
             ccrrs =   grbb * ccpps * eijinv
             ccrr  =   ccrrs * p(5,j) / p(4,j)
             rrcc  =   ccrrs * p(5,i) / p(4,i)

             do n = 1, 3
               rij(n) =   r(n,i) - r(n,j)
               bij(n) = ( p(n,i) + p(n,j) ) * eijinv
               cij(n) =   bij(n) - bi(n)
               ffr(n,i) = ffr(n,i)
     &                  + binds(j) * ccrr * (rij(n) + grbb*cij(n))
     &                  + binds(j) * rrcc * (rij(n) + grbb*cij(n))
               ffp(n,i) = ffp(n,i)
     &                  - binds(j) * ccpp * (rij(n) + grbb*bij(n))
     &                  - binds(j) * ppcc * (rij(n) + grbb*bij(n))
             end do
...
************************************************************************
      ...
      use QMD_COOD2_MOD, only : qqr, qqp, ggr, ccr, ffr, ggp, ccp, ffp
      implicit real*8(a-h,o-z)
...
************************************************************************
*                                                                      *
      subroutine pauliR(i,ntag,phase)
*                                                                      *
*                                                                      *
*        Last Revised:     2004 09 23                                  *
*                                                                      *
...
************************************************************************
*                                                                      *
      subroutine epotprtR(ipart,epot)
*                                                                      *
*                                                                      *
*        Last Revised:     2006 03 22                                  *
*                                                                      *
*        Purpose:                                                      *
*                                                                      *
*              to calculate potential energy for one nucleon           *
*                                                                      *
*        Variables:                                                    *
*                                                                      *
*              ipart       : nucleon index                             *
*              epot        : potential energy for one nucleon          *
*                                                                      *
*                                                                      *
************************************************************************
      use QMD_COOD2_MOD, only : rr2,rbij,pp2,rha, rhe, rhc
...
*-----------------------------------------------------------------------
               ...
               do j = 1, massal
                  if( inds(j) .eq. 0 ) cycle

                  rhoad = rhoad + rha(j,ipart)
                  rhocd = rhocd + rhe(j,ipart)
                  rhosd = rhosd + rha(j,ipart) * inuc(j) * inuc(ipart)
     &                    * ( 1. - 2. * abs(ichg(j)-ichg(ipart)) )
...
************************************************************************
*                                                                      *
      subroutine etotalR(nsel,it,rs,ps,ekin,epot,ebin,emas,epin,jj)
*                                                                      *
*                                                                      *
*        Last Revised:     2004 09 23                                  *
*                                                                      *
...
*-----------------------------------------------------------------------
      ...
      dimension       it(0:nnn)
      dimension       rhoa(nnn), rho3(nnn), rhos(nnn), rhoc(nnn)
      dimension       rs(3,nnn), ps(3,nnn),  es(nnn), ets(nnn)
...
*-----------------------------------------------------------------------
                  ...
                  if( nsel .eq. 1 )  call pcmclR(it,rs,ps,es,jj)
...
*-----------------------------------------------------------------------
                  ...
                  ebin = ( ett - emas )  / dble( it(0) - ipion )
                  emas =   emas          / dble( it(0) - ipion )
                  ekin =   ekin          / dble( it(0) - ipion )
                  epot =   epot          / dble( it(0) - ipion )
                  epin =   epin          / dble( it(0) - ipion )
...
************************************************************************
*                                                                      *
      subroutine pcmclR(it,rs,ps,es,jj)
*                                                                      *
*                                                                      *
*        Last Revised:     2004 09 23                                  *
*                                                                      *
...
*-----------------------------------------------------------------------
      ...
      dimension       it(0:nnn)
      dimension       rs(3,nnn), ps(3,nnn),  es(nnn), etcm(nnn)
...
*-----------------------------------------------------------------------
     ...
     &                        + ps(3,i)**2 )
                  ...
                  etcm(i) = sqrt( p(4,i)**2 - p(6,i)**2 + p(5,i)**2
     &                          + ps(1,i)**2
     &                          + ps(2,i)**2
     &                          + ps(3,i)**2 )
                  ...
                  cmrx = cmrx + rs(1,i) * etcm(i)
                  cmry = cmry + rs(2,i) * etcm(i)
                  cmrz = cmrz + rs(3,i) * etcm(i)
...
*                                                                      *
************************************************************************
      use QMD_COOD2_MOD, only : mascl, num, isort, isorti, it, rs, ps,
     & rhoa, nqmdm, pp2, rr2, rha
...
*-----------------------------------------------------------------------
            ...
            do j = 1, mclst
                     ...
                     pclx  = pclx + p(1,jp)*(1.d0-0.3d0*prat*imosc*ijmq)
                     pcly  = pcly + p(2,jp)*(1.d0-0.3d0*prat*imosc*ijmq)
                     pclz  = pclz + p(3,jp)*(1.d0-0.3d0*prat*imosc*ijmq)
...
*-----------------------------------------------------------------------
         ...
         if( nprot .gt. 0 .and. nnuet .gt. 0 ) then
               ...
               call etotalR(1,it,rs,ps,ekin,epot,ebin,emas,epin,jj)

                     texc = ebin * dble( nprot + nnuet )
     &                    + bindeg( nprot, nnuet ) / 1000.0
                     texcav = texc * 1.d3 / dble( nprot + nnuet ) ! 2014/4?   ogawa    measure of stability

                  if( (texcav .lt. -1.d-1 .and. isucc .le. 1) .or.
     a             (texcav .lt. -2.d-1 .and. isucc .eq. 2 .and.
     a             nprot + nnuet .gt. 4)) then !
                     ...
                     texc = max( 0.d0, texc )
                     if( nprot + nnuet .le. 4) then
                      call esepa(nprot, nnuet, esn,esp,esa)
                      if( texc .lt. min(esn,esp,esa) ) texc = 0.d0 ! 2015/6/03 ogawa.  No excitation states for light clusters. Avoid prompt gamma from He-4
                     endif
...
*-----------------------------------------------------------------------
            ...
            do j = 1, mclst
                  ...
                  if( ichg(ip) .eq. 1 ) then
                     ...
                     pclx = p(1,ip)* (1.d0-0.3d0*prat*imosc*ijmq)
                     pcly = p(2,ip)* (1.d0-0.3d0*prat*imosc*ijmq)
                     pclz = p(3,ip)* (1.d0-0.3d0*prat*imosc*ijmq)
                     pabs = sqrt( pclx**2 + pcly**2 + pclz**2 )
