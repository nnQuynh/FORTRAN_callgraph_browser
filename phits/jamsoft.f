...
c...Rotation so that p_x=p_y=0.
     ...
     & p2(4))
      the=pyangl(p1(3),p1(1))
      call jamrobo(-the,0.d0,0.d0,0.d0,0.d0,1.0d0,p1(1),p1(2),p1(3),
     & p1(4))
      call jamrobo(-the,0.d0,0.d0,0.d0,0.d0,1.0d0,p2(1),p2(2),p2(3),
     & p2(4))
...
c...Store particle propaties in JAM arrary.
     ...
     $                    bex,bey,bez,gam,phi,the,kf1,kf01)
     ...
     $                     bex,bey,bez,gam,phi,the,kf2,kf02)
      if(ipart2.eq.0) then
        kc=jamcomp(kf2)
...
c...end test TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT
      ...
      if(srt0.gt.emin1+emin2) then
       ...
       write(mstc(38),*)'warning in jamsfex:srt<emin1+emin2, repeat JAM' ! T.Sato 2020/12/18, change warning message
        ierror=10
        return
      endif
...
600   continue
     ...
     $   .or.(epp1/(epm1+0.01d0).lt.etp1/(etm1+0.01d0))) then
        miss=miss+1  ! T.Sato 2020/07/03 avoid infinite loop
        if(miss.gt.100) go to 2000
        goto 30
      endif
...
c...pt kick for proj.
      ...
      dumchk1 = rn(0)
      dumchk2 = rn(0)
      if((kickdip.eq.0.and.dumchk1.lt.0.5d0)
     &     .or.(kickdip.ne.0.and.dumchk2
...
c...pt kick for targ.
      dumchk1 = rn(0)
      dumchk2 = rn(0)
      if((kickdit.eq.0.and.dumchk1.lt.0.5d0)
     &     .or.(kickdit.ne.0.and.dumchk2
...
4000  ierror=4000
      ...
      write(ih,*)' jamsfex not performed, but continue'
      write(ih,*)' ptp=',sqrt(ptp2),' ptt=',sqrt(ptt2)
      write(ih,*)' sw=',sqrt(sw),' snn=',sqrt(snn)
...
c...Transform to comp. frame.
      the=pyangl(p(3,ip),sqrt(p(1,ip)**2+p(2,ip)**2))
      phi=pyangl(p(1,ip),p(2,ip))
      bex=p(1,ip)/p(4,ip)
      bey=p(2,ip)/p(4,ip)
      bez=p(3,ip)/p(4,ip)
      gg1=p(4,ip)/emj
...
c... Iwamoto 2022/11/09
        if(kfm .eq. 131) kfm=311
...
c....Meson daughter
        if(ibb.eq.1) then
          ...
          nv=nv+1
          if(nv.gt.nnn) then
            call jamerrm(30,0,'(jamjdec:)Particle too large [nnn]')
          endif
          indx=nv

c.....Baryon daughter
        else if(ibb.eq.2) then
          call jamindb(indx,mdel,idel)
        endif
...
c---------------------------
        ...
        p(2,indx)=pjet(i,2)
        p(3,indx)=pjet(i,3)
        p(6,indx)=sqrt(pjet(i,5)**2
     $            +pjet(i,1)**2+pjet(i,2)**2+pjet(i,3)**2)
        p(5,indx)=pjet(i,5)
        p(4,indx)=p(6,indx) ! corrected afterwards
...
c...Option: print information.
        if(mstc(8).ge.3) then
	  call pyname(kf1,charn)
          write(mstc(38),810)indx,k(1,indx),k(2,indx),kjet(i,4)
...
c....Boost string to its cms.
 5000 continue
     ...
     $    +sqrt(pjet(i,1)**2+pjet(i,2)**2+pjet(i,3)**2+pjet(i,5)**2)
      ...
      mstu(33)=1
      call pyrobo(1,njet,0.d0,0.d0
     $            ,-dps(1)/dps(4),-dps(2)/dps(4),-dps(3)/dps(4))

c...Rotation so that p_x=p_y=0.
      phi=pyangl(pjet(1,1),pjet(1,2))
      call pyrobo(1,njet,0.0d0,-phi,0d0,0d0,0d0)
      pxy=sqrt(pjet(1,1)**2+pjet(1,2)**2)
      the=pyangl(pjet(1,3),pxy)
      call pyrobo(1,njet,-the,0.d0,0d0,0d0,0d0)
...
cABE add @2014/07/29, to retry cal. for photonuclear reaction
      ...
      mstu(33)=0
      call pyrobo(nsave+1,njet,the,phi,dps(1)/dps(4),dps(2)/dps(4),
     &              dps(3)/dps(4))

c...Write particle list.
      if(mstc(8).ge.5) call pylist(1)
...
c...Rescale boost vector if too close to unity.
        ...
        p3=dp(3)+dgabp*dbz
        p4=dga*(dp(4)+dbp)
        if( dabs(p1).lt.1.d-12 ) p1 = 0.d0
        if( dabs(p2).lt.1.d-12 ) p2 = 0.d0
        if( dabs(p3).lt.1.d-12 ) p3 = 0.d0
      endif
...
c***********************************************************************
      ...
      subroutine jamindb(indx,mdel,idel)
...
c...Find index of newly produced baryon/antibaryon.
      ...
      if(nv+1.gt.nnn.or.nbary+1.gt.nnn)
     $  call jamerrm(30,0,'(jamindb:)Particle too large [nnn]')
...
c...Purpose: to conduct soft radiation according to dipole approxiamtion
      ...
      implicit double precision(a-h, o-z)
      common/pyjets/n,npad,k(1000,5),p(1000,6),v(1000,5)
      save  /pyjets/
!$OMP THREADPRIVATE(/pyjets/)
...
C...Minimize pt1-squared plus pt3-squared.....
        if(p1.ge.p3) then
           psi=.5d0*pyangl(p1**2+p3**2*cos(2.d0*bet),
     & -p3**2*sin(2.d0*bet))
           pt1=p1*sin(psi)
           pz1=p1*cos(psi)
           pt3=p3*sin(psi+bet)
           ...
           psi=.5d0*pyangl(p3**2+p1**2*cos(2.d0*bet),
     & -p1**2*sin(2.d0*bet))
           pt1=p1*sin(bet+psi)
           pz1=-p1*cos(bet+psi)
           pt3=p3*sin(psi)
...
c...Change line number of hadron in case of baryon.
      if(iba.eq.1) then
         if(nv+1.gt.nnn)
     $           call jamerrm(30,0,'(jdecay:)Particle too large [nnn]')
         nbary=nbary+1
...
c...Optional extra suppression of eta and eta'.
        if(kf.eq.221) then
          if(pyr(0).gt.parj(25)) goto 110
        elseif(kf.eq.331) then
          if(pyr(0).gt.parj(26)) goto 110
c...eta(1295)
        endif

	if(kf.eq.-211.and.emf.gt.2*pymass(kf)+0.001d0) kf=-213
	if(kf.eq.211.and.emf.gt.2*pymass(kf)+0.001d0) kf=213
	if(kf.eq.111.and.emf.gt.2*pymass(kf)+0.001d0) kf=113
...
c...Form baryon. distinguish lambda- and sigmalike baryons.
     ...
     &  parf(60+kbary)) kfls=4
        ...
        if(kfls.eq.2.and.kfld.gt.kfle.and.kfle.gt.kflf) then
          ...
          if(kflds.eq.1.and.kfla.ne.kfld) kfll=int(0.25d0+pyr(0))
          if(kflds.eq.3.and.kfla.ne.kfld) kfll=int(0.75d0+pyr(0))
        endif
