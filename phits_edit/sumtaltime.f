!***********************************************************************
!                                                                      *
      subroutine sumtal_read_ttime(m,iax,ntf,ierr)
!                                                                      *
!     original routine is read_ttime in resttime.f                     *
!                                                                      *
...
*-----------------------------------------------------------------------
  999   continue
      ...
      end subroutine sumtal_read_ttime


!***********************************************************************
!                                                                      *
      subroutine sumtal_calc_stdev_timereg(m,ntf,
     &                             np,nt,ne,nr,
...
!-----------------------------------------------------------------------
      ...
      double precision  trSUMTAL(np,ne,nt,nr,2)
...
*-----------------------------------------------------------------------
     ...
     &        trRES(ip,ie,it,ir,2) )
          ...
          if( trSUMTAL(ip,ie,it,ir,1) > cmax )
     &         cmax = trSUMTAL(ip,ie,it,ir,1)
          if( trSUMTAL(ip,ie,it,ir,1) < cmin )
     &         cmin = trSUMTAL(ip,ie,it,ir,1)
...
!-----------------------------------------------------------------------
      if( ntf > 1 ) then
       if( maxcasd /= maxcasres ) then
        ...
        ErrCha = ''
        ErrID = 'L:358/R:sumtal_calc_stdev_timereg/F:sumtaltime.f' !E62_001_001


        write(*,'(/'' ***** Error in sumtal calc stdev timereg *****''
     &            /)')
...
!-----------------------------------------------------------------------
      ...
      end subroutine sumtal_calc_stdev_timereg



!***********************************************************************
!                                                                      *
      subroutine sumtal_calc_stdev_timerz(m,ntf,
     &                             np,nt,ne,nr,nz,
     &                             sumfactor,weightRate,
     &                             trRES,trSUMTAL,
     &                             resc2SUMTAL,resc3SUMTAL,
...
!-----------------------------------------------------------------------
      ...
      double precision  trSUMTAL(np,ne,nt,nr,nz,2)
      double precision  resc2SUMTAL
      double precision  resc3SUMTAL
...
!-----------------------------------------------------------------------
      if( ntf > 1 ) then
       if( maxcasd /= maxcasres ) then
        ...
        m_err = 'maxcas is inconsistent.'
        ErrCha = ''
        ErrID = 'L:510/R:sumtal_calc_stdev_timerz/F:sumtaltime.f' !E62_001_002


        write(*,'(/'' ***** Error in sumtal calc stdev timerz *****''
     &            /)')
        call ErrWrite(ErrID, ErrCha)
        write(*,'('' error = '',(a)/)') trim(m_err)
       end if
...
!-----------------------------------------------------------------------
      ...
      end subroutine sumtal_calc_stdev_timerz



!***********************************************************************
!                                                                      *
      subroutine sumtal_calc_stdev_timexyz(m,ntf,
     &                             np,nt,ne,nx,ny,nz,
     &                             sumfactor,weightRate,
     &                             trRES,trSUMTAL,
     &                             resc2SUMTAL,resc3SUMTAL,
...
!-----------------------------------------------------------------------
      ...
      double precision  trSUMTAL(np,ne,nt,nx,ny,nz,2)
      double precision  resc2SUMTAL
      double precision  resc3SUMTAL
...
*-----------------------------------------------------------------------
     ...
     &          trRES(ip,ie,it,ix,iy,iz,2) )
            ...
            if( trSUMTAL(ip,ie,it,ix,iy,iz,1) > cmax )
     &            cmax = trSUMTAL(ip,ie,it,ix,iy,iz,1)
            if( trSUMTAL(ip,ie,it,ix,iy,iz,1) < cmin )
     &            cmin = trSUMTAL(ip,ie,it,ix,iy,iz,1)
...
!-----------------------------------------------------------------------
      if( ntf > 1 ) then
       if( maxcasd /= maxcasres ) then
        ...
        m_err = 'maxcas is inconsistent.'
        ErrCha = ''
        ErrID = 'L:667/R:sumtal_calc_stdev_timexyz/F:sumtaltime.f' !E62_001_003


        write(*,'(/'' ***** Error in sumtal calc stdev timexyz *****''
     &            /)')
        call ErrWrite(ErrID, ErrCha)
        write(*,'('' error = '',(a)/)') trim(m_err)
       end if
...
!-----------------------------------------------------------------------
      ...
      end subroutine sumtal_calc_stdev_timexyz



!***********************************************************************
!                                                                      *
      subroutine sumtal_calc_average_timereg(m,ntf,nfile,
     &                               np,nt,ne,nr,
        ...
        do ie = 1, ne
         do it = 1, nt
          do ip = 1, np
           ...
           ! X_bar
           trSUMTAL(ip,ie,it,ir,1) =
     &     trSUMTAL(ip,ie,it,ir,1) * sumfactor/sumWR

           ! sig_x
           trSUMTAL(ip,ie,it,ir,2) = sqrt(
     &     trSUMTAL(ip,ie,it,ir,2)) * sumfactor/sumWR


           ! Sigma(xi wi)**2 = (sig_X**2 N(N-1)+N X_bar**2)(W/N)**2
           trSUMTAL(ip,ie,it,ir,2) =
     &    (trSUMTAL(ip,ie,it,ir,2)**2 *
     &           resc3SUMTAL * (resc3SUMTAL-1.0d0) +
     &           resc3SUMTAL *
     &     trSUMTAL(ip,ie,it,ir,1)**2) *
     &          (resc2SUMTAL/resc3SUMTAL)**2

           ! Sigma xi wi = X_bar W
           trSUMTAL(ip,ie,it,ir,1) =
     &     trSUMTAL(ip,ie,it,ir,1) * resc2SUMTAL
...
!-----------------------------------------------------------------------
      ...
      end subroutine sumtal_calc_average_timereg



!***********************************************************************
!                                                                      *
      subroutine sumtal_calc_average_timerz(m,ntf,nfile,
     &                               np,nt,ne,nr,nz,
     &                               sumfactor,weightRate,sumWR,
     &                               trRES,trSUMTAL,
     &                               resc2SUMTAL,resc3SUMTAL,
...
!-----------------------------------------------------------------------
      ...
      end subroutine sumtal_calc_average_timerz



!***********************************************************************
!                                                                      *
      subroutine sumtal_calc_average_timexyz(m,ntf,nfile,
     &                               np,nt,ne,nx,ny,nz,
     &                               sumfactor,weightRate,sumWR,
     &                               trRES,trSUMTAL,
     &                               resc2SUMTAL,resc3SUMTAL,
          ...
          do ie = 1, ne
           do it = 1, nt
            do ip = 1, np
             ...
             ! X_bar
             trSUMTAL(ip,ie,it,ix,iy,iz,1) =
     &       trSUMTAL(ip,ie,it,ix,iy,iz,1) * sumfactor/sumWR

             ! sig_x
             trSUMTAL(ip,ie,it,ix,iy,iz,2) = sqrt(
     &       trSUMTAL(ip,ie,it,ix,iy,iz,2)) * sumfactor/sumWR


             ! Sigma(xi wi)**2 = (sig_X**2 N(N-1)+N X_bar**2)(W/N)**2
             trSUMTAL(ip,ie,it,ix,iy,iz,2) =
     &      (trSUMTAL(ip,ie,it,ix,iy,iz,2)**2 *
     &               resc3SUMTAL * (resc3SUMTAL-1.0d0) +
     &               resc3SUMTAL *
     &       trSUMTAL(ip,ie,it,ix,iy,iz,1)**2) *
     &              (resc2SUMTAL/resc3SUMTAL)**2

             ! Sigma xi wi = X_bar W
             trSUMTAL(ip,ie,it,ix,iy,iz,1) =
     &       trSUMTAL(ip,ie,it,ix,iy,iz,1) * resc2SUMTAL
...
!-----------------------------------------------------------------------
      ...
      end subroutine sumtal_calc_average_timexyz
