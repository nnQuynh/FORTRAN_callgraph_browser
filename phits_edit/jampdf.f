...
! T.Sato 2015/03/10, to avoid too much error messages
        call pyerrm(30,'(pypdfu:)unknown kf')
      endif
...
C...Rescale for gammaVDM by effective gamma -> rho coupling.
  ...
  250     continue
        ...
        write(check1,'(i9)')kf
        call pyerrm(30,'(pypdfu:)unknown kf'//check1)
      endif !)
...
C...Put into output array.
        ...
        call pyerrm(40,'(pypdpr:) invalid mstp(51)')
      endif
...
C...Calculation of x * f(x, Q).
      pyctq2 = max(0.d0, a0 *(x**a1) *((1.d0-x)**a2) *(1.d0+a3*(x**a4))
     &                 *(log(1.d0+1.d0/x))**a5 )
...
C...Stop program if this routine is ever called.
      write(mstu(11),5000)
      if(pyr(0).lt.10d0) call parastop( 114 )
      parm(20)=parm(1)
      value(20)=value(1)
