************************************************************************
*                                                                      *
      subroutine incelfin(ityp,eein,mmas,mchg)
*                                                                      *
*                                                                      *
*       control routine of inc-elf calculation                         *
*       original file was made by H.Iwamoto and S.Nogamine             *
*       modified by S.Hashimoto on 2012/09/17                          *
*                                                                      *
*        call subroutine : incelf                                      *
*                                                                      *
*     input  :                                                         *
*                                                                      *
...
*-----------------------------------------------------------------------
     ...
     &             0.4936, 0.4977, 0.4936, 7*0.0,3.75571,0.0,0.0/

      dimension finput(7)

C     nnnx should be set to the same value of nnn given in
C     param-incelf.inc. (S.H. on 2013.12.5)
      integer, parameter :: nnnx=240
      dimension kind(nnnx),ep(nnnx),alpha(nnnx),beta(nnnx),gam(nnnx)

      save init_incelf
      data init_incelf/1/
!$OMP THREADPRIVATE(init_incelf)
...
*-----------------------------------------------------------------------
            ...
            if ( init_incelf .eq. 1) then
               CALL setupelf
               init_incelf = 0
            end if

            call incelf(finput,nopart,kind,ep,alpha,beta,gam)

*-----------------------------------------------------------------------
*        booking of the result of INC-ELF
*-----------------------------------------------------------------------
...
*-----------------------------------------------------------------------
      ...
      if ( nopart.eq.1 ) then
       if ( (ityp.le.2 .and. ityp.eq.kind(1)+1)
     &  .or. (ityp.eq.18 .and. kind(1).eq.13) )
     &  then ! for p,n,alpha

        if( eein-ep(1) .lt. elevel1st ) then
         nclst = -1
         return
        end if
...
*-----------------------------------------------------------------------
         ...
         if( nopart .gt. 0 ) then
            ...
            do n = 1, nopart
               ...
               else if( lk .eq. 11 ) then ! deuteron
               ...
               else if( lk .eq. 12 ) then ! triton (2009/05/26)
               ...
               else if( lk .eq. 13 ) then ! helium3 (2009/05/27)
               ...
               else if( lk .eq. 14 ) then ! alpha (2009/05/26)
...
*-----------------------------------------------------------------------
     ...
     &          masrs .gt. 0 ) then
                  ...
                  rsmas = 0.93827d0 * dble( mchrs )
     &                  + 0.93958d0 * dble( masrs-mchrs )
...
*-----------------------------------------------------------------------
 ...
 1002          format(/' *** error message from inc-elf ***'
...
*-----------------------------------------------------------------------
      ...
      common /exims/ wapsm(0:150,0:260) ! T.Sato 2017/1/11, 250 -> 260
      common /gemms/ niz(10000), nia(10000), bie(10000)
  
!      data wapsm / 37901*0.0d0 /
      data wapsm / 39411*0.0d0 /  ! T.Sato 2017/1/11, 250 -> 260
...
!$OMP THREADPRIVATE(/cook/)
      ...
      if (iz.ge.0.and.iz.le.150.and.ia-iz.ge.0.and.ia-iz.le.250) then
         energmelf=wapsm(iz,ia-iz)
         if(energmelf.eq.0.d0) goto 10
         return
      endif
...
cKN
     ...
     & *(a-2.d0*z)**2/a**2)+25.8357d0*b**2*(1.d0-1.71219d0*(a-2.d0*z)
     & **2/a**2)*(1.d0-0.62025d0/b**2)**2+0.779d0*z*(z-1.d0)*(1.d0-1.5
     & 849d0/b**2+1.2273d0/a+1.5772d0/(a*b))/b-0.4323d0*exp(1.33333333
     & 33d0*log(z))*(1.d0-0.57811d0/b-0.14518d0/b**2+0.49597d0/a)/b

      energmelf=cam+sz(izk)+sn(ink)
