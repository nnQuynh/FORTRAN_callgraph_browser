...
C                                                                    *
C*********************************************************************
      double precision function four(i,j)
        implicit double precision(a-h, o-z)
        common/pyjets/n,npad,k(1000,5),p(1000,6),v(1000,5)
!$OMP THREADPRIVATE(/pyjets/)
        four=p(i,4)*p(j,4)-p(i,1)*p(j,1)-p(i,2)*p(j,2)-p(i,3)*p(j,3)
      end function
...
C                                                                    *
C*********************************************************************
      double precision function fxk(xx)
        implicit double precision(a-h, o-z)
        common/pydat1/mstu(200),paru(200),mstj(200),parj(200)
        common /fxksz/ szw,szm
!$OMP THREADPRIVATE(/pydat1/)
!$OMP THREADPRIVATE(/fxksz/)
        fxk=2d0*log(xx)+parj(161)*log(1d0-xx)+parj(162)*xx+
...
!$OMP THREADPRIVATE(/pyint3/)
!$OMP THREADPRIVATE(/pyint4/)
!$OMP THREADPRIVATE(/pypars/)
!$OMP THREADPRIVATE(/pysubs/)
!$OMP THREADPRIVATE(/pyint5/)
!$OMP THREADPRIVATE(/pyint6/)
!$OMP THREADPRIVATE(/pyint7/)
!$OMP THREADPRIVATE(/pyint8/)
!$OMP THREADPRIVATE(/pymssm/)
...
C.. Start with qq, if there is one. Only allow for rank 1 popcorn meson
  200   k1=k(ic1,2)
        ...
        if(iabs(k(ic2,2)).ne.21) then
          call pyerrm(2,'(PYPREP:) gg system??')
          goto 320
        endif
...
C...this is usually the case!
  ...
  200 ntry=ntry+1
      ...
      elseif(ntry.gt.100) then
        call pyerrm(14,'(PYSTRF:) 200 caught in infinite loop')
        if(mstu(21).ge.1) return
      endif
...
cABE add @2014/07/29, to retry cal. for photonuclear reaction
        ...
        call pyerrm(14,'(PYSTRF:) 640 caught in infinite loop')
        if(mstu(21).ge.1) return
...
C...Junction strings: initialize flavour, momentum and starting pos.
  ...
  320     ntry=ntry+1
          if(ntry.gt.100.and.ntryr.le.4) then
            ...
            goto 140
          elseif(ntry.gt.100) then
            call pyerrm(14,'(PYSTRF:) 320 caught in infinite loop')
            if(mstu(21).ge.1) return
          endif
          i=isav
          mstu(90)=mstu91
  ...
  260 ntry=ntry+1
     ...
     &  cidc)
      ...
      if(ntry.gt.2000) then
        call pyerrm(4,'(PYDECY:) caught in infinite loop')
        icon=2
        return
      endif
      i=n
...
! T.Sato 2015/03/10, to avoid too much error message
        call parastop( 115 )
      endif
...
C...Initialize generation from given seed.
      if(mrpy(2).eq.0) then         !FURUTA
        ij=mod(mrpy(1)/30082,31329) !FURUTA
        kl=mod(mrpy(1),30082)       !FURUTA
        i=mod(ij/177,177)+2
        j=mod(ij,177)+2
        k=mod(kl/169,178)+1
        l=mod(kl,169)
  ...
  110   continue
        ...
        do 120 i24=1,24
          twom24=0.5d0*twom24
  120   continue
        rrpy(98)=362436d0*twom24   !FURUTA
        rrpy(99)=7654321d0*twom24  !FURUTA
        rrpy(100)=16777213d0*twom24 !FURUTA
        mrpy(2)=1                  !FURUTA
        mrpy(3)=0                  !FURUTA
        mrpy(4)=97                 !FURUTA
        mrpy(5)=33                 !FURUTA
...
C...Generate next random number.
  130 runi=rrpy(mrpy(4))-rrpy(mrpy(5))                !FURUTA
      if(runi.lt.0d0) runi=runi+1d0                   !FURUTA
      rrpy(mrpy(4))=runi                              !FURUTA
      mrpy(4)=mrpy(4)-1                               !FURUTA
      if(mrpy(4).eq.0) mrpy(4)=97                     !FURUTA
      mrpy(5)=mrpy(5)-1                               !FURUTA
      if(mrpy(5).eq.0) mrpy(5)=97                     !FURUTA
      rrpy(98)=rrpy(98)-rrpy(99)                      !FURUTA
      if(rrpy(98).lt.0d0) rrpy(98)=rrpy(98)+rrpy(100) !FURUTA
      runi=runi-rrpy(98)                              !FURUTA
      if(runi.lt.0d0) runi=runi+1d0
      if(runi.le.0d0.or.runi.ge.1d0) goto 130

C...Update counters. Random number to output.
      mrpy(3)=mrpy(3)+1                !FURUTA
      if(mrpy(3).eq.1000000000) then   !FURUTA
        mrpy(2)=mrpy(2)+1              !FURUTA
        mrpy(3)=0                      !FURUTA
      endif
      pyrnd=runi
...
C...Charge, total momentum, transverse momentum, transverse mass.
      elseif(j.le.12) then
        if(j.eq.6) pyp=jamchge(k(i,2))/3d0
