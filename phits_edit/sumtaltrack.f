!***********************************************************************
!                                                                      *
      subroutine sumtal_read_ttrack(m,iax,ntf,ierr)
!                                                                      *
!     original routine is read_ttrack in resttrack.f                   *
!                                                                      *
...
*-----------------------------------------------------------------------
     ...
     &                         tallyfname(ntf,m)(1:ltallyfname(ntf,m))
        call check_ttrack(m,iax,jsn(ioe),jsi(ioe),
...
*-----------------------------------------------------------------------
        ...
        if( itmsh(m) .eq. 1 ) then
          ...
          call read_tracreg(m,iax,ioe,jsn(ioe),jsi(ioe),
     &                    dsin(0,ioe),idsi(0,ioe),ill(0,ioe),ilf(0,ioe),
     ...
     &                    itrnv(m),idas(itriv(m)),das(itrrv(m)) )
          ...
          call read_tracrz(m,iax,ioe,jsn(ioe),jsi(ioe),
     &                    dsin(0,ioe),idsi(0,ioe),ill(0,ioe),ilf(0,ioe),
     ...
     &                    trRES(irestalm(m)))
          ...
          call read_tracxyz(m,iax,ioe,jsn(ioe),jsi(ioe),
     &                    dsin(0,ioe),idsi(0,ioe),ill(0,ioe),ilf(0,ioe),
...
*-----------------------------------------------------------------------
  999   continue
      ...
      end subroutine sumtal_read_ttrack


!***********************************************************************
!                                                                      *
      subroutine sumtal_calc_stdev_tracreg(m,ntf,
     &                             np,ne,nt,nr,nm,
...
!-----------------------------------------------------------------------
      if( ntf > 1 ) then
       if( maxcasd /= maxcasres ) then
        ...
        ErrCha = ''
        ErrID = 'L:374/R:sumtal_calc_stdev_tracreg/F:sumtaltrack.f' !E51_002_001


        write(*,'(/'' ***** Error in sumtal calc stdev tracreg *****''
     &            /)')
...
!-----------------------------------------------------------------------
      ...
      end subroutine sumtal_calc_stdev_tracreg



!***********************************************************************
!                                                                      *
      subroutine sumtal_calc_stdev_tracrz(m,ntf,
...
!-----------------------------------------------------------------------
      ...
      double precision  trSUMTAL(np,ne,nt,nr*nz,na,nm,2)
...
*-----------------------------------------------------------------------
          ...
          do ie = 1, ne
           do ip = 1, np
            ...
            if( ntf > 1 ) then
             ...
             ! sig_N1+N2 xi wi = F r1 sig_N1 xi wi
             !                 + F r2 sig_N2 xi wi
             trSUMTAL(ip,ie,it,icf(ir,iz),ia,im,1) =
     &              trSUMTAL(ip,ie,it,icf(ir,iz),ia,im,1) +
     &              ( sumfactor * weightRate(ntf) *
     &              trRES(ip,ie,it,icf(ir,iz),ia,im,1) )
             ...
             ! sig_N1+N2(xiwi)**2 = F**2 r1**2 sig_N1(xiwi)**2
             !                    + F**2 r2**2 sig_N2(xiwi)**2
             trSUMTAL(ip,ie,it,icf(ir,iz),ia,im,2) =
     &            trSUMTAL(ip,ie,it,icf(ir,iz),ia,im,2) +
     &            ( sumfactor**2 * weightRate(ntf)**2 *
     ...
     &            trRES(ip,ie,it,icf(ir,iz),ia,im,2) )
            ...
            if( trSUMTAL(ip,ie,it,icf(ir,iz),ia,im,1) > cmax )
     &           cmax = trSUMTAL(ip,ie,it,icf(ir,iz),ia,im,1)
            if( trSUMTAL(ip,ie,it,icf(ir,iz),ia,im,1) < cmin )
     &           cmin = trSUMTAL(ip,ie,it,icf(ir,iz),ia,im,1)
...
!-----------------------------------------------------------------------
      if( ntf > 1 ) then
       if( maxcasd /= maxcasres ) then
        ...
        m_err = 'maxcas is inconsistent.'
        ErrCha = ''
        ErrID = 'L:542/R:sumtal_calc_stdev_tracrz/F:sumtaltrack.f' !E51_002_002


        write(*,'(/'' ***** Error in sumtal calc stdev tracrz *****''
     &            /)')
        call ErrWrite(ErrID, ErrCha)
        write(*,'('' error = '',(a)/)') trim(m_err)
       end if
...
!-----------------------------------------------------------------------
      ...
      end subroutine sumtal_calc_stdev_tracrz



!***********************************************************************
!                                                                      *
      subroutine sumtal_calc_stdev_tracxyz(m,ntf,
...
!-----------------------------------------------------------------------
      if( ntf > 1 ) then
       if( maxcasd /= maxcasres ) then
        ...
        m_err = 'maxcas is inconsistent.'
        ErrCha = ''
        ErrID = 'L:707/R:sumtal_calc_stdev_tracxyz/F:sumtaltrack.f' !E51_002_003
...
!-----------------------------------------------------------------------
      ...
      end subroutine sumtal_calc_stdev_tracxyz

!***********************************************************************
!                                                                      *
      subroutine sumtal_calc_average_tracreg(m,ntf,nfile,
     &                               np,ne,nt,nr,nm,
...
C parameter of calc_stdev is needed. (2020.12.16)
            if( trSUMTAL(ip,ie,it,ir,im,1)  < 0d0 ) then
                trSUMTAL(ip,ie,it,ir,im,1)= 0d0
                trSUMTAL(ip,ie,it,ir,im,2)= 0d0
            end if
...
!-----------------------------------------------------------------------
      ...
      end subroutine sumtal_calc_average_tracreg

!***********************************************************************
!                                                                      *
      subroutine sumtal_calc_average_tracrz(m,ntf,nfile,
     &                               np,ne,nt,na,nr,nz,nm,
     &                               sumfactor,weightRate,sumWR,
     &                               trRES,trSUMTAL,
     &                               resc2SUMTAL,resc3SUMTAL,
          ...
          do it = 1, nt
           do ie = 1, ne
            do ip = 1, np
             ...
             ! X_bar
             trSUMTAL(ip,ie,it,icf(ir,iz),ia,im,1) =
     &         trSUMTAL(ip,ie,it,icf(ir,iz),ia,im,1) * sumfactor/sumWR

             ! sig_x
             trSUMTAL(ip,ie,it,icf(ir,iz),ia,im,2) = sqrt(
     &         trSUMTAL(ip,ie,it,icf(ir,iz),ia,im,2)) * sumfactor/sumWR


             ! Sigma(xi wi)**2 = (sig_X**2 N(N-1)+N X_bar**2)(W/N)**2
             trSUMTAL(ip,ie,it,icf(ir,iz),ia,im,2) =
     &            (trSUMTAL(ip,ie,it,icf(ir,iz),ia,im,2)**2 *
     &            resc3SUMTAL * (resc3SUMTAL-1.0d0) +
     &            resc3SUMTAL *
     &            trSUMTAL(ip,ie,it,icf(ir,iz),ia,im,1)**2) *
     &            (resc2SUMTAL/resc3SUMTAL)**2

             ! Sigma xi wi = X_bar W
             trSUMTAL(ip,ie,it,icf(ir,iz),ia,im,1) =
     &            trSUMTAL(ip,ie,it,icf(ir,iz),ia,im,1) * resc2SUMTAL

C S.H. added the following processes for negative values when isumtally=3.
C This is tentative. Correct calculation can be done if processes of
C subroutine calc_stdev is performed here, but preparation of the last
C parameter of calc_stdev is needed. (2020.12.16)
             if( trSUMTAL(ip,ie,it,icf(ir,iz),ia,im,1) < 0d0 ) then
                 trSUMTAL(ip,ie,it,icf(ir,iz),ia,im,1) = 0d0
                 trSUMTAL(ip,ie,it,icf(ir,iz),ia,im,2) = 0d0
              end if
...
!-----------------------------------------------------------------------
      ...
      end subroutine sumtal_calc_average_tracrz



!***********************************************************************
!                                                                      *
      subroutine sumtal_calc_average_tracxyz(m,ntf,nfile,
     &                               np,ne,nt,nx,ny,nz,nm,
     &                               sumfactor,weightRate,sumWR,
     &                               trRES,trSUMTAL,
     &                               resc2SUMTAL,resc3SUMTAL,
...
C subroutine calc_stdev is performed here, but preparation of the last
C parameter of calc_stdev is needed. (2020.12.16)
              if( trSUMTAL(ip,ie,it,icf(ix,iy,iz),im,1) < 0d0 ) then
                 trSUMTAL(ip,ie,it,icf(ix,iy,iz),im,1) = 0d0
                 trSUMTAL(ip,ie,it,icf(ix,iy,iz),im,2) = 0d0
              end if
...
!-----------------------------------------------------------------------
      ...
      end subroutine sumtal_calc_average_tracxyz
...
!***********************************************************************
!                                                                      *
      subroutine sumtal_calc_stdev_tracreg_sub(m, ntf,
     &                                      calcfact1,calcfact2)
!                                                                      *
!***********************************************************************
...
!***********************************************************************
!                                                                      *
      subroutine calc_stdev_restal2sumtally(trRES_SUM,trSUMTAL_SUM,
     &       ln_sum, ntf, calcfact)
!                                                                      *
!***********************************************************************
      ...
      do i=1,2
        if( ntf > 1 ) then
          ...
          trSUMTAL_SUM(1:ln_sum,i) =  trSUMTAL_SUM(1:ln_sum,i)
     &                     +  calcfact(i) * trRES_SUM(1:ln_sum,i)

        else
          trSUMTAL_SUM(1:ln_sum,i) = calcfact(i) * trRES_SUM(1:ln_sum,i)
        end if
      enddo
