...
c                                                                      *
c s hifun    to setut probability for any function                     *
...
c s aran9    to generate random number for vegas0                      *
...
c...the total inelastic cross sections.
      ...
      save  /hiparnt/,/njet/,/pypars/
!$OMP THREADPRIVATE(/hiparnt/)
!$OMP THREADPRIVATE(/njet/)
...
!$OMP THREADPRIVATE(/njet/)
      ...
      c0 = 0.d0
      if( omg1 .ne. 0.d0 ) c0=exp(n*dlog(omg1)-sgmin(n+1))
...
!$OMP THREADPRIVATE(/bveg1/)
!$OMP THREADPRIVATE(/bveg2/)
!$OMP THREADPRIVATE(/bveg3/)
!$OMP THREADPRIVATE(/hiparnt/)
!$OMP THREADPRIVATE(/njet/)
...
c...Setup Monte Calro generation of a specific function.
      ...
      character code*20
      save  /hijhb/
!$OMP THREADPRIVATE(/hijhb/)
      save itag,mstc21
!$OMP THREADPRIVATE(itag,mstc21)
      external fhb
...
200   format('0input parameters for vegas0:  ndim=',i3,'  ncall=',f8.0
     1    /28x,'  it=',i5,'  itmx=',i5/28x,'  acc=',g9.3
     2    /28x,'  mds=',i3,'   nd=',i4/28x,'  (xl,xu)=',
     3    (t40,'( ',g12.6,' , ',g12.6,' )'))
201   format(///' integration by vegas0' / '0iteration no.',i3,
