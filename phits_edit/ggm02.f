...
*-----------------------------------------------------------------------
         ...
         if( m .eq. 31 .or. m .eq. 34 ) then
               lem = km-jmd(1+mn)+1
            if( m .eq. 31 ) then
                  dsmax = das_kmate(kmate(mn)+(lem-1)*5+14)
                  izia = nint(das_kmatg(kmatg(mn)+(lem-1)*3+32))
                  if ( dsmax .le. emin(15) ) goto 230
            else if( m .eq. 34 ) then
                  dsmax = das_kmate(kmate(mn)+(lem-1)*5+15)
                  izia = nint(das_kmatg(kmatg(mn)+(lem-1)*3+32))
...
*-----------------------------------------------------------------------
  ...
  194 if( m .eq. 34 .and. mcal .eq. 0 ) then
        l = lxd(34,mn)
        ht(8:10) = char(mod(l,256))//char(mod(l/256,256))//char(l/65536)
      end if
...
*-----------------------------------------------------------------------
  ...
  244 if( hs .eq. ht ) goto 246
      ...
      i = mxe
      if(mxe.gt.mxe1) then
         write(iom,'(''Error: material (mex) overflow in setmat 2'')')
         ierr = 1
         return
      end if
      call zaid(1,ht,ixl(1,i))
...
*-----------------------------------------------------------------------
  ...
  248 if( mcal .ne. 0 ) goto 262
      ...
      do 260 km = 1, indt
         ...
         do 250 i = 1, mxe
            ...
            if( hs(8:9) .ne. ' ' .and. ht(8:9) .ne. ' ' ) goto 250
            write(iom1,*)
     &      '## warning. '//ht//' and '//hs//
  ...
  250    continue
         ...
         if( mxe .gt. mxe1 ) then
            write(iom,
     &           '(''Error: material (mex) overflow in setmat 3'')')
            ierr = 1
            return
         end if
...
*-----------------------------------------------------------------------
      ...
      do i = 1, mxe
        if( nty(i).eq.0 ) then
          mxe_reduce = mxe_reduce + 1
          iturn(i) = 0
          i1_flag = 1
          cycle
        else if( nty(i).eq.-10 ) then
          i1_flag = 1
        end if
...
c data max for neutron library
      ...
      do km = 1, mix
        ...
        if( lme(1,km).ne.0 ) then ! frtati 2022/06/13
          if( dnmax(2).gt.20.d0 ) then ! frtati 2022/03/23
            if( nty(lme(1,km)).eq.-10 ) then ! use JENDL up to 20MeV
              i2_flag = 1
              lme(1,km) = iturn(lme(1,km))
              das_kmate(kmate(mn)+(lem_tem-1)*5+12) = 20.d0
              ixl(3,lme(1,km)) = ixc(3,lme(1,km)) ! replace extension to neutron default
            else if( nty(lme(1,km)).eq.0 ) then
              i2_flag = 1
              lme(1,km) = 0
              das_kmate(kmate(mn)+(lem_tem-1)*5+12) = 0.d0
            else
              lme(1,km) = iturn(lme(1,km))
            end if
          else ! frtati 2022/06/13
            lme(1,km) = iturn(lme(1,km))
          end if
        end if
      end do
      do km = 1, mix ! frtati 2022/03/22
        if( nty(lme(1,km)).eq.-10 ) nty(lme(1,km)) = 1
      end do
        ...
        do m = 1, mxmat
          ...
          do l = 1, nel
            ...
            do km = 1, mix
              ...
              itz = iza(km) / 1000
              ita = iza(km) - 1000 * itz
              if( m.eq.mn .and. iza(km).eq.1001 ) then ! frtati 2022/03/11
                das_kmatd(kmatd(m)+7) =
     ...
     &          das_kmate(kmate(m)+(lem_tem-1)*5+12)
            ...
            if ( das_kmatd(kmatd(mn)+(l-1)*5+12).gt.t1_dmax ) then
              t1_dmax = das_kmatd(kmatd(m)+(l-1)*5+12)
            end if
            if ( das_kmatd(kmatd(mn)+(l-1)*5+12).lt.t2_dmax ) then
              t2_dmax = das_kmatd(kmatd(m)+(l-1)*5+12)
            end if
          end do
          das_kmatd(kmatd(m)+3) = t1_dmax
...
c data max for proton, deuteron, alpha libraries
     ...
     &       m.eq.9 .or. m.eq.31.or. m.eq.34 ) then
          mn = 1
          do km = 1, mix
            if ( km.ge.jmd(1+mn+1) ) mn = mn + 1
            lem_tem = km-jmd(1+mn)+1
            if( lme(m,km).ne.0 ) then ! frtati 2022/03/23
              if( nty(lme(m,km)).eq.0 ) then
                i2_flag = 1
                lme(m,km) = 0
                if ( m.eq.9 ) then
                  das_kmate(kmate(mn)+(lem_tem-1)*5+11) = 0.d0
                else if ( m.eq.31 ) then
                  das_kmate(kmate(mn)+(lem_tem-1)*5+14) = 0.d0
                else if ( m.eq.34 ) then
                  das_kmate(kmate(mn)+(lem_tem-1)*5+15) = 0.d0
                end if
              else
                lme(m,km) = iturn(lme(m,km))
              end if
            end if
          end do
        end if
     ...
     &          das_kmate(kmate(m)+(lem_tem-1)*5+15)
            ...
            if ( das_kmatd(kmatd(m)+(l-1)*5+11).gt.t1_dmax ) then
              t1_dmax = das_kmatd(kmatd(m)+(l-1)*5+11)
            end if
            if ( das_kmatd(kmatd(m)+(l-1)*5+11).lt.t2_dmax ) then
              t2_dmax = das_kmatd(kmatd(m)+(l-1)*5+11)
            end if
            if ( das_kmatd(kmatd(m)+(l-1)*5+14).gt.t3_dmax ) then
              t3_dmax = das_kmatd(kmatd(m)+(l-1)*5+14)
            end if
            if ( das_kmatd(kmatd(m)+(l-1)*5+14).lt.t4_dmax ) then
              t4_dmax = das_kmatd(kmatd(m)+(l-1)*5+14)
            end if
            if ( das_kmatd(kmatd(m)+(l-1)*5+15).gt.t5_dmax ) then
              t5_dmax = das_kmatd(kmatd(m)+(l-1)*5+15)
            end if
            if ( das_kmatd(kmatd(m)+(l-1)*5+15).lt.t6_dmax ) then
              t6_dmax = das_kmatd(kmatd(m)+(l-1)*5+15)
            end if
          end do
          das_kmatd(kmatd(m)+1) = t1_dmax
          das_kmatd(kmatd(m)+2) = t2_dmax
          das_kmathd(kmathd(m)+3) = t3_dmax
          das_kmathd(kmathd(m)+4) = t4_dmax
          das_kmathd(kmathd(m)+5) = t5_dmax
...
c data max for photo-nuclear library
      if( ispn.eq.1 ) then
        ...
        do km = 1, mix
          if ( km .ge. jmd(1+mn+1) ) mn = mn + 1
          lem_tem = km-jmd(1+mn)+1
          if( lmn(km).ne.0 ) then ! frtati 2022/03/23
            if( nty(lmn(km)).eq.0 ) then
              i2_flag = 1
              lmn(km) = 0
              das_kmate(kmate(mn)+(lem_tem-1)*5+13) = 0.d0
            else
              lmn(km) = iturn(lmn(km))
            end if
          end if
      ...
      if( iemode.ne.0 ) then
        do iex = 1, mxe
          ...
          do km = 1, mix
            if( km .ge. jmd(1+mn+1) ) mn = mn + 1
            lem_tem = km-jmd(1+mn)+1
            if( iza(km).eq.1001 .and.
     &         (lme(1,km).eq.iex .or. lme(9,km).eq.iex .or.
     &          lme(31,km).eq.iex .or. lme(34,km).eq.iex) ) then
              if( lme(1,km).eq.iex ) then
                das_kmatd(kmatd(mn)+7) =
     &          min(das_kmate(kmate(mn)+(lem_tem-1)*5+12),20.d0)
                das_kmate(kmate(mn)+(lem_tem-1)*5+12) =
     &          min(das_kmate(kmate(mn)+(lem_tem-1)*5+12),20.d0)
              else if( lme(9,km).eq.iex ) then
                das_kmatd(kmatd(mn)+6) = 0.d0
                das_kmate(kmate(mn)+(lem_tem-1)*5+11) = 0.d0
              else if( lme(31,km).eq.iex ) then
                das_kmatd(kmatd(mn)+9) = 0.d0
                das_kmate(kmate(mn)+(lem_tem-1)*5+14) = 0.d0
              else if( lme(34,km).eq.iex ) then
                das_kmatd(kmatd(mn)+10) = 0.d0
                das_kmate(kmate(mn)+(lem_tem-1)*5+15) = 0.d0
              end if
            end if
          end do
...
*-----------------------------------------------------------------------
     ...
     &/ 0.99928126,  3.96821753,  6.88137314,  8.93476259, 10.71812779,
     & 11.90752173, 13.88637504, 15.86195669, 18.83519687, 20.00634570,
     & 22.79227500, 24.09620719, 26.74975291, 27.84423070, 30.70768100,
     & 31.78954469, 35.14843998, 39.60482555, 38.76242492, 39.73370878,
     & 44.56971693, 47.45579715, 50.50388557, 51.54942600, 54.46609813,
     & 55.36526191, 58.42692768, 58.18919265, 63.00010624, 64.81835121,
     & 69.12404255, 72.01598397, 74.27798382, 78.28169183, 79.21758237,
     & 83.07813085, 84.73358639, 86.86729785, 88.14210172, 90.44033758,
     & 92.10826505, 95.13565284, 96.07400000,100.20175523,102.02148732,
     &105.50579590,106.94155510,111.44533005,113.83165264,117.69021830,
     &120.71401719,126.50384850,125.81429347,130.16512368,131.76372376,
     &136.14728843,137.71219853,138.91232944,139.69717875,143.00288492,
     &143.66800000,149.06832806,150.65854885,155.89913932,157.56009717,
     &161.10403904,163.51348130,165.82215671,167.48297577,171.56737460,
     &173.46374263,176.95667648,179.39344199,182.26071715,184.60738337,
     &188.59582367,190.56575429,193.40812524,195.27452198,198.86682579,
     &202.62753934,205.42004239,207.18514782,207.20500000,208.19600000,
...
*-----------------------------------------------------------------------
     ...
     &                   ''section.'')')
                write(*,'(''Atomic weight of '',a6,'' is zero.'')') hm !S.H. added (2017.1.24)
                write(*,'(''Please try natural=0 in [parameters] ''
     &                   ''section.'')')
                ierr = 1
                return
...
*-----------------------------------------------------------------------
      ...
      do 40 i = jb, mxe+nt
            call zaid(2,hc,ixc(1,i))
         do 10 j = 11, 181
            k = mod(ixc((j+1)/3,i)/256**mod(j+1,3),256)
            if( k .eq. 0 ) goto 20
   10       hc(j:j) = char(k)
   20       iu = 10
         do 30 j = 2, 7
...
*-----------------------------------------------------------------------
  ...
  240    do 260 m = jmd(1+i), jmd(1+i+1) - 1
         ...
         do 250 j = 1, 3
            ...
            lmt(m) = iex
            iq(j) = 0
            if( iz(mod(j,3)+1)/1000 .eq. iz(j)/1000 )
     &         iq(mod(j,3)+1) = 0
...
*-----------------------------------------------------------------------
     ...
     &   nint(xss(jxs(31,iex)+ip-1)).eq.1)go to 358
      do ix=1,nint(xss(jxs(31,iex)+ip-1))
        do k=1,nxs(4,iex)
          if(nint(xss(ixs(2,ip,iex)+ix-1)).eq.
     &       nint(xss(jxs(3,iex)+k-1)))
     &      xss(ixs(5,ip,iex)+nint(xss(ixs(4,ip,iex)+ix-1)))=k
        enddo
      enddo
  358 continue
*-----------------------------------------------------------------------
...
! T.Sato 2014/9/18, following checks are not necessary for EGS mode
  ...
  444    format(/29h maximum photon energy set to,e11.4,
     ...
     &                ''some photon cross-section tables'')')
            write(jom,'(/''Error : maximum electron energy > '',
     &                ''some photon cross-section tables'')')
            ierr = 1
            return
         end if
   ...
   60    format(a70,a10/4(i7,f11.0)/4(i7,f11.0)/
     &          4(i7,f11.0)/4(i7,f11.0)/8i9/8i9/8i9/8i9/8i9/8i9)
         ...
         do i=10,1,-1 ! JIMD 2021/10/11, avoid ifeng>2 for thermal kernel
          if(hz(i:i).ne.' ') exit
         enddo
         if(hz(i:i).eq.'t'.and.nxs(7,iex).gt.2) then
          write(*,*) 'in thermal kernel ',hz
          write(*,*) 'ifeng > 2 option is not available in PHITS'
          stop
         endif
         ihtnele = index(htn,'e')
         if( nty(iex) .ne. ihtnele )
     &      read(iux,70,err=200) (xss(lp+i),i=1,ly(3))
         if( nty(iex) .eq. ihtnele )
...
*-----------------------------------------------------------------------
   ...
   80    read(iux,rec=ly(2),err=200) hz, aw(0), tz, hd, hk, hm,
     ...
     &       (jxs(i,iex),i=1,32)
         ...
         ihtnele = index(htn,'e')
            if( k .ne. ihtnele ) read(iux,rec=ly(2)+i,err=200)
     &                          (xss(j),j=j1,j2)
...
*-----------------------------------------------------------------------
      ...
      do 10 m = jmd(1+j),jmd(1+j+1)-1
         if(lme(1,m).eq.iex) lc = 1
         if(kpt(2).eq.0) goto 10
...
*-----------------------------------------------------------------------
      ...
      do 84 ip = 1, npert
         n1 = iptb(9,ip)
         if(n1.eq.0) goto 84
      do 83 m = nptb(ip)+1,nptb(ip+1)
         if(lme(1,nint(dptb(1,m))).ne.iex) goto 83
      do 82 i = 1, n1
         i1 = nint(rptb(iptb(10,ip)+i-1))
      do 81 k = 1, nr
   81    if(jf(k).eq.i1) goto 82
         nr = nr + 1
         jf(nr) = i1
   82 continue
         goto 84
...
*-----------------------------------------------------------------------
  ...
  310    if(nx(4).le.0) goto 380
      ...
      do 330 i = 3, 6
         ...
         jx(i) = lw
      do 320 j = 1, nxs(4,iex)
         if(m2(j).eq.0) goto 320
         xss(lw) = xss(jxs(i,iex)+j-1)
         lw = lw+1
  320 continue
  330 continue
...
*-----------------------------------------------------------------------
         ...
         xss(ib+i) = lw-jx(9)+1
      do 430 k = 1, 33
  430    xss(lw+k-1) = xss(jxs(9,iex)+ic+k-2)
         lw = lw+33
         goto 440
 432     xss(ib+i) = -(lw-jx(9)+1)
         lk = jxs(9,iex)-1-ic
         np = nint(xss(lk+1))
         nk = np*3+2
      do 434 k = 1, nk
  434    xss(lw+k-1) = xss(lk+k-1)
         lw = lw+nk
  440 continue
  450 continue
...
*-----------------------------------------------------------------------
         ...
         jx(10) = lw
         iw = 0
      do 460 i = 1, nxs(5,iex)
         if(m2(i).eq.0) goto 460
         xss(lw) = xss(jxs(10,iex)+i-1)
         if(m2(i).eq.18.and.jxs(2,iex).ge.jxs(3,iex))
     &    iw=nint(xss(lw))
         lw = lw+1
  460 continue
         jx(11) = lw
...
*-----------------------------------------------------------------------
  ...
  480    if(jxs(23,iex).eq.0) goto 483
         ...
         if(jxs(23,iex).lt.lw) then
            write(iom,'(''Error : unresolved data overwritten.'',a)') ht
            ierr = 1
            return
         end if
     ...
     &                        xunru = xss(l+5+nint(xss(l)))
         jx(23) = lw
         in = 6+nint(xss(l))*(1+6*nint(xss(l+1)))
      do 482 i = 0, in-1
...
*-----------------------------------------------------------------------
  ...
  486    xss(lw+i-1) = xss(jxs(26,iex)+i-1)
         ...
         jx(27) = lw
         mo = jxs(22,iex)-jxs(27,iex)+1
         if(jxs(13,iex).ne.0) mo=jxs(13,iex)-jxs(27,iex)
         if(jxs(12,iex).ne.0) mo=jxs(12,iex)-jxs(27,iex)
      do 487 i = 1, mo
...
*-----------------------------------------------------------------------
  ...
  510 do 540 i = 1, nxs(6,iex)
         ...
         if(nint(xss(j)).eq.12.or.nint(xss(j)).eq.16) goto 520
         if(j2.gt.nint(xss(j+1)).and.
     &      jd.lt.nint(xss(j+1))+nint(xss(j+2))-1) goto 540
         goto 530
  520    j = j+2*nint(xss(j+2))+3
         if(xss(j+1).lt.em.and.xss(j+nint(xss(j))).gt.ec) goto 540
  530    nx(6) = nx(6)-1
         m2(i) = 0
  540 continue
...
*-----------------------------------------------------------------------
      ...
      do 570 n = 1, nxs(6,iex)
         if(m2(n).eq.0) goto 570
         xss(lw) = xss(jxs(i,iex)+n-1)
         lw = lw+1
  570 continue
  580 continue
...
*-----------------------------------------------------------------------
      ...
      do 640 j = 1, nx(6)
         ...
         lw = lw+1
         ib = ib+1
         if(nint(xss(ib-1)).eq.12.or.nint(xss(ib-1)).eq.16) goto 600
         jb = nint(xss(ib))+nint(xss(ib+1))-1
         ic = max(nint(xss(ib)),j1)
         lr = ic-nint(xss(ib))+ib+2
         xss(lw) = ic-j1+1
         xss(lw+1) = min(jb,j2)-ic+1
      do 590 i = 1, nint(xss(lw+1))
...
*-----------------------------------------------------------------------
  ...
  620    xss(ib) = i
         n = 2*nint(xss(ib+1))+2*nint(xss(ib+2+2*nint(xss(ib+1))))+3
      do 630 i = 1, n
...
*-----------------------------------------------------------------------
      ...
      do 710 j = 1, nx(6)
         i = jx(16)+j-1
         if(nint(xss(i)).eq.0) goto 710
         iw = jxs(17,iex)+nint(xss(i))-1
         xss(i) = lw-jx(17)+1
         m = nint(xss(iw))
         i1 = 1
         i2 = 1
  ...
  670    xss(lw+i-1)  =xss(iw+i1+i-1)
      do 680 i = 1, nd
  680    xss(lw+nd+i-1) = xss(iw+i1+m+i-1)
         lw = lw+2*nd
      do 700 i = 1, nd
         ic = nint(xss(ib+i))
         if(ic.eq.0) goto 700
         ...
         xss(ib+i) = lw-jx(17)+1
      do 690 k = 1, 33
  690    xss(lw+k-1) = xss(jxs(17,iex)+ic+k-2)
         lw = lw + 33
         goto 700
  692    xss(ib+i) = -(lw-jx(17)+1)
         lk = jxs(17,iex)-1-ic
         np = nint(xss(lk+1))
         nk = np*3+2
      do 694 k = 1, nk
  694    xss(lw+k-1) = xss(lk+k-1)
         lw = lw+nk
  700 continue
  710 continue
...
*-----------------------------------------------------------------------
      ...
      do 720 n = 1, nxs(6,iex)
         ...
         xss(lw) = xss(jxs(18,iex)+n-1)
         lw = lw+1
  720 continue
         jx(19) = lw
         ns = jxs(20,iex)-jxs(19,iex)
         if(ns.le.0) ns = jxs(22,iex)-jxs(19,iex)+1
      do 730 i = 1, ns
...
*-----------------------------------------------------------------------
  ...
  740    if(npikmt.eq.0) goto 800
         ...
         read(ht,'(e9.0)') tr
         ip = lpik+1
  750    if(abs(tr-pik(ip)).le.1e-06.or.aint(tr).eq.pik(ip)) goto 760
...
*-----------------------------------------------------------------------
      ...
      do 812 i = 30, 31
         jx(i) = lw
      do 813 j = 1, nxs(7,iex)
         if(m2(j).eq.0) goto 813
         xss(lw) = xss(jxs(i,iex)+j-1)
         lw = lw+1
  813 continue
  812 continue
...
*-----------------------------------------------------------------------
     ...
     &         xss(lr+j+1+nint(xss(ib+1)))
        ...
        if ( xss(jxs(1,iex)+nint(xss(lw))-2+j) .gt. 20d0 ) then
         if(ih.eq.1) xss(jxs(1,iex)+4*nx(3)+nint(xss(lw))-2+j) =
     &          xss(jxs(1,iex)+4*nx(3)+nint(xss(lw))-2+j)-
     &          xss(lr+j+1+nint(xss(ib+1)))
        end if
...
*-----------------------------------------------------------------------
         ...
         np = nint(xss(lb))
         xss(lb-1) = 9999
      do 840 ns = lb+1,lb+np-1
  840    if(xss(ns+1).gt.ecf(2)) goto 850
         goto 860
  850    wc = 1.-xss(ns+2*np)-(ecf(2)-xss(ns))*xss(np+ns)
         if(jj.eq.2) wc = wc-.5*(ecf(2)-xss(ns))**2*
     &    (xss(ns+np+1)-xss(ns+np))/(xss(ns+1)-xss(ns))
         xss(lb) = .5*wc+np
         if(xss(lb).gt.np) xss(lb-1) = 10000*jj+ns-lb
  860 continue
...
*-----------------------------------------------------------------------
      ...
      do 60 i = 1, int(tds(iv))
         ip = abs(tds(iv+i))
         if( ip .le. 1 ) goto 60
         ii = tds(ip-1)
         n2 = 1
         if( ii .lt. 0 ) n2 = tds(ip)
         n1 = 1
         if( n2 .eq. 1 ) n1 = mod(int(abs(tds(ip))),10000000)
         i1 = 1
      do 50 j2 = 1, n2, 2
         if( n2 .gt. 1 ) ii = tds(ip+j2)
      do 40 m = jmd(1+ii), jmd(1+ii+1) - 1
         if( lme(jp,m) .ne. iex ) goto 40
      do 30 j = 1, n1
...
*-----------------------------------------------------------------------
  ...
  190    m = max(m,nint(xss(l+i-1)))
         ...
         if( m .gt. 0 )
     &   call mgex(jxs(1,iex)-1+nint(xss(jxs(17,iex)+k)),i)
         call mgex(l,n*nxs(5,iex))
  200    call mgex(jxs(1,iex)-1+nint(xss(jxs(13,iex)+k)),
     &             n*nxs(5,iex))
  ...
  210 continue
      ...
      do 220 k = 1, nxs(8,iex)
         ...
         n = n + 1
         xss(jxs(11,iex)+n-1) = nint(xss(jxs(11,iex)+k-1))
         xss(jxs(12,iex)+n-1) = nint(xss(jxs(12,iex)+k-1))
         xss(jxs(13,iex)+n) = nint(xss(jxs(13,iex)+k))
         xss(jxs(14,iex)+n-1) = nint(xss(jxs(14,iex)+k-1))
         xss(jxs(15,iex)+n-1) = nint(xss(jxs(15,iex)+k-1))
         xss(jxs(16,iex)+n) = nint(xss(jxs(16,iex)+k))
...
*-----------------------------------------------------------------------
         ...
         n = 0
         l = jxs(1,iex) - 1 + nint(xss(jxs(13,iex)))
         if( m .ne. 3 ) goto 250
...
*-----------------------------------------------------------------------
   ...
   50    p(l,4-k) = (1.+2.*b) * p(l,5-k) + p(1,5-k)
     &            - ((l+1)*p(l+1,5-k)+l*p(l-1,5-k)) / (2*l+1)
      ...
      do 60 m = 1, 4
   60    t = t + exs(jxs(4,j)+14*(n-1)+1+m)*p(l,m)
         if( l .lt. 7 ) t = t - 2. * exs(jxs(4,j)+14*(n-1)+6+l)
     &                    / (2*l+1)
...
*-----------------------------------------------------------------------
  ...
  165    if( iz .eq. iza(j)/1000 ) fm = fm + fme(j)
      ...
      do 170 n = 1, nee
         ...
         if( r .gt. 1. )
     &      pxr(n+nee*(mkc-1))
     &      = cn * (.42*((s+1.)**2 * log(1.19*s*(s+2.)/a)-s*(s+2.))
     &      + 1.5 * ((s+1.)**2 - (.5*s**2+2.*s+1.+(2.*s+1.)*log(r))/r))
     &      / (s*(s+2.))
  170    if( n .gt. 1 .and. eee(n) .gt. ecf(3))
     &      d = d + .5 * (pxr(n-1+nee*(mkc-1))
     &        + pxr(n+nee*(mkc-1))) * (rng(n-1+nee*(mkc-1))
     &        - rng(n+nee*(mkc-1)))
         xnm(mkc) = 1.
...
*-----------------------------------------------------------------------
  ...
  174 format(/43h1electron secondary production for material,i5,56x,
     1 14hprint table 86//9x,6henergy,11x,14hstopping power,11x,
     2 5hbrems,3x,9hthick tgt,3x,7hk x-ray,3x,8hknock-on/4x,1hn,14x,
...
*-----------------------------------------------------------------------
         ...
         s = (e-.5*de) / gpt(3)
         t2 = (s+1.)**2
         asp(n+nee*(mkc-1)) = c1 * az * t2 / (s*(s+2.))
         ear(n+nee*(mkc-1)) = log((e-.5*de)
     &                           / (dr*asp(n+nee*(mkc-1))))
     &                           + c5 + c4 * s**2 / t2
...
*-----------------------------------------------------------------------
   ...
   90       format(25h1range table for material,i5,a12,62x,
     ...
     &             24hmean ionization energy =,1pe12.5,4h ev./)
      ...
      do 92 j = jmd(1+mkc), jmd(1+mkc+1) - 1
         iz = iza(j) / 1000
         ns = nxs(11,lme(3,j))
         write(iuo,'(6h   z =,i4)') iz
         write(iuo,'(4x,20hocc no, be(ev) pairs)')
   ...
   96    format(9x,6henergy,11x,14hstopping power,11x,5hrange,3x,
     ...
     &          3x,6hdrange,4x,6hdyield/4x,1hn,14x,
     &          19hcollision radiation,3x,5htotal,15x,
...
*-----------------------------------------------------------------------
      ...
      do 20 j = jmd(1+mkc), jmd(1+mkc+1) - 1
         ...
         ns = nxs(11,lme(3,j))
      do 20 n = 1, ns
         zs = exs(jxs(11,lme(3,j))+n-1)
         nt = nt + 1
         fo(nt) = fme(j) * abs(zs) / az
         if( jcond(mkc) .ne. 0 .and. zs .le. 0. ) goto 10
...
*-----------------------------------------------------------------------
      ...
      do 90 m = 2, maxi
         ...
         if( cm .ge. calph(m) .and. cm .lt. calph(m-1) )
...
*-----------------------------------------------------------------------
     ...
     3 .09815,.09854,.09893,.09933,.09973,.10013,.10054,.10094,.10136,
     4 .10177,.10219,.10261,.10304,.10347,.10390,.10434,.10478,.10522,
     5 .10567,.10612,.10657,.10703,.10750,.10796,.10844,.10891,.10939,
     6 .10988,.11037,.11086,.11136,.11186,.11237,.11288,.11339,.11391,
     7 .11444,.11497,.11551,.11605,.11659,.11714,.11770,.11826,.11883,
     8 .11940,.11998,.12057,.12116,.12175,.12235,.12296,.12357,.12419,
     9 .12482,.12545,.12609,.12674,.12739,.12805,.12872,.12939,.13008/
      data (ca(i),i= 87,172)/      .13076,.13146,.13216,.13287,.13359,
     1 .13432,.13506,.13580,.13655,.13731,.13808,.13886,.13964,.14044,
     2 .14125,.14206,.14288,.14372,.14456,.14542,.14628,.14716,.14804,
     3 .14894,.14985,.15077,.15170,.15264,.15360,.15456,.15554,.15654,
     4 .15754,.15856,.15959,.16064,.16170,.16278,.16387,.16497,.16609,
     5 .16723,.16838,.16955,.17074,.17194,.17316,.17440,.17566,.17693,
     6 .17823,.17954,.18087,.18223,.18361,.18500,.18642,.18786,.18933,
     7 .19082,.19233,.19387,.19543,.19702,.19864,.20028,.20196,.20366,
     8 .20539,.20715,.20894,.21076,.21262,.21451,.21643,.21839,.22039,
     9 .22242,.22450,.22661,.22876,.23096,.23319,.23547,.23780,.24017/
      data (ca(i),i=173,258)/      .24260,.24507,.24759,.25016,.25279,
     1 .25547,.25821,.26100,.26386,.26678,.26976,.27281,.27593,.27911,
     2 .28237,.28570,.28910,.29259,.29615,.29980,.30353,.30735,.31126,
     3 .31526,.31936,.32355,.32785,.33225,.33676,.34138,.34611,.35096,
     4 .35593,.36102,.36623,.37158,.37706,.38267,.38842,.39433,.40038,
     5 .40658,.41293,.41944,.42611,.43295,.43995,.44713,.45449,.46202,
     6 .46973,.47763,.48572,.49399,.50246,.51112,.51998,.52903,.53828,
     7 .54773,.55737,.56722,.57726,.58749,.59791,.60853,.61932,.63030,
     8 .64144,.65275,.66421,.67583,.68757,.69944,.71142,.72349,.73564,
     9 .74785,.76010,.77236,.78462,.79685,.80902,.82111,.83308,.84491/
...
*-----------------------------------------------------------------------
     ...
     &      kx = min(max(51,int(exp(1.794-.397*log(ea)))),240)
         ae = log(e*1000.)
         j = min(max(1,int(log(.256/e)/log(two))),7)
         x1 = log(exs(jxs(4,ix)+14*(j-1)))
         x2 = log(exs(jxs(4,ix)+14*j))
      ...
      do 10 l = 2, kx
         ...
         c3 = exs(jxs(7,ix)+l-2+240*(j+1))
         ad = c1*(x2-x3)-c2*(x1-x3)+c3*(x1-x2)
         bd = (x1**2)*(c2-c3)-(x2**2)*(c1-c3)+(x3**2)*(c1-c2)
...
*-----------------------------------------------------------------------
   ...
   60    if( abs(ap(n,j)) .gt. abs(ap(mx,j)) ) mx = n
      ...
      do 70 m = 1, 6
         ...
         ap(mx,m) = ap(j,m)
   70    ap(j,m) = t
   80    if( abs(ap(j,j)) .lt. 1e-6 * abs(ap(1,1)) ) then
            write(iom,'(/''Error : in sing, am is '',
     &                   ''an ill-conditined matrix.'')')
            ierr = 1
            return
         end if
  ...
  190    ta(3,l) = e3 * ta(1,l) + ta(1,2)
     ...
     &                        + am(5,6)*(e3**2+2.*third*e3+third)))
         p(3) = p(3) + wq * ((1.+am(1,6))*ta(1,3)
...
*-----------------------------------------------------------------------
      ...
      do 110 n = 1, nee
         ...
         wt = 1.
         if( e .le. tq(13) ) wt = 0.
         if( e .gt. tq(13) .and. e .lt. tq(1) ) wt = qpol(e,tq,sw,13)
         s = e / gpt(3)
         tl = log(s)
         eo = s + 1.
         po = sqrt(s*(s+2.))
   ...
   10       g(i) = 0.
      ...
      do 80 j = jmd(1+mkc), jmd(1+mkc+1) - 1
         ix = lme(3,j)
         z = nxs(2,ix)
         ze = .00232376*(1.-exp(-pf*z*eo/po))*z*(z+1.)
         cr = qpol(tl,exs(jxs(5,ix)),exs(jxs(5,ix)
     &      + nxs(5,ix)),nxs(5,ix))*fme(j)
         if( e .ge. 1. ) goto 40
...
*-----------------------------------------------------------------------
      ...
      do 140 j = jmd(1+mkc), jmd(1+mkc+1) - 1
         ...
         z = nxs(2,ix)
      do 140 n = 1, nm
         tal(l1+n) = log(exs(jxs(5,ix)+n-1))
      do 140 k = 1, km
  140    tal(l2+n+(k-1)*nm) = tal(l2+n+(k-1)*nm)
  ...
  150    tal(l2+n+(k-1)*nm) = log(tal(l2+n+(k-1)*nm))
      ...
      do 160 n = 1, nee
         tl = log(eee(n))
         bl = log(one-(gpt(3)/(eee(n)+gpt(3)))**2)
  160    tal(l3+k+(n-1)*km) = spol(tl,tal(l1+1),tal(l2+1+(k-1)*nm),
     &                        tal(l5+1),tal(l5+m8+1),nm,in,id,m1,m2)-bl
      do 200 n = 1, nee
         eba(1,n+nee*(mkc-1)) = 0.
         in = 1
...
*-----------------------------------------------------------------------
     ...
     &      + dk * ((ee+po**2)*es*p**3-(ee+p**2)*ey*po**3)
         s4 = .5 * p * (f1*pp+log((ee+pp-1.)/dk)*f2) / pp**4
         g = 100. * dk / (ee*zf(1))
         h1 = 1. + (e/eo)**2
         h2 = 2. * third * e / eo
         if( g .gt. .735 ) p1 = (19.83-4.184*log(g+.7))*(h1-h2)
         if( g .le. .735 ) p1 = (20.867-g*(4.409-1.156*g))*h1
     &                        - (20.209-g*(2.625+ .159*g))*h2
         xsecmc = ( s4 + po * (.25*p1-(zf(2)*third+wt*zf(3)
     &                      + log(2.*ee/dk)-.5)
     &                      * (h1-h2))) * ze * e
     &                      / ((1.-exp(-pf*z*e/p))*p*eo)
...
*-----------------------------------------------------------------------
   ...
   30    ch(l) = (n1-l+r2*b)/(r2+b*(n1-l))
      ...
      do 150 k = 1, nwng
         ...
         f2 = eb + e**2
         if( s .gt. 4. ) goto 50
         p = sqrt((s-ek(k)+2.) * (s-ek(k)))
         pp = po * p
         e2 = ek(k)**2
         ef = p2 - ek(k) * eo
         pf = sqrt(p2*(p2-2.*ek(k)*eo+e2))
         if(ef.le.pf) el = log(b**2*(ef+pf)/(.5*e2*(1.-b**2)))/pp
         if(ef.gt.pf) el = log((ef+pf)/(ef-pf))/pp
         ey = log((e+p)/(e-p))
         f = ca*p/po
         f3 = (5.*eb+2.*ee+3.)/(d*p2)
         f4 = (p2-e2)/d
      ...
      do 40 l = 1, mpng
         ...
         q = sqrt(q2)
   40    y(l) = f * (s2*(xi(l)**3*(1.-ch(l)**2))-(f3+f4/q2)*xi(l)
   ...
   60    y(l) = 0.
      ...
      do 70 l = 1, mpng
         q2 = p2+e2-2.*po*ek(k)*ch(l)
         q = sqrt(q2)
   70    y(l) = y(l)+f*(s2*(xi(l)**3*(1.-ch(l)**2))-(f3+f4/q2)*xi(l)
     &        + f5 + el*(f6*xi(l)**3*(1.-ch(l)**2)+f7*xi(l)+f8)
     &        + (log((q+p)/(q-p))/(p*q))*(2.*xi(l)/d
     &        - 3.*ek(k)-ek(k)*(p2-e2)/q2))/xi(l)
...
*-----------------------------------------------------------------------
   ...
   50    if(abs(tbt(iex)-th).le.er*th) return
      ...
      do 80 j = 1, nxs(3,iex)
         ...
         f1 = 1.
         if(tbt(iex).eq.0.) goto 60
         a = sqrt(awn(iex)*xss(l)/tbt(iex))
         if(a.ge.2.) f1 = 1.+.5/a**2
         i = a/.04
         if(a.lt.2.) f1 = (thgf(i)+(a/.04-i)*(thgf(i+1)-thgf(i)))/a
   60    f2 = 1.
         if(th.eq.0.) goto 70
         a = sqrt(awn(iex)*xss(l)/th)
         if(a.ge.2.) f2 = 1.+.5/a**2
         i = a/.04
         if(a.lt.2.) f2 = (thgf(i)+(a/.04-i)*(thgf(i+1)-thgf(i)))/a
...
! T.Sato 2021/08/03, check memory usage
      ...
      if(mbmemory*parz(201).lt.addressmax) then
       xsmemory2=addressmax/mbmemory+0.01 ! 0.01 is safe mergin
       write(ErrCha,'("Insufficient memory for nuclear data. ",
     & "Please set xsmemory more than ",f8.2)') xsmemory2
       ErrID = 'L:6425/R:tapefl/F:ggm02.f'
       call ErrWrite(ErrID,ErrCha)
       stop
      endif
