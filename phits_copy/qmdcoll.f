...
*-----------------------------------------------------------------------
                  ...
                  em1  = p(5,i1)
                  ek1  = p(6,i1)
                  call epotprtR(i1,epotp1)
                  emp1 = sqrt( em1**2 + 2. * em1 * epotp1 )
...
*-----------------------------------------------------------------------
                  ...
                  em2  = p(5,i2)
                  ek2  = p(6,i2)
                  call epotprtR(i2,epotp2)
                  emp2 = sqrt( em2**2 + 2. * em2 * epotp2 )
...
*-----------------------------------------------------------------------
               ...
               if( em1 .lt. 0.94 .and. em2 .lt. 0.94 ) then
                  ...
                  if(srt - cutoff .gt. 0.4286d0) then
                   bcmax  = bcmax0
                   sig    = sig0

                  else ! 2015/1/30 ogawa. cross section modification for peripheral collisions

                   sig  = elcrosnn(iz1, iz2, srt - cutoff)
                   bcmax = sqrt(sig/(10.d0 * pi))
                   if(b .ge. bmax * 0.6) then
                    sig  = 7.5d0 * elcrosnn(iz1, iz2, srt - cutoff)
                    bcmax = sqrt(sig/(10.d0 * pi))
                   endif
...
*-----------------------------------------------------------------------
                  ...
                  p12  = ek1 * ek2 - px1 * px2 - py1 * py2 - pz1 * pz2
...
*-----------------------------------------------------------------------
                  ...
                  t1     = (   p1dr / em1 - b12 / a12 ) * ek1 / em1
                  t2     = ( - p2dr / em2 - b21 / a12 ) * ek2 / em2
...
*-----------------------------------------------------------------------
               ...
               call croswR(ichannel,sig,cutoff,
...
*-----------------------------------------------------------------------
               ...
               call caldis2R(i1,i2)
               if( ntag .eq. 0 ) call pauliR( i1, ntag, phase )
               if( ntag .eq. 0 ) call pauliR( i2, ntag, phase )
...
************************************************************************
*                                                                      *
       function elcrosnn(iz1,iz2,csrt)
*                                                                      *
*                                                                      *
*        Last Revised:     2014  7 27                                  *
*                                                                      *
*        Purpose:                                                      *
*                                                                      *
*              to calculate nucleon-nucleon elastic cross section      *
*                                                                      *
*        Variables:                                                    *
*                                                                      *
*              csrt        : sqrt of s                                 *
*              iz1         : charge of one nucleon                     *
*              iz2         : charge of the other nucleon               *
*              elcrosnn    : nucleon-nucleon cross section (mb)        *
*                                                                      *
************************************************************************
...
*-----------------------------------------------------------------------
      ...
      eng = 0.533d3 * (csrt**2 + 3.79d0 * csrt + 0.07544d0) ! convert sqrt(s) to energy (MeV/u)

            if( iz1 .eq. iz2 ) then

               if( csrt .lt. 0.4286 ) then

                if(b .le. bmax * 0.6d0) then ! Pauli suppressed sigma
                  sigel = 35.0 / ( 1. + csrt * 100.0 )  +  20.0
                else  ! raw sigma
                 if( eng .lt.  190.d0 ) then
                   sigel = exp(0.3506d0 * (5.48d0 - log(eng))**2 +
     &                     3.09d0)
                 else
                   sigel = 22.7d0 -1.7d0 * (csrt - 0.073d0) / (0.4286d0
     &                     - 0.073d0)
                 endif
                endif
     ...
     &                  *   2. / pi + 1.0 ) * 9.65 + 7.0
               ...
               if( csrt .lt. 0.4286 ) then
                ...
                if(b .le. bmax * 0.6d0) then ! Pauli suppressed sigma
                  ...
                  if( eng .lt.  350.d0 ) then
                   sigel = exp(0.34437d0 * (5.99d0 - log(eng))**2 +
     &                     3.55d0)
                  else
                   sigel = 35.d0 - 7.d0 * (csrt - 0.15d0) / (0.4286 -
     &                     0.15)
                  endif
                endif
               else
...
************************************************************************
*                                                                      *
      subroutine croswR(ichannel,sig,cutoff,
     &                 pcm,prcm,srt,beta,gamma,i1,i2)
*                                                                      *
*                                                                      *
*        Last Revised:     1998 11 27                                  *
...
*-----------------------------------------------------------------------
                  ...
                  sigel = elcrosnn(iz1, iz2, csrt) ! 2014/12 ogawa.
...
*-----------------------------------------------------------------------
            ...
            if( x1 .lt. ( sigel + signd ) / sig ) then
               ...
               if( x2 .gt. 0.5 ) then
                  ...
                  call resmasR(1,idd1,idd2,srt,dem1,dem2,gam0,fqrq)
...
*-----------------------------------------------------------------------
            ...
            if( x1 .lt. ( sigel + signd + signr + sigdd ) / sig ) then
                  ...
                  call resmasR(1,idd1,idd1,srt,dem1,dem2,gam0,fqrq)
...
*-----------------------------------------------------------------------
               ...
               if( ( iz1 .ne. iz2 ) .and. ( iz1 + iz2 .ne. 1 ) ) then
                  ...
                  pidn  = pidenoR(2,rmass+pmass,rmass,srt)
...
*-----------------------------------------------------------------------
                  ...
                  pidn  = pidenoR(3,rmass+pmass,rmass,srt)
...
*-----------------------------------------------------------------------
               ...
               if( x1 .lt. ( sigel + sigrn + sigdr ) / sig ) then
                  ...
                  call resmasR(1,idd2,idd1,srt,dem2,dem1,gam0,fqrq)
...
*-----------------------------------------------------------------------
     ...
     &                  ( iz1 .eq. 1 .or. iz2 .eq. 1 ) ) ) then
                     ...
                     pidn   = pidenoR(2,rmass+pmass,dem2,srt)
...
*-----------------------------------------------------------------------
     ...
     &                     - 4.0 * ( dem1 * dem2 )**2 ) / ( 2.0 * srt )
                     ...
                     pidn   = pidenoR(3,rmass+pmass,dem2,srt)
...
*-----------------------------------------------------------------------
                  ...
                  call epotprtR(i1,epotp)

                  e1cm  = sqrt( em1**2 + 2 * em1 * epotp
     &                        + pcm(1)**2
     &                        + pcm(2)**2
     &                        + pcm(3)**2 )
     ...
     &                   / ( gamma + 1 ) + e1cm )
                  ...
                  p(6,i1) = sqrt( p(5,i1)**2 + p(1,i1)**2
     &                          + p(2,i1)**2 + p(3,i1)**2 )
                  rmu1    = em1**2 + 2. * em1 * epotp

                  call epotprtR(i2,epotp)

                  e2cm  = sqrt( em2**2 + 2 * em2 * epotp
     &                        + pcm(1)**2
     &                        + pcm(2)**2
     &                        + pcm(3)**2 )
     ...
     &                   / (gamma + 1.) + e2cm)
                  ...
                  p(6,i2) = sqrt( p(5,i2)**2 + p(1,i2)**2
     &                          + p(2,i2)**2 + p(3,i2)**2 )
                  rmu2    = em2**2 + 2. * em2 * epotp

         do ipart = 1, massal

                  call epotprtR(ipart,epotp)

                  p(4,ipart) = sqrt( p(1,ipart)**2
     &                         + p(2,ipart)**2
     &                         + p(3,ipart)**2
     &                         + 2. * p(5,ipart) * epotp
     &                         + p(5,ipart)**2 )
...
*-----------------------------------------------------------------------
                  ...
                  call resmasR(0,id1,1,em1,dem1,dem2,gam,fqrq)
...
*                    ---------  N* -> Delta + pi ---------
                     ...
                     call resmasR(1,2,1,em1,dem1,dem2,gam0,fqrq)
...
*-----------------------------------------------------------------------
                  ...
                  e1cm   = sqrt( dnmass**2 + 2. * dnmass * epotp
     &                         + pcm(1)**2
     &                         + pcm(2)**2
     &                         + pcm(3)**2 )
     ...
     &                         + p(2,i)**2 + p(3,i)**2 )
                  rmui    = p(5,i)**2 + 2. * p(5,i) * epotp

                  call epotprtR(j,epotp)

                  e2cm   = sqrt( pmass**2 + 2. * pmass * epotp
     &                         + pcm(1)**2
     &                         + pcm(2)**2
     &                         + pcm(3)**2 )
     ...
     &                   / ( gamma + 1 ) - e2cm )
                  ...
                  p(6,j) = sqrt( p(5,j)**2 + p(1,j)**2
     &                         + p(2,j)**2 + p(3,j)**2 )
                  rmuj    = p(5,j)**2 + 2. * p(5,j) * epotp
...
*-----------------------------------------------------------------------
     ...
     &                         - py2**2 - pz2**2 )
                  ...
                  s   = (ek1+ek2)**2 - (px1+px2)**2 - (py1+py2)**2
     &                                            - (pz1+pz2)**2
                  srt = sqrt(s)
...
*-----------------------------------------------------------------------
              ...
              if( inds(i2) .eq. 1 ) then
                  ...
                  call resmasR(0,2,1,srt,dem1,dem2,gamdl,fqrq)
     ...
     &               / ( 4.0 * ( srt - dmass )**2 / gamdl**2 + 1.0 )
              ...
              if( izzz .eq. 1 ) then
                  ...
                  call resmasR(0,3,1,srt,dem1,dem2,gamrs,fqrq)
...
*-----------------------------------------------------------------------
         ...
         do 1200 ipart = 1, massal

            if( ipart.eq.i1 ) goto 1200
...
*-----------------------------------------------------------------------
                  ...
                  srt = eini - efin + gamma * ( p(4,i2)
     &                              - beta(1) * p(1,i2)
     &                              - beta(2) * p(2,i2)
     &                              - beta(3) * p(3,i2) )

                  call epotprtR(i2,epotp)
                  rrms = sqrt( srt**2 + epotp**2 ) - epotp

               if( rrms .gt. rmass + pmass ) then

                  p(5,i2) = rrms
                  p(6,i2) = sqrt( p(5,i2)**2 + pps )
                  p(4,i2) = sqrt( pps + p(5,i2)**2
     &                                + 2 * p(5,i2) * epotp )
...
************************************************************************
*                                                                      *
      subroutine resmasR(ipw,isd1,isd2,srt,dem1,dem2,gam0,fqrq)
*                                                                      *
*                                                                      *
*        Last Revised:     1998 11 27                                  *
*                                                                      *
...
************************************************************************
*                                                                      *
      function pidenoR(isd1,dem1,dem2,srt)
*                                                                      *
*                                                                      *
*        Last Revised:     1998 11 27                                  *
*                                                                      *
...
*-----------------------------------------------------------------------
     ...
     &            p02 .le. 0.0 ) return
              ...
              sooR = fterm * alfa * ( pr / p0 )**beta
              den  = ( sstar - m02 )**2 + m02 * gam2
              sooR = sooR * m02 * gam2 * ( q / q0 )**3. / den
