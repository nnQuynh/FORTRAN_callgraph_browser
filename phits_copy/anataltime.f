!***********************************************************************
!                                                                      *
      subroutine anatal_calc_anova_timereg(m,ntf,
     &                             np,nt,ne,nr,
...
!-----------------------------------------------------------------------
      ...
      double precision  trRES   (np,ne,nt,nr,2)
      integer           nfile
      double precision  tranatal(np,ne,nt,nr,2*nfile)
       ...
       do ie = 1, ne
        do it = 1, nt
         do ip = 1, np
             ...
             if( trRES(ip,ie,it,ir,1) .gt. 0.d0 ) then
              call calc_stdev(m,Xa,sigx,
     &                        trRES(ip,ie,it,ir,1),
     &                        trRES(ip,ie,it,ir,2),
     &                        1.0d+0)
              trRES(ip,ie,it,ir,1) = Xa
              trRES(ip,ie,it,ir,2) = sigx
             end if

              ! X_bar = Xj_bar
              tranatal(ip,ie,it,ir,iat(1,ntf)) =
     &           trRES(ip,ie,it,ir,1)

              ! (sig_xj)**2
              tranatal(ip,ie,it,ir,iat(2,ntf)) =
     &        (  trRES(ip,ie,it,ir,2)
     &         * trRES(ip,ie,it,ir,1) )**2

              ! sig_x
              tranatal(ip,ie,it,ir,iat(2,ntf)) = dsqrt(
     &        tranatal(ip,ie,it,ir,iat(2,ntf)))

              ! Sigma(xi wi)**2 = (sig_X**2 N(N-1)+N X_bar**2)(W/N)**2
              tranatal(ip,ie,it,ir,iat(2,ntf)) =
     &       (tranatal(ip,ie,it,ir,iat(2,ntf))**2 *
     &        resc3(m) * (resc3(m)-1.0d0) +
     &        resc3(m) *
     &        tranatal(ip,ie,it,ir,iat(1,ntf))**2) *
     &        (resc2(m)/resc3(m))**2

              ! Sigma xi wi = X_bar W
              tranatal(ip,ie,it,ir,iat(1,ntf)) =
     &        tranatal(ip,ie,it,ir,iat(1,ntf)) * resc2(m)

            if( tranatal(ip,ie,it,ir,iat(1,ntf)) > cmax )
     &            cmax = tranatal(ip,ie,it,ir,iat(1,ntf))
            if( tranatal(ip,ie,it,ir,iat(1,ntf)) < cmin )
     &            cmin = tranatal(ip,ie,it,ir,iat(1,ntf))
...
!-----------------------------------------------------------------------
      return
      end subroutine anatal_calc_anova_timereg



!***********************************************************************
!                                                                      *
      subroutine anatal_calc_anova_timerz(m,ntf,
     &                             np,nt,ne,nr,nz,
     &                             sumfactor,weightRate,
     &                             trRES,tranatal,nfile,
     &                             resc2SUMTAL,resc3SUMTAL,
...
!-----------------------------------------------------------------------
      return
      end subroutine anatal_calc_anova_timerz



!***********************************************************************
!                                                                      *
      subroutine anatal_calc_anova_timexyz(m,ntf,
     &                             np,nt,ne,nx,ny,nz,
     &                             sumfactor,weightRate,
     &                             trRES,tranatal,nfile,
     &                             resc2SUMTAL,resc3SUMTAL,
...
!-----------------------------------------------------------------------
      return
      end subroutine anatal_calc_anova_timexyz



************************************************************************
*                                                                      *
      subroutine anatal_ptimereg(m,np,nr,mr,ne,nt,kr,eb,tb,tr,
     &                    nfile,weightRate,
     &                    nvl,ivl,rvl,
     &                    nx,ny,nz,xm,ym,zm,igsh,idasa,manatally)
*                                                                      *
*       output the time tally in region mesh                           *
...
!      dimension   lr(nr)
!      dimension   tw(nt)
      dimension   tr(np,ne,nt,nr,2*nfile)
      dimension   ivl(nvl)
      dimension   rvl(nvl)
...
!      dimension   dlr(nr+1) !  integer lr(nr) ---> double dlr(nr)
      real(8),allocatable :: ew(:),vl(:),tw(:),val(:),rdata(:,:),dlr(:)
...
*-----------------------------------------------------------------------
      ...
      character hsunit(4)*28

      data hsunit / '[1/source]              ',
     &              '[1/nsec/source]         ',
     &              '[1/nsec/cm^3/source]    ',
     &              '[1/nsec/cm^3/MeV/source]'/
...
!-----------------------------------------------------------------------
      ...
      allocate (ew(ne),vl(nr),tw(nt),val(nr),rdata(2,nfile),dlr(nr+1))
      allocate (lr(nr))
...
*-----------------------------------------------------------------------
      ...
      if ( iMeVperu.eq. 1 ) then
         hsunit(4) = '[1/nsec/cm^3/(MeV/n)/source]'
      end if
...
*-----------------------------------------------------------------------
         ...
         if( igsh .eq. 0 ) then
            ...
            do ntf = 1, nfile
               ...
               if( tr(ip,ie,it,ir,iat(1,ntf)) .gt. 0.d0 ) then

                  cc = rtfac(m) / ew(ie) / tw(it)

                  if( itunt(m) .ge. 3 ) cc = cc / vl(ir)

                  call calc_stdev(m,Xa,sigx,
     &                            tr(ip,ie,it,ir,iat(1,ntf)),
     &                            tr(ip,ie,it,ir,iat(2,ntf)),
     &                            cc)

                  tr(ip,ie,it,ir,iat(1,ntf)) = Xa
                  tr(ip,ie,it,ir,iat(2,ntf)) = sigx
                  if( sigx .gt. stdm ) stdm = sigx

                  if( tr(ip,ie,it,ir,iat(1,ntf)) .gt. cmax )
     &                             cmax = tr(ip,ie,it,ir,iat(1,ntf))

                  if( tr(ip,ie,it,ir,iat(1,ntf)) .lt. cmin )
     &                             cmin = tr(ip,ie,it,ir,iat(1,ntf))

               else

                  isdz = 1
                  tr(ip,ie,it,ir,iat(2,ntf)) = 0.0

               end if

! sumover
               if( itunt(m) .ge. 3 ) then
                 call ptimereg_sumover_stdev_ntf(0,m,ntf,
     &               ip,ie,it,ir,
     &               rtfac(m),ew(ie),tw(it),vl(ir),
     &               ew_sum,tw_sum,vl_sum)
               else
                 call ptimereg_sumover_stdev_ntf(0,m,ntf,
     &               ip,ie,it,ir,
     &               rtfac(m),ew(ie),tw(it),1.0d0,
     &               ew_sum,tw_sum,1.0d0)
               endif
...
!$OMP PARALLEL
!$OMP& private(ipomp,ir,ie,it,ip,ntf,isdz,rdata,answer,rerr)
...
!$OMP DO SCHEDULE(dynamic) reduction(max:cmax) reduction(min:cmin)
            ...
            if( manatally .eq. 0 ) then ! T.Sato 2020/10/22
               ...
               do ntf = 1, nfile
                  rdata(1,ntf) = tr(ip,ie,it,ir,iat(1,ntf))
                  rdata(2,ntf) = tr(ip,ie,it,ir,iat(2,ntf))
               end do
...
!sumover
            do iax = 1,itaxn(m)
              ...
              if(ido_ana == 1) then
                 ...
                 call get_timereg_tr_sum_data(m,iax,nfile,
     &                 ip,ie,it,ir,rdata)
...
*-----------------------------------------------------------------------
            ...
            call prestart(m,iot) !OBINATA(2012.9.18)
...
*-----------------------------------------------------------------------
      ...
      deallocate (ew,vl,tw,val,rdata,dlr)
      deallocate (lr)

      return
      end subroutine anatal_ptimereg



************************************************************************
*                                                                      *
      subroutine anatal_ptimerz(m,np,nr,nz,ne,nt,rm,zm,eb,tb,tr,
     &                          nfile,weightRate,idasa,manatally)
*                                                                      *
*       output r-z scoring mesh time tally                             *
*       created by T.Miura on 2021/09/13                               *
*                                                                      *
************************************************************************
      use sangelmod, only: itsans
...
*-----------------------------------------------------------------------
      ...
      return
      end subroutine anatal_ptimerz



************************************************************************
*                                                                      *
      subroutine anatal_ptimexyz(m,nl,lt,np,nx,ny,nz,ne,nt,
     &                                      xm,ym,zm,eb,tb,tr,
     &                    nfile,weightRate,
     &                    igsh,idasa,manatally)
*                                                                      *
*       output xyz scoring mesh time tally                             *
*       created by T.Miura on 2021/09/13                               *
*                                                                      *
************************************************************************
      use sangelmod, only: itsans
...
*-----------------------------------------------------------------------
      ...
      return
      end subroutine anatal_ptimexyz
...
!***********************************************************************
!                                                                      *
      subroutine get_timereg_tr_sum_data(m,iax,nfile,
     &           ip,ie,it,ir,rdata)
!                                                                      *
!***********************************************************************
...
! manatalm_sum(m,iax,nfile(m)) :  one sumover length
      ...
      integer :: m,iax,nfile,ip,ie,it,ir
...
!$       end if
        ...
        call get_timereg_tr_sum_data_sub(p_sum,
     &     itpan_sum(m,iax),itenm_sum(m,iax),ittnm_sum(m,iax),
     &     itrgn_sum(m,iax),
     &     ip,ie,it,ir,rdata(1,ntf))
...
!***********************************************************************
!                                                                      *
      subroutine get_timereg_tr_sum_data_sub(tr_sum,
     &          np,ne,nt,nr,ip,ie,it,ir,rdata)
!                                                                      *
!***********************************************************************

      implicit double precision (a-h,o-z)

      integer :: m,iax,np,ne,nt,nr,ip,ie,it,ir
      real(8) :: rdata(2)

      real(8) :: tr_sum(np,ne,nt,nr,2)
 
      do i=1,2
        rdata(i)= tr_sum(ip,ie,it,ir,i)
      enddo
...
!***********************************************************************
!                                                                      *
      subroutine get_timerz_tr_sum_data(m,iax,nfile,
     &           ip,ie,it,ir,iz,rdata)
!                                                                      *
!***********************************************************************
...
!$       end if
        ...
        call get_timerz_tr_sum_data_sub(p_sum,
     &     itpan_sum(m,iax),itenm_sum(m,iax),ittnm_sum(m,iax),
...
!***********************************************************************
!                                                                      *
      subroutine get_timerz_tr_sum_data_sub(tr_sum,
...
!***********************************************************************
      ...
      real(8) :: tr_sum(np,ne,nt,nr,nz,2)
...
!***********************************************************************
!                                                                      *
      subroutine get_timexyz_tr_sum_data(m,iax,nfile,
     &           ip,ie,it,ix,iy,iz,rdata)
!                                                                      *
!***********************************************************************
...
!$       end if
        ...
        call get_timexyz_tr_sum_data_sub(p_sum,
     &     itpan_sum(m,iax),itenm_sum(m,iax),ittnm_sum(m,iax),
...
!***********************************************************************
!                                                                      *
      subroutine get_timexyz_tr_sum_data_sub(tr_sum,
...
!***********************************************************************
      ...
      integer :: np,ne,nt,nx,ny,nz,nm,ip,ie,it,ix,iy,iz
      real(8) :: rdata(2)

      real(8) :: tr_sum(np,ne,nt,nx,ny,nz,2)


      do i=1,2
        rdata(i)= tr_sum(ip,ie,it,ix,iy,iz,i)
      enddo
