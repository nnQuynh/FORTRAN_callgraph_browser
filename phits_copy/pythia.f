...
C...differential cross-sections to be used for weighting.
      ...
      subroutine pyinit(frame,kfp,kft,win,icon)
...
C...Interface to PDFLIB.
      ...
      save /w50512/
!$OMP THREADPRIVATE(/w50512/)
      double precision value(20),qcdl4,qcdl5
      character*20 parm(20)
      data value/20*0d0/,parm/20*' '/
      save value,parm !FURUTA
!$OMP THREADPRIVATE(value,parm)
C...Data:Lambda and n_f values for parton distributions; months.
     ...
     $ 3*0.2d0/,nfin/20*4/
      ...
      mstp(122)=0 ! no need to output initialization information, T.Sato 2018/06/27
...
C...Identify beam and target particles and frame of process.
      chfram=frame//' '
      call pyinbm(chfram,kfp,kft,win)
...
C...Showering of initial state partons (optional).
          ...
          if(ncascerr.le.-1) return ! T.Sato 2018/06/20
          parj(81)=alamsv
          if(mint(51).eq.1) goto 100
...
C...Identifies the two incoming particles and the choice of frame.
       ...
       subroutine pyinbm(chfram,kfp,kft,win)
...
C...Store initial energy. Default frame.
      ...
      pm(2)=pymass(mint(12))
      vint(4)=pm(2)
      call pyname(mint(11),chbeam)
      call pyname(mint(12),chtarg)
      if(mint(11).eq.0) write(mstu(11),5000) chbeam
      if(mint(12).eq.0) write(mstu(11),5100) chtarg
      if(mint(11).eq.0.or.mint(12).eq.0) call parastop( 116 )
      chcom(1)(1:8)=chfram
      chcom(2)=chbeam
      chcom(3)=chtarg
...
! T.Sato, to avoid too much error message
        call parastop( 116 )
      endif
...
!$OMP THREADPRIVATE(imin,imax,wti,wts)
...
!$OMP THREADPRIVATE(ncp,nsubcp,msubcp,coefcp,ngencp,xseccp,intcp,recp)
  ...
  200   continue
     ...
     &    vint(44)**2
        ...
        elseif(isub.eq.54) then
          if((kchg(jamcomp(kflf),1)/2d0)**2.lt.pyr(0)) goto 180
        elseif(isub.eq.58) then
          if((kchg(jamcomp(kflf),1)/3d0)**2.lt.pyr(0)) goto 180
        endif
      endif
...
!$OMP THREADPRIVATE(is)
      the2t=0.0 ! initialization T.Sato 2016/03/11
...
C...1) Three-body decays of SUSY particles (plus special case top).
        if(kfl3(jt).ne.0) then
c...jam
          call pyerrm(30,'(pyresd:)susy remove')
          do 200 i=n+1,n+3
            do 190 j=1,5
              k(i,j)=0
              v(i,j)=0d0
...
!$OMP THREADPRIVATE(xt2,xt2fac,xc2,xts,irbin,rbin,nmul,sigm)
c...JAM:
      common/jampyda1/qmult(10),imult(2,10)
!$OMP THREADPRIVATE(/jampyda1/)
...
C...Relative transverse momentum when two remnants.
  ...
  370     loop=loop+1
          ...
          if(iii.eq.0) then
            ...
            pt=0.5d0*sqrt(-log(max(1d-10,pyr(0))))
            pt=paru(2)*pyr(0)
            p(i-1,1)=pt*cos(phix)
            p(i-1,2)=pt*sin(phix)
...
!$OMP THREADPRIVATE(mofsv,widwsv,wid2sv)
      data mofsv/6*0/,widwsv/6*0d0/,wid2sv/6*0d0/

      wid2=0.0 ! T.Sato 2016/03/10
...
C...in case it is more complicated than just a quark or a diquark.
      ...
      subroutine pyspli(kf0,kflin,kflch,kflsp)
...
C...Preliminaries. Parton composition.
      ...
      if(kfa.eq.22.and.mint(109).eq.2) then
        ...
        kfma=kfl(2)*10+kfl(3)
        if(kfma.eq.11.and.pyr(0).gt.0.5d0)then
          kfl(2)=2
          kfl(3)=2
        else if(kfma.eq.22.and.pyr(0).gt.0.5)then
          kfl(2)=1
          kfl(3)=1
        endif
      endif
...
C...Subdivide baryon.
  ...
  110     continue
      ...
      if(jamcomp(kflsp).eq.0)then
        ...
        call jamerrm(30,2,'(pyspli:)kflsp=0')
      endif
...
C...You should not copy these lines to your own routine.
      write(mstu(11),5000)
      if(pyr(0).lt.10d0) call parastop( 116 )
