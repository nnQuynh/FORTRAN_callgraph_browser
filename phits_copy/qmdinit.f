...
*                                                                      *
************************************************************************
      use QMD_COOD2_MOD, only : rr2,rbij,pp2,rha, rhe, rhc, nqmdm
...
*------------------------------------------------------------
*           Light cluster judgement condition [100-119]
*------------------------------------------------------------

                  iprnt  = mstq1(100)   ! D =0 ( no output)

                  fdr    = 1.d0
                  fdp    = 1.d0

                  hhh   = qmdscm_h0       ! D=0.25
                  ddd   = qmdscm_d        ! D=1.75
                  rcls00= qmdscm_rcls     ! D=-1.124
...
*-----------------------------------------------------------------------
                  ...
                  if( n1 .gt. 2 ) radpr = r00 * dble( n1 )**(1./3.)
                  if( n2 .gt. 2 ) radta = r00 * dble( n2 )**(1./3.)
...
*-----------------------------------------------------------------------
               ...
               rzpr  = -rmax0 * dble( n2 ) / dble( n2 + n1 )
               rxpr  =  0.0

               rzta  =  rmax0 * dble( n1 ) / dble( n2 + n1 )
               rxta  =  0.0
...
*-----------------------------------------------------------------------
            ...
            if( insys .eq. 0 ) then
               ...
               zeroz = rmax0 * dble( n1 ) / dble( n2 + n1 )
     &               + 16.0 - radpr - rmax0
...
*-----------------------------------------------------------------------
               ...
               if( ibch .eq. 0 ) then
                  ...
                  do i = 1, ibin
                     ...
                     ibnum(i)   = jprun
                     bval(i)    = bmin + bdef / 2.0 + dble(i-1) * bdef
