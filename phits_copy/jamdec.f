...
c...Resonance decay.
          ...
          iang=0
          iswang=mstc(61)
          if(k(1,ip).eq.2.and.abs(k(9,ip)).eq.3) then
            iang=1
            if(kq(1,ip).eq.999999) then
              if(emd.gt.1.1d0)  iang=2
              if(id0.eq.id_delt.and.emd.gt.1.1d0)iswang=5
            endif
          endif
...
c...B-B~ mixing: flip sign of meson appropriately.
      ...
      if((kfa.eq.511.or.kfa.eq.531).and.mstj(26).ge.1) then
        ...
        vip5=-pmas(kc,4)*log(pyr(0))
        if(sin(0.5d0*xbbmix*vip5/pmas(kc,4))**2.gt.pyr(0)) mmix=1
...
c...Sum branching ratios of allowed decay channels.
  ...
  235  if(brsu.le.0.0d0)then
        write(mstu(11),*)'================================'
        call pyerrm(2,'(jamrdec:) All decay channels closed')
...
c...[
        ...
        pdd3=pd(3)
        pdd4=pd(4)
        call jamrobo(0d0,0d0,-vbex,-vbey,-vbez,vgam,pdd1,pdd2,pdd3,pdd4)
        phi=pyangl(pdd1,pdd2)
        the=pyangl(pdd3,sqrt(pdd1**2+pdd2**2))

        if(iswang1.eq.5) then
         itry=0
...
c...Find virtual gamma mass in Dalitz decay.
  420 if(nd.eq.2) then
      elseif(mmat.eq.2) then
        ...
        pmrho2=pmas(131,1)**2
        pgrho2=pmas(131,2)**2
  430   pmst=pmes*(p(nsav,5)**2/pmes)**pyr(0)
...
c...Matrix elements for omega and phi decays.
     ...
     &  +2.d0*four(n+1,n+2)*four(n+1,n+3)*four(n+2,n+3)
        if(max(wt*wtcor(9)/p(nsav,5)**6,0.001d0).lt.pyr(0)) goto 420
...
c...Matrix elements for pi0 or eta Dalitz decay to gamma e+ e-.
     ...
     &  pmes*(four12*four13+four12**2+four13**2)
        if(wt.lt.pyr(0)*0.25d0*pmst*(p(nsav,5)**2-pmst)**2) goto 490
...
c...S0 -> gamma + V1 -> gamma + S2 + S3, of form sin**2(theta02).
     ...
     &  pms1*four02**2-pms0*four12**2-pms2*four10**2+pms1*pms0*pms2)
        ...
        if(hnum.lt.pyr(0)*hden) goto 490
...
c.....Lepton, etc.
        ...
        if(emres.lt.em1+em2+parj(64)) goto 10
...
c...Mom. dep. width
          ...
          ldec=mdme(idl,3)
          if(emcm.gt.em1+em2+parj(64)) then
...
c...B-B~ mixing: flip sign of meson appropriately.
                ...
                if((kfa.eq.511.or.kfa.eq.531).and.mstj(26).ge.1) then
                  ...
                  if(kfa.eq.531) xbbmix=parj(77)
                  vip5=-pmas(kc,4)*log(pyr(0))
                  if(sin(0.5d0*xbbmix*vip5/pmas(kc,4))**2.gt.pyr(0))
     $                mmix=1
                  if(mmix.eq.1) kfs=-kfs
                endif
...
c...Decay unstable particles at the end of simulation.
      ...
      if(mstc(42).eq.0) then
        mdcy(jamcomp(111),1)=1  ! pi0
        mdcy(jamcomp(311),1)=1  ! k0
        mdcy(jamcomp(-311),1)=1 ! ak0
        mdcy(jamcomp(411),1)=1    ! D+
        mdcy(jamcomp(-411),1)=1   ! D-
        mdcy(jamcomp(421),1)=1    ! D0
        mdcy(jamcomp(-421),1)=1   ! aD0
        mdcy(jamcomp(221),1)=1    ! eta
        mdcy(jamcomp(331),1)=1    ! eta'
        mdcy(jamcomp(441),1)=1    ! eta_c
        mdcy(jamcomp(310),1)=1
        mdcy(jamcomp(431),1)=1
        mdcy(jamcomp(-431),1)=1
        mdcy(jamcomp(511),1)=1
        mdcy(jamcomp(-511),1)=1
        mdcy(jamcomp(521),1)=1
        mdcy(jamcomp(-521),1)=1
        mdcy(jamcomp(531),1)=1
        mdcy(jamcomp(-531),1)=1
...
2200  continue
      itry=itry+1
      if(itry.ge.100) then
        call jamerrm(3,0,'(jamfdec:) Infinit loop???')
        call jamlist(1)
        return
      endif
...
c...Avoid numerical error for long lived particles.
        ...
        do j=1,3
         r(j,ip)=r(j,ip)+dect*p(j,ip)/p(4+mstc(190)*2,ip)
        end do
        r(4,ip)=v(5,ip)
...
c...Forbit unstable particle decay.
          ...
          if(k1.ge.2.and.k1.le.10) then
             goto 2200
          end if
          if(mdcy(kc,1).eq.1) goto 2200
 300    end do
