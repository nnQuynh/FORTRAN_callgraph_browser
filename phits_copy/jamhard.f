...
c...PYHIA common block.
      ...
      save  /pyjets/,/pysubs/,/pypars/,/pyint1/,/pyint2/,/pyint5/
...
!$OMP THREADPRIVATE(/jamxhit/)

      save mstp122
      save kfcol1,kfcol2
!$OMP THREADPRIVATE(mstp122,kfcol1,kfcol2)
c...Local values.
      real*8 jamdtim
     ...
     $,ipa(2,2)
      ...
      common /errorcom/ncascerr ! T.Sato 2018/06/27
!$OMP THREADPRIVATE(/errorcom/)


! T.Sato 2018/06/26, kf1col1 & kfcol2 are kf codes used in the previous initialization.
! Therefore, they should be 0 at the very first time called this routine (i.e. PYTHIA should be always initialized)
      data kfcol1, kfcol2/0,0/
      data mstp122/1/
...
c...Must have enough energy to produce jets.
      ...
      ecut1=hipr1(1)+hipr1(8)+pymass(kfq(1,1))+pymass(kfq(1,2))
      ecut2=hipr1(1)+hipr1(8)+pymass(kfq(2,1))+pymass(kfq(2,2))
...
c...Initialize Pythia.
	...
	if(mstp122.ge.5) mstp(122)=0
        call pyinit('five',kfcol3,kfcol4,0.0d0,icon)
        if(icon.ne.0) then
           kcol3=0 !cKN
           kcol4=0 !cKN
           write(check(1),
     ...
     $ icon,srtc,kcol3,kcol4
           call jamerrm(1,0,'iflg after pyinit')
          jflg=-2
          return
        endif
	kfcol1=kfcol3
...
c...Find index of JAM array.
      ...
      nv=nv+1
      if(nv.gt.nnn)
     $   call jamerrm(30,0,'particle too large (jamhrd)nnn->change')
      indx=nv
      ncup=ncup+1
      ind(ncup)=indx
...
c....XXX BB and BB~ collision only
	...
	k11=nv+lc+jpq(1,1)-jpq(1,2)-1+jpq(2,1)-jpq(2,2)-1
      ...
      if(k11.gt.nnn)
     $  call jamerrm(30,0,'particle too large (jamhrd)nnn')
...
cxxxxxxxxxxxxxxxxxxxxxxxxxxx
      ...
      nmeson=nmeson+1
      nv=nv+1
      if(nv.gt.nnn) call jamerrm(30,0,'(jamhrd:chh)particle too large')
      indx=nv
      do j=1,5
      p(j,indx)=pc(j)
      end do
...
!$OMP THREADPRIVATE(/partdis3/)
      save /partdis1/,/partdis2/
!$OMP THREADPRIVATE(/partdis1/)
!$OMP THREADPRIVATE(/partdis2/)
...
100   continue
     ...
     $ i,(k(i,j),j=1,3),p(i,5)
        ...
        return ! T.Sato 2018/06/23, avoid infinite loop
...
c....Spacelike partion branching
	...
	if(im.eq.3.or.im.eq.4) then  ! initiator
     ...
     $               i,iq,k(i,1),k(i,2),k(i,3),p(i,5),jt
             ...
             return ! T.Sato 2018/06/26, avoid infinite loop
        endif
        v(i,5)=1.d+35
        v(i,4)=v(iq,4)
        do j=1,3
