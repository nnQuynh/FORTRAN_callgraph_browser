...
c     PROGRAM ATIMA
c=======================================================================
c     This routine is combined by two main routines,
c     atimaspline.f and testspline.f in the ATIMA system,
c
c     atimaspline.f: makes a spline data and writes into a file (which
c                    is named "tmpfile" here)
c     testspline.f:  reads the temporaly file and gives results
c
c     2004.08.13 by Hiroshi Iwase, GSI
c-----------------------------------------------------------------------
c
c     Informations written below are from ATIMA website.
c     (http://www-linux.gsi.de/~weick/atima/)
c
c-----------------------------------------------------------------------
c
c     ATIMA was written by Hans Geissel, Christoph Scheidenberger, Peter
c     Malzacher and Joerg Kunzendorf (and maintained by Helmut Weick in
c     GSI, Germany).
c
c     Above 30 MeV/u the stopping power is obtained from the theory by
c     Lindhard and Soerensen (LS) [1] including the following
c     corrections: the shell corrections [2], a Barkas term [3, 4] and
c     the Fermi-density effect [5]. The projectiles are treated as
c     point-like particles of a mean charge according to
c     ref. [6]. Below 10 MeV/u an older version of Ziegler's SRIM [7]
c     is used. In the intermediate energy range we interpolate between
c     the two. The LS theory differs substantially for ions with high Z
c     from the Bethe formula [8] it also considers the nuclear size
c     effect for very relativistic ions.
c     Energy-loss straggling comes also from the LS-theory above 30 MeV
c     /u. Below 10 MeV/u the theory of Firsov [9] and Hvelplund [10] is
c     used.
c
c     Before the LS theory was established the significant differences
c     in stopping-power and energy-loss straggling were observed [11,12]
c     with the FRS at GSI. A more detailed discussion of the results one
c     can find in refs. [13, 14]. Finally, a review article [15]
c     combines all previous obtained results and even new data.
c     The effects of charge-exchange straggling [16, 17] and more
c     detailed charge dependence of dE/dx [18] were observed but are not
c     yet included in ATIMA.
c
c     References
c     [1]    J. Lindhard, A.H. Soerensen, Phys. Rev. A53 (1996) 2443.
c     [2]    W.H. Barkas, M.J. Berger, NASA Report SP-3013 (1964).
c     [3]    J.D. Jackson, R.L. McCarthy, Phys. Rev. B6 (1972) 4131.
c     [4]    J. Lindhard, Nucl. Instr. and Meth. 132 (1976) 1.
c     [5]    R.M. Sternheimer and R.F. Peierls, Phys. Rev. B3 (1971)
c            3681.
c     [6]    T.E. Pierce, M. Blann, Phys. Rev. 173 (1968) 390.
c     [7]    J.F. Ziegler, J.B. Biersack, U. Littmark, The Stopping and
c            Range of Ions in Solids, Vol.1, Pergamon Press (1985).
c     [8]    H. Bethe, Z. Phys. 76 (1932) 293.
c     [9]    O.B. Firsov, Sov. Phys. JETP 5 (1957) 1192.
c     [10]   P. Hvelplund, Dan. Mat. Fys. Medd. Dan. Vid. Selsk. 38 no.4
c            (1971).
c     [11]    C. Scheidenberger et al., Phys. Rev. Lett. 73 (1994) 50.
c     [12]    C. Scheidenberger et al., Phys. Rev. Lett. 77 (1996) 3987.
c     [13]    C. Scheidenberger, H. Geissel, Nucl. Instr. and Meth. B136
c             (1998) 114.
c     [14]    H. Geissel, C. Scheidenberger, Nucl. Instr. and Meth. B135
c             (1998) 25.
c     [15]    H. Geissel et al., Nucl. Instr. and Meth.  B195 (2002) 3.
c     [16]    H. Weick et al., Phys. Rev. Lett. 85 (2000) 2725.
c     [17]    H. Weick, A.H. Soerensen et al., Nucl. Instr. and Meth. B
c             193 (2002) 1.
c     [18]    H. Weick et al., Nucl. Instr. and Meth.  B 164/165 (2000)
c             168.
c
c=======================================================================
...
C              CORRESPONDING TO THE MESH XI(*),...,XI(N+1)
...
C
C...         FOR XI(*),XI(2), THE DIVIDED DIFFERENCE IS A SPECIAL CASE
C
             DO 60 L=1,NR
...
C-----------------------------------------------------------------------
C       HIGHER ORDER CORRECTIONS FOR ZP>2 MODIF. BERKELEY TRERMS
C       BETHE + BLOCH1 + BLOCH2 + BARKAS + MOTT + DENSITY +
C-----------------------------------------------------------------------
C     Input:
C        ZP     = Nuclear Charge of Projectile
C        ZT     = Nuclear Charge of Target
C        MP     = Mass of Projectile
C        MT     = Mass of Target
...
c   dichteeff dichteeffekt nach Sternheimer 1984
c.scheidenberger
...
C     LOW ENERGY BLOCH CORRECTION ======> INCLUDED

      V1 = ETA/(ALPHA*DSQRT(DBLE(ZT)))
      IF (V1 .GE. 4.D0) THEN
        V2FV = 0.45D0/DSQRT(V1)
      ELSE IF ((V1 .GE. 1.D0).AND.(V1 .LT. 4.D0)) THEN
C       VALUES FROM THE JACKSON MC CARTHY FUNCTION
C       PHYS. REV. B 6 4131 P4136
        DO 2,I = 1,4
...
************************************************************************
C
C           LIEST AUS FSPACE UND ISPACE DIE B-DARSTELLUNG DER
C           I-TE LOESUNGSKOMPONENTE NACH 'T,BCOEF,NB,KPM'
C           (ZUR WEITERVERWENDUNG MIT DEN B-SPLINE PROGRAMMEN
C            VON DE BOOR)
C
      IMPLICIT REAL*8 (A-H,O-Z)
...
c-----------------------------------------------------------------------
c     hier muss das Zeitintegral hin!
...
* Fullsource for module DB2VAL from package CMLIB.
* Retrieved from GRANTA on Thu Jul 18 07:52:46 1996.
* ======================================================================
...
C********************************************************************/
C*                                                                  */
C*         BOHR RELATIVISTIC ENERGY LOSS STRAGGLING                 */
C*                                                                  */
C*               QE IN MEV**2/(MG/CM**2)                            */
C*                                                                  */
C********************************************************************/
C STATUS UNGETESTET 19.5.94
C********************************************************************/
...
C    better use only Z not Zeff, H. Weick
      ZPEFF = ZETA * DBLE(ZP)
...
c     * X_Lindhard,Sorensen,
c     unit is MEV**2/(MG/CM**2)
      DEBOHR = BOHR * ( x_lisoe(ZP, MP, BETA) * GSQ + titeica )
c     write(*,*) beta, bohr, x_lisoe(zp,mp,beta), titeica, debohr
      RETURN
      END
...
C-----------------------------------------------------------------------
C Inputs:
C     ZP = Atomic Number of the Projectile
C     ZT = Atomic Number of the Target
C     MP = Mass Number of the Projectile
C     MT = Mass Number of the Target
C-----------------------------------------------------------------------
      INTEGER*4 ZP,ZT,GAS
...
C
C       Originai routine is "subroutine DEDXspar" in range.f
C       modified by S.Abe on 2017/09/13
*                                                                      *
************************************************************************
...
C
C-----------------------------------------------------------------------
C       CALCULATE UNIVERSAL NUCLEAR STOPPING POWERS
C       NONRELATIVISTIC
C-----------------------------------------------------------------------
C       Input:
C          ZT     =  Nuclear Charge of Projetile
C          ZP     =  Nuclear Charge of Target
C          ENERGY =  Energy per Unit of the Projetile
C          MP     =  Mass of Projectil
C          MT     =  Mass of Target
C-----------------------------------------------------------------------
C 10.05.94  Status getestet
C-----------------------------------------------------------------------
C
      implicit real*8 (a-h,o-z)
      include 'atimacnt.inc'
...
C     ENERGY IN KEV,DEDXELA IN MEV/(MG/CM**2)
C     EPSIL = Reduced Energy Lss-Unit
C
C     ENERGY IN KEV
...
C
      ...
      END IF
C
C     NOW CONVERT FROM REDUCED UNITS TO EV-CM**2/E15
      SN = SN*DBLE(ZP)*DBLE(ZT)*MP*8.462D0/
     &      ((MP+MT)*(DBLE(ZP)**.23D0+DBLE(ZT)**.23D0))
C     CONVERT TO MEV-CMT/MG
      SN = SN * .1*N_AVO / MT
      DEDXELA = SN
      RETURN
      END
...
C----  CALCULATES DE/DX FOR MOLECULES
...
C       write(6,'(2(1pe12.5,1x))') energy,dedxver
...
************************************************************************
      ...
      include 'atimacnt.inc'
C**********************************************************************
C     PROCEDURE TO CALCULATE ENERGYLOSS STRAGGLING WITH FIRSOV FORM
C**********************************************************************
C 10.5.94 STATUS UNGESTET
C**********************************************************************
...
C ccccccccccccccccccccccccccccccccccccccccccc
C ccc                                     ccc
C ccc     U N T E R P R O G R A M M       ccc
C ccc     -------------------------       ccc
C ccc     D i c h t e - E f f e k t       ccc
C ccc                                     ccc
C ccccccccccccccccccccccccccccccccccccccccccc
C ccccccccccccccccccccccccccccccccccccccccccc
...
C ccc Output an das rufende Programm ccc
C cccccccccccccccccccccccccccccccccccccc

C     Dichte_Effekt nach Sternheimer 1984
C     (At. Data Nucl. D. Tables Vol. 30, No. 2, 262 (1984));

      double precision d_del_71,d_del_84

C ccccccccccccccccccccccccccccccccccc
C ccc Input vom rufenden Programm ccc
C ccccccccccccccccccccccccccccccccccc

      double precision d_beta

c     double precision d_z2
      integer*4 d_z2
...
C cccccccccccccccccccccccccccccccccccccc
C ccc Globale Parameter und Routinen ccc
C cccccccccccccccccccccccccccccccccccccc


C ccccccccccccccccccccccccccccccccccccc
C ccc UP-interne Parameter/Variable ccc
C ccccccccccccccccccccccccccccccccccccc
...
C ccc Beginn Unterprogramm ccc
C cccccccccccccccccccccccccccc
...
C ccc Sternheimer 1984 ccc
C cccccccccccccccccccccccc


C ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
C     Vorbesetzen mit Werten aus ADND 1984, S. 262
C ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
 	...
 	d_x0(2)     = 2.2017
        ...
        d_x0(83)    = 0.4152
        d_x0(84)    = 0.4267
ccccccc d_x0(85)    = 0.0000
        d_x0(86)    = 1.5368
ccccccc d_x0(87)    = 0.0000
        d_x0(88)    = 0.5991
        d_x0(89)    = 0.4559
        d_x0(90)    = 0.4202
        d_x0(91)    = 0.3144
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        ...
        d_x1(83)    = 3.8248
        d_x1(84)    = 3.8293
ccccccc d_x1(85)    = 0.0000
        d_x1(86)    = 4.9889
ccccccc d_x1(87)    = 0.0000
        d_x1(88)    = 3.9428
        d_x1(89)    = 3.7966
        d_x1(90)    = 3.7681
        d_x1(91)    = 3.5079
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        ...
        d_a(83)     = 0.09410
        d_a(84)     = 0.09282
ccccccc d_a(85)     = 0.0000
        d_a(86)     = 0.20798
ccccccc d_a(87)     = 0.0000
        d_a(88)     = 0.08804
        d_a(89)     = 0.08567
        d_a(90)     = 0.08655
        d_a(91)     = 0.14770
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        ...
        d_m(83)     = 3.1671
        d_m(84)     = 3.1830
ccccccc d_m(85)     = 1.1111
        d_m(86)     = 2.7409
ccccccc d_m(87)     = 1.1111
        d_m(88)     = 3.2454
        d_m(89)     = 3.2683
        d_m(90)     = 3.2610
        d_m(91)     = 2.9845
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        ...
        d_del_0(83) = 0.14
        d_del_0(84) = 0.14
ccccccc d_del_0(85) = 0.0
        d_del_0(86) = 0.0
ccccccc d_del_0(87) = 0.0
        d_del_0(88) = 0.14
        d_del_0(89) = 0.14
        d_del_0(90) = 0.14
        d_del_0(91) = 0.14
...
C
c     real*8 FUNCTION e_out(t,bcoef,n,k,range,ei,fraction)
...
C                                                                      C
C           BERECHNUNG DES UNBESTIMMTEN INTEGRALS I(X) EINER FUNKTION  C
C           F(X) IM INTERVALL A<=X<=B DURCH LOESUNG DER                C
C                                                                      C
C           DGL: (D/DX) I(X) = F(X)             MIT DER                C
C           RB :  I(X=C) = 0                    (MIT COLSYS)           C
C                                                                      C
C           D. H. I(B)-I(A) = DAS BESTIMMTE INTEGRAL VON A BIS B       C
C                                                                      C
C           TOL,IPRINT,FSPACE,ISPACE,NDIMF,NDIMI,ISPACE SIEHE CLSYS    C
C                                                                      C
C           DAS INTEGRAL STEHT IN FSPACE UND ISPACE        ''          C
C                                                                      C
C                                                       20.3.86        C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
c
c--- modified to include m as input parameter   P.M.
c******  i n p u t  ******
...
C          CONSTANT ASSIGNMENT) ROUTINES -- SEE (*), (2), (5)
C-----------------------------------------------------------------------
C
C***********************************************************************
C
...
C----  Calculates angular straggling of molecules
       ...
       do i = 1, nnuc
         omthver = omthver + anuc(i) * omth(zp, mp, zt(i), mt(i),
     &       energy) * mt(i) / m
       enddo
...
C----  Calculates angular straggling
...
************************************************************************
C-----------------------------------------------------------------------
C   calculates proton electronic stopping powers
C     Z   target
C     E   energy
C-----------------------------------------------------------------------
C   Ref.: Ziegler et al:
...
C-----------------------------------------------------------------------
C 10.5.94 status: OK
C                 PCOEF(8,92) aus KC71.SCOEF.DATA
C                   ist das ok ???
C-----------------------------------------------------------------------
      implicit real*8 (a-h,o-z)
      INTEGER*4  Z
     ...
     &        .59674d0, .0050837d0, 4.2073d0, .30612d0,
     &        2394.2d0, 1.0255d0, 4892.1d0, .0160060d0,
     &        .75253d0, .0050314d0, 4.0824d0, .30067d0,
...
C
C Output:
C           ATRHO  = ATOMS/CM3*1C22, (NORMAL ABUNDANCE)
C           VFERMI = (FERMI VELOCITY OF SOLID) / V0,
C           LFCTR  = LAMBDA SCREENING FACTOR FOR IONS
C           POT    = IONISATION POTENTIAL IN EV (AHLEN)
C                AHLEN 1980, RECOMMENDED VALUES
C     9. Jan. 1996: Ahlen's I-values replaced by values
C                   recommended in ICRU Report #49 (1994)
C                   CSch. 9. 1. 1995
C-----------------------------------------------------------------------
C Ref:      Ziegler et al :
C           THE STOPPING AND RANGE OF IONS IN SOLID
C           PERGAMON 1985
C           PAGE :  262 - 263
C-----------------------------------------------------------------------
C STATUS:    NOCH NICHT GETESTET 10.05.94
C            TABLE(4,92) AUS KC71,SCOEF DATA
C-----------------------------------------------------------------------
*-----------------------------------------------------------------------
...
*-----------------------------------------------------------------------
*     for PHITS, modified ionizaton potential for water
...
************************************************************************
C-----------------------------------------------------------------------
C Calculates dE/dx (Ziegler,Bethe), Range & Rangestraggling, Q/ZP
C-----------------------------------------------------------------------
C Ref.: Ziegler et all:
C       The Stopping and Range of Ions in Solids
C       Pergamon 1985
C       Pages 256-263
C-----------------------------------------------------------------------
C INPUTS:
C
C     ZP = Atomic Number of the Proctile
C     ZT = Atomic Number of the Target
C     MP = Mass Number of the Projectile
C     MT = Mass Number of the Target
C     ENERGY = Energy of the Projectile
C     RHO = Density in
C
C  OUTPUTS:
C     SEOUT = Calculated electronic Stopping
C     ZETA  = Calculated Screening
C-----------------------------------------------------------------------
C 10.05.94 Status: ungetestet
C-----------------------------------------------------------------------
      implicit real*8 (a-h,o-z)
      INTEGER*4 ZP,ZT
...
C     ..VELOCITY PRORTIONAL STOPPING BELOW 1 KEV / AMU
        HE0 = 1.0D0
        HE = DMAX1(HE0, E)
C     ..ZEFF - HELIUM
        B = DLOG (HE)
     ...
     &        DEXP(-(7.6D0 - DMAX1(0.0D0 , DLOG(HE)))**2))
        ...
        IF (E.LE.HE0) THEN
C       ..CALCULATE HE VELOCITY PROP. STOPPING
          SE = SE * DSQRT(E/HE0)
        END IF
        SEOUT = SE * 0.60222D0/MT
      ELSE IF (ZP .GT. 2) THEN
C     ..HEAVY ION ELECTRONIC STOPPING POWER USE VELOCITY STOPPING FOR
C     ..YRMIN = VR / ZP**0.67 <= 0.13 OR VR <= 1.0
        YRMIN = 0.130D0
        VRMIN = 1.0D0
     ...
     &    (1.D0 + (2.D0/3.D0*V**2) - (V**4)/15.D0)
        END IF
C       SET YR = MAX OF: VR/ZP**0.67 , VRMIN/ZP**0.67 , YRMIN
...
C       Q = IONIZATION LEVEL OF THE ION AT RELATIVE VELOCITY YR
        Q = DMIN1(1.0D0, DMAX1(0.0D0 , (1.0D0 -
     &        DEXP (-DMIN1(A, 50.0D0)))))
        if(ifixchg.ge.1) Q=1.0d0 ! fixed charge mode, T.Sato 2019/02/17
C       COVERSATION OF IONIZATION LEVEL TO EFFECTIVE CHARGE
...
C        ..CALCULATE VELOCITY STOPPING FOR YR < YRMIN
     ...
     &       (ZP .LE. 19))) THEN
           ...
           HELP5 = HELP2 * HELP4
           SE = SP * HELP5
C          SE = SP*((ZETA*DBLE(ZP))**2)*((E/EEE)**POWER)
           SEOUT=SE*0.60222D0/MT
         ELSE
...
C  *******************************************************************
C  *                                                                 *
C  *  THIS FUNCTION CALCULATES THE RADIATION LENGTH                  *
C  *  REF.: DATA BOOKLET 1984                                        *
C  *  LR IN MG/CM**2                                                 *
C  *******************************************************************
C  22.8.94 STATUS UNGETESTET
C  *******************************************************************
     ...
     &     -1.202D0*ALPHA**2*DBLE(Z)**2+1.0369D0*ALPHA**4*DBLE(Z)**4
...
C       X(*),...,X(NCOL)  CONTAINS, ON INPUT, THE RIGHT SIDE FOR THE
...
C       X(*),...,X(NCOL) CONTAINS, ON OUTPUT, THE COMPONENTS OF THE
