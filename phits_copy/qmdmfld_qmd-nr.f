...
*-----------------------------------------------------------------------
            ...
            eij  = p(4,i) + p(4,j)
            eijinv  = 1. / (p(4,i) + p(4,j))

         do l = 1, 3

            rij  =   r(l,i) - r(l,j)
            pij  =   p(l,i) - p(l,j)
            bij  = ( p(l,i) + p(l,j) ) *  eijinv
      ...
      if( iqmdscm .eq. 1 ) then
         ...
         do j = 2, massal
            ...
            do i = 1, j - 1
               ...
               hlp0 = dble( iesc(i) * iesc(j) )

               rha(i,j) = hlp0 * rha(i,j)
               rha(j,i) = rha(i,j)
               rhe(i,j) = hlp0 * rhe(i,j)
               rhe(j,i) = rhe(i,j)
               rhc(i,j) = hlp0 * rhc(i,j)
               rhc(j,i) = rhc(i,j)

               if( hlp0 .eq. 0.d0 ) then
                  rr2(i,j) = 1.d+2
                  rr2(j,i) = rr2(i,j)
               endif

               rbij(i,j) = hlp0 * rbij(i,j)
               rbij(j,i) = -1.d0 * rbij(i,j)
               pp2(i,j) = hlp0 * pp2(i,j)
               pp2(j,i) = pp2(i,j)
...
*-----------------------------------------------------------------------
     ...
     &            +  cl  * rhc(j,i)
             ...
             do n = 1, 3
               ...
               cij(n) =   bij(n) - bi(n)
               ffr(n,i) = ffr(n,i)
     &                  + binds(j) * 2.0 * ccrr * (rij(n) + grbb*cij(n))
               ffp(n,i) = ffp(n,i)
     &                  - binds(j) * 2.0 * ccpp * (rij(n) + grbb*bij(n))
             end do
...
*-----------------------------------------------------------------------
      ...
      if( iqmdscm .eq. 1 ) then
         do i = 1,massal
            do k = 1, 3
               ffr(k,i) = ffr(k,i) * dble(iesc(i))
               ffp(k,i) = ffp(k,i) * dble(iesc(i))
            enddo
         enddo
      endif
...
*-----------------------------------------------------------------------
               ...
               if( iqmdscm .eq. 1 .and. iesc(i) .eq. 0 ) return      ! S.Abe 2020/09/29
...
*-----------------------------------------------------------------------
               ...
               do 100 i = 1, massal
                  if( inds(i) .eq. 0 ) goto 100
                  if( iqmdscm .eq. 1 .and. iesc(i) .eq. 0 ) goto 100      ! S.Abe 2020/09/29

               do 110 j = 1, massal
                  if( inds(j) .eq. 0 ) goto 110
                  if( iqmdscm .eq. 1 .and. iesc(j) .eq. 0 ) goto 110      ! S.Abe 2020/09/29
...
************************************************************************
*                                                                      *
      subroutine etotal(nsel,ekin,epot,ebin,emas,epin,jj)
*                                                                      *
*                                                                      *
*        Last Revised:     2004 09 23                                  *
*                                                                      *
...
*-----------------------------------------------------------------------
     ...
     &                 + cl * epotc
                  ...
                  ebin = ( ekin + epot ) / dble( it(0) - ipion )
...
*                                                                      *
************************************************************************
      use QMD_COOD2_MOD, only : it, rs, ps
...
*------------------------------------------------------------
      ...
      if( iqmdscm. eq. 1 .and. massal .gt. massba ) then
