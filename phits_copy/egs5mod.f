!-
! better , save max size for EGS5 and PHITS_input
! (1) phits input size
!----------------------------------------------------------!
!  val_name / common_name      = parameter                 !
!  PHITS2                        EGS5                      !
!----------------------------------------------------------!
!  mxmat / kmat1a              = MXMED
!  iregn / inggs , igcel/inpec = MXREG
!  mxnel/kmat1a                = MXNEL ( or MXEPERMED )
!
! (2) EGS5  size
!     parameter = mxns, mxcp
!     MODULE  WORK_MAX_SIZE
!     implicit none
!     integer  max_mat, max_el, max_region
!     END MODULE  WORK_MAX_SIZE
!======================== ALLOCATE brempr ==============================
* module routine replacing include/egs5_brempr.f for variable allocation
!> original include
!-----------------------------------------------------------------------
! egs5_brempr.f
! Version: 051219-1435
...
!..
!>ada.2014.12> private
! THREADPRIVATE(/BREMPR_priv/)
!<ada.2014.12< private
!< original include
!------------------------last line of egs5_brempr.f---------------------
      MODULE EGS5_BREMPR_THREADPRIVATE
      implicit none
!>private
      real*8 :: fbrspl
      integer*4  :: ibrdst, iprdst, ibrspl,nbrspl
c>
!$OMP THREADPRIVATE ( fbrspl, ibrdst, iprdst, ibrspl, nbrspl )
c
      data fbrspl /0.0d0/
      data ibrdst,iprdst,ibrspl,nbrspl /0,0,0,0 /
c
      END MODULE EGS5_BREMPR_THREADPRIVATE
!........................................................................
      MODULE  egs5_brempr_mod
          USE EGS5_BREMPR_THREADPRIVATE
      implicit none
!
      real*8 , allocatable ::
c> flag allocate check flg_brempr_1
     + asym(:,:,:),
c> flag allocate check flg_brempr_2
     + dl1(:,:), dl2(:,:), dl3(:,:),
     + dl4(:,:), dl5(:,:), dl6(:,:),
c> flag allocate check flg_brempr_3
     + alphi(:,:), bpar(:,:), delpos(:,:),
     + wa(:,:), pz(:,:),
     + zelem(:,:), rhoz(:,:),
     + pwr2i(:), delcm(:),
     + zbrang(:)
c> flag allocate check flg_brempr_4
      integer*4 , allocatable :: nne(:)
c> flag allocate check brempr
      integer flg_brempr_1, flg_brempr_2,
     +        flg_brempr_3, flg_brempr_4

      data flg_brempr_1, flg_brempr_2,
     +     flg_brempr_3, flg_brempr_4  /4* -1/
c< flag allocate check brempr
...
!.....
      CONTAINS
!-----------------------------------------------------------!
      SUBROUTINE ALLOCATE_EGS5_BREMPR(iMXMED, MXEL, MXPWR2I)
      implicit none
      integer , intent(in) :: iMXMED, MXEL, MXPWR2I
c1
      allocate( asym(iMXMED,MXEL,2) ,stat= flg_brempr_1 )
c2
      allocate(
     + dl1(6,iMXMED), dl2(6,iMXMED), dl3(6,iMXMED),
     + dl4(6,iMXMED), dl5(6,iMXMED), dl6(6,iMXMED),
     +  stat= flg_brempr_2 )
c3
      allocate(
     + alphi(2,iMXMED), bpar(2,iMXMED), delpos(2,iMXMED),
     + wa(iMXMED,MXEL), pz(iMXMED,MXEL),
     + zelem(iMXMED,MXEL), rhoz(iMXMED,MXEL),
     + pwr2i(MXPWR2I), delcm(iMXMED),
     + zbrang(iMXMED), stat= flg_brempr_3 )
c4
      allocate( nne(iMXMED), stat= flg_brempr_4 )
c<
      return
      END SUBROUTINE ALLOCATE_EGS5_BREMPR
!.....
      SUBROUTINE DEALLOCATE_EGS5_BREMPR
      implicit none

      if(  flg_brempr_1 .eq. 0 )then
       DEALLOCATE( asym )
      endif
      if(  flg_brempr_2 .eq. 0 )then
       DEALLOCATE( dl1 , dl2 , dl3 , dl4 , dl5 , dl6 )
      endif
      if( flg_brempr_3 .eq. 0 ) then
       DEALLOCATE( alphi, bpar, delpos, wa, pz,
     +             zelem, rhoz,   pwr2i, delcm, zbrang )
      endif
      if( flg_brempr_4 .eq. 0 ) then
       DEALLOCATE( nne  )
      endif

      END SUBROUTINE DEALLOCATE_EGS5_BREMPR
      END MODULE  egs5_brempr_mod
!==================== END ALLOCATE brempr ==============================
!==================== ALLOCATE edge ====================================
* module routine replacing include/egs5_edge.f for variable allocation
!> original include
!-----------------------------egs5_edge.f-------------------------------
! Version: 051219-1435
!-----------------------------------------------------------------------
...
! EDGE BLOCKDATA ATOM
!
       MODULE EGS5_EDGE_BLOCKDATA_ATOM
       implicit none
       real*8 ::
     + eedge(4,100), ekx(10,100), dfkx(10,100),
     + elx1(8,100), elx2(5,100), elx3(7,100),
     + dflx1(8,100), dflx2(5,100), dflx3(7,100),
     + pm0(5,100), pm1(5,100), pm2(5,100), pm3(5,100),
     + ekaug(14,100), dfkaug(13,100),
     + el1aug(6,100), el2aug(6,100), el3aug(6,100),
     + dfl1aug(6,100), dfl2aug(6,100), dfl3aug(6,100),
     + omegak(100), embind(100),
     + omegal1(100), omegal2(100), omegal3(100),
     + f12(100),f13(100), f23(100)
...
!---------------------------------------------------------------------!
     ...
     *0.99830D+00,0.99821D+00,0.99812D+00,0.99802D+00,0.99793D+00/
!!
!! note :  NEPM(:) size was MXMED
!!
!------------------last line of egs5_block_data_atom.f------------------
       END MODULE EGS5_EDGE_BLOCKDATA_ATOM

!.......................................................................!
       MODULE EGS5_EDGE_THREADPRIVATE
      implicit none
c>  OpenMP ThreadPrivate
       real*8 ::
     + exray(10), eauger(10),  !<- r*8 thread private
     + ebind                   !<- r*8 thread private
      integer ::
     + nauger,nxray, iz, iextp !<- i*4 thread private
!$OMP THREADPRIVATE(exray ,eauger ,ebind,nauger,nxray,iz,iextp)
cccc: note: can not compile on Intel-Fortran
c> Initialize for OpenMP ThreadPrivate
      data exray /10*0.0d0/
      data eauger /10*0.0d0/
      data ebind /0.0d0/
      data nauger,nxray,iz,iextp /0,0,0,0/
c< Initialize for OpenMP
       END MODULE EGS5_EDGE_THREADPRIVATE
!.....................................................................!
       MODULE EGS5_EDGE_MOD
           USE EGS5_EDGE_BLOCKDATA_ATOM
           USE EGS5_EDGE_THREADPRIVATE
c> EDGE
c> flag allocate check edge 1
       real*8 , allocatable :: edgb(:,:)
c>
       integer , allocatable ::
c> flag allocate check edge 2
     + ledgb(:,:), nedgb(:),
c> flag allocate check edge 3
     + nepm(:),
c> flag allocate check edge 4
     + iedgfl(:),iauger(:)
!< EDGE dynamic
c> flag allocate check edge
       integer  flg_edge_1, flg_edge_2,
     +          flg_edge_3, flg_edge_4
       data  flg_edge_1, flg_edge_2,
     +       flg_edge_3, flg_edge_4 /4* -1/
c< flag allocate check edge
!.....
      CONTAINS
!-----------------------------------------------------------!
       SUBROUTINE ALLOCATE_EGS5_EDGE(iMXMED,MXEPERMED,MXREG)
       implicit none
       integer , intent(in) :: iMXMED , MXEPERMED,MXREG
       integer :: i
c1
       allocate( edgb(MXEPERMED*4, iMXMED) , stat=flg_edge_1 )
c2
       allocate(ledgb(MXEPERMED*4, iMXMED), nedgb(iMXMED),
     $          stat=flg_edge_2)
c3 corrected size
      allocate( nepm(imxmed), stat=flg_edge_3)
c4
       allocate( iedgfl(MXREG), iauger(MXREG) , stat=flg_edge_4)
c> initialized BLOCKDATA ATOM & correct size -2015.09.10 size of "nepm"
c
       do i=1,imxmed  ! do i=1,mxepermed
        NEPM(i) = 1
       enddo
       return
       END SUBROUTINE ALLOCATE_EGS5_EDGE
!.....
      SUBROUTINE DEALLOCATE_EGS5_EDGE
      implicit none

      if( flg_edge_1 .eq. 0 )then
        DEALLOCATE( edgb )
      endif
      if( flg_edge_2 .eq. 0 )then
        DEALLOCATE( ledgb, nedgb )
      endif
      if( flg_edge_3 .eq. 0 )then
        DEALLOCATE( nepm )
      endif
      if( flg_edge_4 .eq. 0 )then
        DEALLOCATE( iedgfl ,iauger  )
      endif

       END SUBROUTINE DEALLOCATE_EGS5_EDGE
       END MODULE EGS5_EDGE_MOD
!==================== END ALLOCATE edge ================================
!======================== ALLOCATE eiicom ==============================
* module routine replacing include/egs5_eiicom.f for variable allocation
!> original include
!-----------------------------egs5_eiicom.f-----------------------------
! Version: 051219-1435
!-----------------------------------------------------------------------
...
c> THREADPRIVATE EIICOM
      MODULE  EGS5_EIICOM_THREADPRIVATE
      implicit none

      real*8 feispl
      integer ieispl,neispl
!$OMP THREADPRIVATE( feispl,ieispl,neispl )
cccc note: can not compile on Intel-Fortran
c Initialize for ThreadPrivate
       data feispl /0.0d0/
       data ieispl,neispl /0,0/
c
      END MODULE  EGS5_EIICOM_THREADPRIVATE
c< THREADPRIVATE EIICOM
!....
      MODULE EGS5_EIICOM_MOD
           USE EGS5_EIICOM_THREADPRIVATE
      implicit none
      real*8 , allocatable ::
c> flag allocate check eiicom 1
     + eico0(:) , eico1(:),
c> flag allocate check eiicom 2
     + eii0(:,:,:) , eii1(:,:,:)
c> flag allocate check eiicom 2
       integer flg_eiicom_1, flg_eiicom_2
       data flg_eiicom_1, flg_eiicom_2 /2* -1/
c< flag allocate check eiicom 2
!.....
      CONTAINS
!-----------------------------------------------------------!
      SUBROUTINE ALLOCATE_EGS5_EIICOM (iMXMED,MXEKE,MXEPERMED)

      implicit none

      integer , intent(IN) :: imxmed , MXEKE,MXEPERMED

c1 eii0,eii1 = read_file in HATCH
      allocate( eico0( iMXMED ), eico1( iMXMED ),
     +  stat= flg_eiicom_1 )
c2
      allocate(
     + eii0( MXEKE, MXEPERMED, iMXMED) ,
     + eii1( MXEKE, MXEPERMED, iMXMED) ,
     +  stat= flg_eiicom_2 )
c
      END SUBROUTINE ALLOCATE_EGS5_EIICOM

!.....
      SUBROUTINE DEALLOCATE_EGS5_EIICOM
      implicit none

      if( flg_eiicom_1 .eq. 0 ) then
        DEALLOCATE( eico0,eico1 )
      endif
      if( flg_eiicom_2 .eq. 0 ) then
        DEALLOCATE( eii0, eii1 )
      endif

      END SUBROUTINE DEALLOCATE_EGS5_EIICOM
      END MODULE EGS5_EIICOM_MOD
!======================== END ALLOCATE eiicom ==============================
!======================== ALLOCATE elecin ==============================
* module routine replacing include/egs5_elecin.f for variable allocation
!> original include
!-----------------------------egs5_elecin.f-----------------------------
! Version: 060317-1500
!-----------------------------------------------------------------------
...
! ELECIN BLOCK DATA EGS5
      MODULE EGS5_ELECIN_BLOCKDATA_EGS5
      implicit none
c> allocatable 2015.08xx : module "egs5_elecin_mod"
      real*8 ekelim
      integer   icomp
      data                                              ! common/ELECIN/
     + ekelim/0./,
     + icomp/1/
      END MODULE EGS5_ELECIN_BLOCKDATA_EGS5
!.....
      MODULE EGS5_ELECIN_MOD
          USE EGS5_ELECIN_BLOCKDATA_EGS5
      implicit none
c
c... ELECIN , array
      real*8 , allocatable ::
     1 eke0(:), eke1(:),
     2 xr0(:), teff0(:), blcc(:), xcc(:),
     3 esig0(:,:), esig1(:,:) ,
     4 psig0(:,:), psig1(:,:) ,
     5 ededx0(:,:), ededx1(:,:) ,
     6 pdedx0(:,:), pdedx1(:,:) ,
...
c
      integer   , allocatable ::
     7 msmap(:),
     8 iunrst(:), epstfl(:), iaprim(:)
c
c... ELECIN , scaler
c> Initialized BLOCK DATA EGS5
c< Initialized BLOCK DATA EGS5
      integer msteps,jrmax,mxv1,mxv2
c....... ELECIN

c flag allocate check elecin
      integer flg_elecin
      data flg_elecin /-1/
!.....
      CONTAINS
!-----------------------------------------------------------!
c
      SUBROUTINE ALLOCATE_EGS5_ELECIN
     ...
     + imxmed , MXEKE,MSSTEPS,MXVRT1,MXVRT2,MXJREFF
      ...
      allocate(
     1 eke0(imxmed),eke1(imxmed),
     2 xr0(imxmed),teff0(imxmed),blcc(imxmed),xcc(imxmed),
     3 esig0(MXEKE,imxmed), esig1(MXEKE,imxmed),
     4 psig0(MXEKE,imxmed), psig1(MXEKE,imxmed),
     5 ededx0(MXEKE,imxmed), ededx1(MXEKE,imxmed),
     6 pdedx0(MXEKE,imxmed), pdedx1(MXEKE,imxmed),
     7 erang0(MXEKE,imxmed), erang1(MXEKE,imxmed),
     8 prang0(MXEKE,imxmed), prang1(MXEKE,imxmed),
     9 estep0(MXEKE,imxmed),estep1(MXEKE,imxmed),
     1  ebr10(MXEKE,imxmed), ebr11(MXEKE,imxmed),
     2  pbr10(MXEKE,imxmed), pbr11(MXEKE,imxmed),
     3  pbr20(MXEKE,imxmed), pbr21(MXEKE,imxmed),
     4  tmxs0(MXEKE,imxmed), tmxs1(MXEKE,imxmed),
     5  fstep(MSSTEPS),fsqr(MSSTEPS),
     6  vert1(MXVRT1),vert2(MXVRT2,MSSTEPS),
     7  msmap(MXJREFF),
     8 iunrst(imxmed), epstfl(imxmed), iaprim(imxmed),
     + stat = flg_elecin )

      END SUBROUTINE ALLOCATE_EGS5_ELECIN
!.....
      SUBROUTINE DEALLOCATE_EGS5_ELECIN
      implicit none

      if( flg_elecin .eq. 0 )then
          DEALLOCATE(
     1       eke0, eke1,
     2        xr0, teff0,  blcc,  xcc,
     3      esig0,  esig1,
     ...
     8     iunrst, epstfl, iaprim )
      ...
      END SUBROUTINE DEALLOCATE_EGS5_ELECIN
      END MODULE EGS5_ELECIN_MOD
!======================== END ALLOCATE elecin ==============================
************************************************************************
      module EGS5_MS_MOD
*
*     module routine replacing include/egs5_ms.f for variable allocation
*     created by T.Furuta on 2014/08/25
*
!-----------------------------------------------------------------------
! Version: 060313-1000
!-----------------------------------------------------------------------
...
c
      ...
      double precision,allocatable :: eamu(:,:,:,:,:), ebms(:,:,:,:,:),
     &     eetams(:,:,:,:,:), ecdf(:,:,:,:,:), pnoscat(:,:,:,:),
     &     msgrid(:,:), k1grd(:,:)
      double precision :: dk1log(2)      ! K1 grid spacing constant
      integer,allocatable ::  nmsgrd(:), initde(:), nmsdec(:), jskip(:),
     &     neqp(:), neqa(:)

c>flag allocate check egs5_ms_mod
      integer flg_ms
      data flg_ms  /-1/
c<flag allocate check egs5_ms_mod
!.....
      CONTAINS
!-----------------------------------------------------------!
      SUBROUTINE ALLOCATE_EGS5_MS(NFIT,NFIT1,NEXFIT1,NMSE,NK1,iMXMED)
      implicit none
      integer , intent(in) :: NFIT,NFIT1,NEXFIT1,NMSE,NK1,iMXMED

c
      allocate(
     +   eamu(NFIT1,2,NMSE,NK1,iMXMED),   ! endpoints of the e- intervals
     +   ebms(NFIT,2,NMSE,NK1,iMXMED),    ! Second e- fitting coeff
     +   eetams(NFIT,2,NMSE,NK1,iMXMED),  ! Fitted e- screening parameter
     +   ecdf(NEXFIT1,2,NMSE,NK1,iMXMED), ! e- cdf for final interval
     +   pnoscat(2,NMSE,NK1,iMXMED),      ! non-scatter probability
     +   msgrid(NMSE,iMXMED),             ! energy grid
     +   k1grd(2,NK1),                   ! K1 grid
     +   nmsgrd(iMXMED),                  ! number of energy grid pts.
     +   initde(iMXMED),                  ! initial energy decade
     +   nmsdec(iMXMED),                  ! energy grid points per decade
     +   jskip(iMXMED),                   ! number to skip in 1st decade
     +   neqp(iMXMED),                    ! actual # eq prob angles
     +   neqa(iMXMED)                     ! actual # eq spaced angles
     +   , stat = flg_ms )
c
      if( flg_ms .ne. 0 ) then
      write(*,*)'# EGS5_MS_MOD can not allocate !! '
      STOP'# EGS5_MS_MOD can not allocate !! '
      endif
      END SUBROUTINE ALLOCATE_EGS5_MS
!.....
      SUBROUTINE DEALLOCATE_EGS5_MS
      implicit none

      if( flg_ms .eq. 0 )then
       DEALLOCATE(eamu, ebms, eetams, ecdf, pnoscat,msgrid, k1grd,
     +          nmsgrd, initde, nmsdec, jskip, neqp, neqa)
      endif

      END SUBROUTINE DEALLOCATE_EGS5_MS
      END module EGS5_MS_MOD
************************************************************************
!======================================================================!
      module EGS5_BCOMP_MOD
*
*     module routine replacing include/egs5_bcomp.f for variable allocation
*     created by T.Ogawa on 2014/08/30
*
!------------------------------egs5_bcomp.f-----------------------------
! Version: 051219-1435
!-----------------------------------------------------------------------
************************************************************************
      ...
      double precision,allocatable :: sco0(:),sco1(:),
     * sxz0(:,:),sxz1(:,:),cco0(:),cco1(:),
     * cpr0(:,:),cpr1(:,:),
     * elecno(:,:),capio(:,:),ccos0(:),ccos1(:),
     * cprs0(:,:,:),cprs1(:,:,:)
      double precision cpimev
      integer, allocatable :: icprof(:), mxshel(:)
c
c>ada 2015.10.01
c> max size ,  "lay" and  "hatch"
      integer  mx_ngsim, mx_ngcim  , mx_ngcim3 , mx_mxsim
      data mx_ngsim, mx_ngcim  , mx_ngcim3 , mx_mxsim /4*0/
c> flag allocate check bromp_1
      integer flg_bcomp_1, flg_bcomp_2,
     +        flg_bcomp_3, flg_bcomp_4
      data flg_bcomp_1, flg_bcomp_2,
     +     flg_bcomp_3, flg_bcomp_4 /4* -1/
c< flag allocate check bromp_1
!.....
      CONTAINS
!-----------------------------------------------------------!
      SUBROUTINE ALLOCATE_EGS5_BCOMP(iMXMED, MXSCTFF, MXCP, MXNS )

      implicit none
      integer , intent(in) :: iMXMED, MXSCTFF, MXCP, MXNS
c 1d
      allocate(
     1   sco0(iMXMED),sco1(iMXMED), cco0(iMXMED),cco1(iMXMED),
     1   ccos0(iMXMED),ccos1(iMXMED), icprof(iMXMED), mxshel(iMXMED),
     +                          stat = flg_bcomp_1 )
c
cc reallocate_1
cc reallocate_2
cc reallocate_3
      END SUBROUTINE ALLOCATE_EGS5_BCOMP
!.....
      SUBROUTINE DEALLOCATE_EGS5_BCOMP
      implicit none

      if( flg_bcomp_1 .eq. 0 ) then
        DEALLOCATE( sco0, sco1, cco0, cco1,
     +              ccos0, ccos1, icprof, mxshel )
      endif
c
      if( flg_bcomp_2 .eq. 0 ) then
        DEALLOCATE( sxz0, sxz1)
      endif
c
      if( flg_bcomp_3 .eq. 0 ) then
        DEALLOCATE( cpr0, cpr1)
      endif
c
      if( flg_bcomp_4 .eq. 0 ) then
        DEALLOCATE( elecno, capio )
        DEALLOCATE( cprs0, cprs1 )
      endif

      END SUBROUTINE DEALLOCATE_EGS5_BCOMP
      END module EGS5_BCOMP_MOD
!======================================================================!
!==================== ALLOCATE  MEDIA ================================!
* module routine replacing include/egs5_media.f for variable allocation
!> original include
!-----------------------------egs5_media.f------------------------------
! Version: 051219-1435
!-----------------------------------------------------------------------
...
!< original include
!
      MODULE EGS5_MEDIA_BLOCKDATA_EGS5
      implicit none
c
      character(len=4) media1(24)
      integer nmed
!
! "media1" , "nmed"
!
     ...
     + nmed/1/
      ...
      END MODULE EGS5_MEDIA_BLOCKDATA_EGS5
!
      MODULE egs5_media_mod
         USE EGS5_MEDIA_BLOCKDATA_EGS5
!
      implicit none
!
      real*8 , allocatable ::
     + rlcm(:), rldu(:), rhom(:), charD(:)

      integer , allocatable ::
     + iraylm(:), incohm(:), iprofm(:), impacm(:),
     + useGSD(:)
!
!> BLOCK DATA initialized  "media1", "nmed"
!  equivalence media1 and media
       character(len=4), allocatable ::
     + media(:,:)
!<
c> flag allocate check media
       integer flg_media
       data flg_media /-1/
c< flag allocate check media
!.....
      CONTAINS
!-----------------------------------------------------------!
      SUBROUTINE ALLOCATE_egs5_media(imxmed)
      implicit none
      integer , intent(in) :: imxmed
      integer i

      allocate(
     + rlcm(iMXMED), rldu(iMXMED), rhom(iMXMED),
     + charD(iMXMED),
     + iraylm(iMXMED), incohm(iMXMED),
     + iprofm(iMXMED), impacm(iMXMED),
     + media(24,iMXMED) , useGSD(iMXMED)   ,
     + stat = flg_media )
!> Initialize BLOCKDATA EGS5 / original equivalence media1(24) .
        DO i=1,iMXMED
          charD(i) = 0.d0
          iraylm(i) = 0
          incohm(i) = 0
...
!< Initialize BLOCKDATA EGS5 / original equivalence media1(24) .
      ...
      END SUBROUTINE ALLOCATE_egs5_media
!.....
      SUBROUTINE DEALLOCATE_egs5_media
      implicit none

      if( flg_media .eq. 0 ) then
       DEALLOCATE( rlcm, rldu, rhom, charD,
     +              iraylm, incohm, iprofm, impacm,
     +              useGSD, media )
      endif

      END SUBROUTINE DEALLOCATE_egs5_media
      END MODULE egs5_media_mod
!==================== END ALLOCATE  MEDIA =============================!
!==================== ALLOCATE  MSCON =================================!
* module routine replacing include/egs5_mscon.f for variable allocation
!> original include
!-------------------------------egs5_mscon.f----------------------------
! Version: 060313-1030
!-----------------------------------------------------------------------
...
c     COMMON/MSCON/
c    +       MSCATE(NESCPW),            !  Energy grid
c    +       SCPOW(2,NESCPW),           !  Scattering power
c    +       AMUMS(2,NMSE,NK1,NFIT1),   !  endpoint of the fit interval
c    +       AMS(2,NMSE,NK1,NFIT),      !  First fitting coeff
c    +       BMS(2,NMSE,NK1,NFIT),      !  Second fitting coeff
c    +       CMS(2,NMSE,NK1,NFIT),      !  Third fitting coeff
c    +       ETAMS(2,NMSE,NK1,NFIT),    !  Fitted 'screening parameter'
c    +       CUMDIST(2,NMSE,NK1,NEXFIT),!  Cum msdist over each interval
c    +       PROBNS(2,NMSE,NK1),        !  No Scatter probabilities
c    +       K1MINE,                    !  e- min scat strngth, all med
c    +       K1MAXE,                    !  e- max scat strngth, all med
c    +       K1MINP,                    !  e+ min scat strngth, all med
c    +       K1MAXP,                    !  e+ max scat strngth, all med
c    +       GSOPT(MXMED),              !  flag to use Salvat GS MS
c    +       NMSCPW,                    !  Actual # of scat pow points
c    +       NMSCATE,                   !  Actual # of mscat points
c    +       DECADE1, DECADE2,          !  The decade range of grid
c    +       JOFFSET                    !  1st decade points to skip
c
!-----------------------last line of egs5_mscon.f-----------------------
!< original include
!
      MODULE EGS5_MSCON_MOD

      implicit none
!.....
       double precision , allocatable ::
     1       MSCATE(:),         !  Energy grid
     2       SCPOW(:,:),        !  Scattering power
     3       AMUMS(:,:,:,:),    !  endpoint of the fit interval
     4       AMS(:,:,:,:),      !  First fitting coeff
     5       BMS(:,:,:,:),      !  Second fitting coeff
     6       CMS(:,:,:,:),      !  Third fitting coeff
     7       ETAMS(:,:,:,:),    !  Fitted 'screening parameter'
     8       CUMDIST(:,:,:,:),  !  Cum msdist over each interval
     9       PROBNS(:,:,:)      !  No Scatter probabilities
!.....
       integer ,allocatable ::
     1       GSOPT(:)               !  flag to use Salvat GS MS
!.....
       double precision ::
...
!.....
      CONTAINS
!-----------------------------------------------------------!
      SUBROUTINE ALLOCATE_EGS5_MSCON
     + ( imxmed, NESCPW, NMSE, NK1, NFIT, NFIT1, NEXFIT)
!
      implicit none
      integer , intent(in) ::  imxmed,
     +          NESCPW, NMSE, NK1, NFIT, NFIT1, NEXFIT
!
       allocate(
     1       MSCATE(NESCPW),
     2       SCPOW(2,NESCPW),
     3       AMUMS(2,NMSE,NK1,NFIT1),
     4       AMS(2,NMSE,NK1,NFIT),
     5       BMS(2,NMSE,NK1,NFIT),
     6       CMS(2,NMSE,NK1,NFIT),
     7       ETAMS(2,NMSE,NK1,NFIT),
     8       CUMDIST(2,NMSE,NK1,NEXFIT),
     9       PROBNS(2,NMSE,NK1),
     1       GSOPT(imxmed)
     +  , stat=flg_mscon )
!
      END SUBROUTINE ALLOCATE_EGS5_MSCON
!.....
      SUBROUTINE DEALLOCATE_EGS5_MSCON
      implicit none

      if( flg_mscon .eq. 0 ) then
       DEALLOCATE(
     1       MSCATE, SCPOW, AMUMS, AMS, BMS,
     6       CMS, ETAMS, CUMDIST, PROBNS, GSOPT )
      endif

      END SUBROUTINE DEALLOCATE_EGS5_MSCON
      END MODULE EGS5_MSCON_MOD
!==================== END ALLOCATE MSCON =============================!
!============== ALLOCATE pegscommons/dcsstr.f ========================!
* module routine replacing pegscommons/dcsstr.f for variable allocation
!> original include
!----------------------------------dcsstr.f-----------------------------
!  Version: 060308-2100
!-----------------------------------------------------------------------
...
c
     ...
     1 MEDNAM(:,:)
      real*8 , allocatable::
     2     ATOMD(:),              !  atom density
     3     SECS(:,:),        !  e- cross section
     4     SETCS1(:,:),      !  e- 1st transport cross section
     5     SETCS2(:,:),      !  e- 2nd transport cross section
     6     SEDCS(:,:,:), !  e- differential cross section
     7     SPCS(:,:),        !  e+ cross section
     8     SPTCS1(:,:),      !  e+ 1st transport cross section
     9     SPTCS2(:,:),      !  e+ 2nd transport cross section
     1     SPDCS(:,:,:), !  e+ differential cross section
     2     EGRDHI(:),             !  essentially, UE
     3     EGRDLO(:)              !  essentially, AE
      integer, allocatable ::
     4      NLEGMD(:)              !  # legendres for GS dist
      real*8 , allocatable::
     5     EFRCH(:),              !  efrach for this medium
     6     EFRCL(:)               !  efracl for this medium
c>  BLOCK_DATA PEGSDAT
      integer   :: NSDCS
      data                                              ! common/DCSSTR/
     * NSDCS/0/                           ! Number of stored media DCS's
c
c> flag allocate check pegscommons/dcsstr
      integer   flg_dcsstr
      data   flg_dcsstr /-1/
c< flag allocate check pegscommons/dcsstr
c
!.....
      CONTAINS
!-----------------------------------------------------------!
      SUBROUTINE ALLOCATE_PEGS_DCSSTR
     + (imxmed, NEGRID,NREDA)

      implicit none
c
      integer , intent(in) :: imxmed, NEGRID,NREDA

c
      allocate(
     1     MEDNAM(imxmed,24),
     2     ATOMD(imxmed),
     3     SECS(imxmed,NEGRID),
     4     SETCS1(imxmed,NEGRID),
     5     SETCS2(imxmed,NEGRID),
     6     SEDCS(imxmed,NEGRID,NREDA),
     7     SPCS(imxmed,NEGRID),
     8     SPTCS1(imxmed,NEGRID),
     9     SPTCS2(imxmed,NEGRID),
     1     SPDCS(imxmed,NEGRID,NREDA),
     2     EGRDHI(imxmed),
     3     EGRDLO(imxmed),
     4     NLEGMD(imxmed),
     5     EFRCH(imxmed),
     6     EFRCL(imxmed)  , stat=flg_dcsstr )
      END SUBROUTINE ALLOCATE_PEGS_DCSSTR
!....
      SUBROUTINE DEALLOCATE_PEGS_DCSSTR
      implicit none

       if( flg_dcsstr .eq. 0)then
         DEALLOCATE(
     1               MEDNAM, ATOMD, SECS, SETCS1, SETCS2,
     6               SEDCS, SPCS, SPTCS1, SPTCS2, SPDCS,
     2               EGRDHI, EGRDLO, NLEGMD,
     5               EFRCH, EFRCL )
      endif

      END SUBROUTINE DEALLOCATE_PEGS_DCSSTR
      END MODULE PEGS_DCSSTR_MOD
!======== END ALLOCATE pegscommons/dcsstr.f ========================!
!======== ALLOCATE  photin  ========================================!
* module routine replacing include/egs5_photin.f for variable allocation
!> original include
!-----------------------------egs5_photin.f-----------------------------
! Version: 051219-1435
!-----------------------------------------------------------------------
!23456789|123456789|123456789|123456789|123456789|123456789|123456789|12
c
c     common/PHOTIN/                             ! Photon transport data
c    * ebinda(100),                      ! Average K-edge binding energy
c    * gmfp0(MXGE,MXMED),                     ! Gamma Mean Free Path fit
c    * gmfp1(MXGE,MXMED),                     !  coefficients
c    * gbr10(MXGE,MXMED),       ! Gamma Branching Ratio fit coefficients
c    * gbr11(MXGE,MXMED),       !  (pair/(pair+compton+photo)
c    * gbr20(MXGE,MXMED),       ! Gamma Branching Ratio fit coefficients
c    * gbr21(MXGE,MXMED),       !  (pair+compton)/(pair+compton+photo)
c>ada.2014.12>not use
c<ada.2014.12<not use
c>ada.2014.12>not use = mpgem
!-----------------------last line of egs5_photin.f----------------------
!< original include
!
      MODULE EGS5_PHOTIN_MOD

      implicit none
!
      real*8 ebinda(100)                       ! Average K-edge binding energy
!
      real*8 , allocatable ::
     + ge0(:),
     + ge1(:),
     + gmfp0(:,:),                     ! Gamma Mean Free Path fit
     + gmfp1(:,:),                     !  coefficients
     + gbr10(:,:),       ! Gamma Branching Ratio fit coefficients
     + gbr11(:,:),       !  (pair/(pair+compton+photo)
     + gbr20(:,:),       ! Gamma Branching Ratio fit coefficients
     + gbr21(:,:),       !  (pair+compton)/(pair+compton+photo)
     + rco0(:),
     + rco1(:),
     + rsct0(:,:),
     + rsct1(:,:),
...
!.....
      CONTAINS
!-----------------------------------------------------------!
      SUBROUTINE ALLOCATE_EGS5_PHOTIN(imxmed, MXGE, MXRAYFF)
      implicit none
      integer , intent(in) :: imxmed, MXGE, MXRAYFF
!
       allocate (
     + ge0(imxmed),
     + ge1(imxmed),
     ...
     + rco0(imxmed),
     + rco1(imxmed),
     + rsct0(MXRAYFF,imxmed),
     + rsct1(MXRAYFF,imxmed),
     + cohe0(MXGE,imxmed),
     + cohe1(MXGE,imxmed),
     + ngr(imxmed), stat=flg_photin )
      END SUBROUTINE ALLOCATE_EGS5_PHOTIN
!.....
      SUBROUTINE DEALLOCATE_EGS5_PHOTIN
      implicit none
!
       if( flg_photin .eq. 0) then
       DEALLOCATE(
     + ge0, ge1, gmfp0, gmfp1,
     + gbr10, gbr11, gbr20, gbr21, rco0, rco1,
     + rsct0, rsct1, cohe0, cohe1, ngr )
       endif
!
      END SUBROUTINE DEALLOCATE_EGS5_PHOTIN
      END MODULE EGS5_PHOTIN_MOD
!==================== END ALLOCATE photin ==============================
!==================== ALLOCATE scpw ====================================
* module routine replacing include/egs5_scpw.f for variable allocation
!> original include
!---------------------------egs5_scpw.f---------------------------------
! Version: 051219-1435
!-----------------------------------------------------------------------
!23456789|123456789|123456789|123456789|123456789|123456789|123456789|12
c
c     common/SCPW/                               ! scattering power data
c    * escpw0(MXEKE,MXMED),             ! first e- scpow interp constant
c    * escpw1(MXEKE,MXMED),            ! second e- scpow interp constant
c    * pscpw0(MXEKE,MXMED),             ! first e+ scpow interp constant
c    * pscpw1(MXEKE,MXMED),            ! second e+ scpow interp constant
c    * ekini0(MXEKE,MXMED),             ! first e- kinit interp constant
c    * ekini1(MXEKE,MXMED),            ! second e- kinit interp constant
c    * pkini0(MXEKE,MXMED),             ! first e+ kinit interp constant
c    * pkini1(MXEKE,MXMED),            ! second e+ kinit interp constant
c    * ek1s0(MXEKE,MXMED),             ! first e- k1s0 interp constant
c    * ek1s1(MXEKE,MXMED),            ! second e- k1s0 interp constant
c    * pk1s0(MXEKE,MXMED),             ! first e+ k1s0 interp constant
c    * pk1s1(MXEKE,MXMED),            ! second e+ k1s0 interp constant
c    * meke(MXMED)                       ! number of energy ladder steps
c
!-----------------------last line of egs5_scpw.f------------------------
!< original include
!
      MODULE EGS5_SCPW_MOD
!
      implicit none
       real*8 , allocatable ::
     + escpw0(:,:),             ! first e- scpow interp constant
     + escpw1(:,:),            ! second e- scpow interp constant
     + pscpw0(:,:),             ! first e+ scpow interp constant
     + pscpw1(:,:),            ! second e+ scpow interp constant
     + ekini0(:,:),             ! first e- kinit interp constant
     + ekini1(:,:),            ! second e- kinit interp constant
     + pkini0(:,:),             ! first e+ kinit interp constant
     + pkini1(:,:),            ! second e+ kinit interp constant
     + ek1s0(:,:),             ! first e- k1s0 interp constant
     + ek1s1(:,:),            ! second e- k1s0 interp constant
     + pk1s0(:,:),             ! first e+ k1s0 interp constant
     + pk1s1(:,:)             ! second e+ k1s0 interp constant

      integer , allocatable ::
     + meke(:)                       ! number of energy ladder steps
c> flag allocate check scpw
      integer   flg_scpw
      data   flg_scpw /-1/
c< flag allocate check scpw
!.....
      CONTAINS
!-----------------------------------------------------------!
      SUBROUTINE ALLOCATE_EGS5_SCPW(imxmed, MXEKE)
      implicit none
      integer ,intent(in) :: imxmed , MXEKE
!
       allocate(
     + escpw0(MXEKE,imxmed),
     + escpw1(MXEKE,imxmed),
     ...
     + meke(imxmed)
     +  , stat= flg_scpw )
      END SUBROUTINE ALLOCATE_EGS5_SCPW
!.....
      SUBROUTINE DEALLOCATE_EGS5_SCPW
      implicit none
!
       if( flg_scpw .eq. 0 )then
       DEALLOCATE(
     + escpw0, escpw1, pscpw0, pscpw1,
     + ekini0, ekini1, pkini0, pkini1,
     + ek1s0, ek1s1, pk1s0, pk1s1,
     + meke )
       endif

      END SUBROUTINE DEALLOCATE_EGS5_SCPW
      END MODULE EGS5_SCPW_MOD
!==================== END ALLOCATE scpw ==============================
!==================== ALLOCATE thresh     ==============================
* module routine replacing include/egs5_thresh.f for variable allocation
!> original include
!------------------------------egs5_thresh.f----------------------------
! Version: 051219-1435
!-----------------------------------------------------------------------
!23456789|123456789|123456789|123456789|123456789|123456789|123456789|12
c
c     common/THRESH/                    ! Threshold (and other) energies
c    * RMT2,                              ! Two times electron rest mass
c    * RMSQ,                                ! Electron rest mass squared
c    * thdum3,           !**** Dummy position marker to prevent mismatch
c    * ap(MXMED),              ! PEGS lower photon cutoff (total) energy
c    * ae(MXMED),            ! PEGS lower electron cutoff (total) energy
c    * up(MXMED),              ! PEGS upper photon cutoff (total) energy
c    * ue(MXMED),            ! PEGS upper electron cutoff (total) energy
c    * te(MXMED),            ! PEGS lower electron cutoff (total) energy
c    * thmoll(MXMED)                   ! Moller threshold (total) energy
c
!-----------------------last line of egs5_thresh.f---------------------
!< original include
!
!> BLOCKDATA EGS5 common/THRESH/
      MODULE EGS5_THRESH_BLOCKDATA_EGS5
      real*8 :: RMT2, RMSQ  ! , thdum3  <- 'thdum3' not use
! common/THRESH/
      data
     *    RMT2/1.021997804/    ! Two times electron rest mass (MeV)
     *    RMSQ/0.261119878/    ! Electron rest mass squared (MeV**2)
      END MODULE EGS5_THRESH_BLOCKDATA_EGS5
!< BLOCKDATA EGS5 common/THRESH/
!----------------------------------------------------------------------!
!
      MODULE EGS5_THRESH_MOD
           USE EGS5_THRESH_BLOCKDATA_EGS5
      implicit none
!....
      real*8 , allocatable::
     + ap(:),      ! PEGS lower photon cutoff (total) energy
     + ae(:),      ! PEGS lower electron cutoff (total) energy
     + up(:),      ! PEGS upper photon cutoff (total) energy
     + ue(:),      ! PEGS upper electron cutoff (total) energy
     + te(:),      ! PEGS lower electron cutoff (total) energy
     + thmoll(:)   ! Moller threshold (total) energy
c> flag allocate check thresh
      integer flg_thresh
      data   flg_thresh /-1/
c< flag allocate check thresh
!.....
      CONTAINS
!-----------------------------------------------------------!
      SUBROUTINE ALLOCATE_EGS5_THRESH( imxmed  )
      implicit none
      integer , intent(in) :: imxmed
!
       allocate(
     + ap(imxmed), ae(imxmed),
     + up(imxmed), ue(imxmed),
     + te(imxmed), thmoll(imxmed)
     +  , stat=flg_thresh )
      END SUBROUTINE ALLOCATE_EGS5_THRESH
!.....
      SUBROUTINE DEALLOCATE_EGS5_THRESH
      implicit none
!
      if( flg_thresh .eq. 0 )then
       DEALLOCATE( ap, ae, up, ue, te, thmoll)
      endif
!
      END SUBROUTINE DEALLOCATE_EGS5_THRESH
      END MODULE EGS5_THRESH_MOD
!==================== END ALLOCATE thresh ==============================
