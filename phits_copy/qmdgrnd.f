...
!$OMP THREADPRIVATE(/coodid/)
      ...
      common /coodrr/ rr(5,nnn),  pr(6,nnn)
!$OMP THREADPRIVATE(/coodrr/)
      common /coodre/ ichgr(nnn), inucr(nnn), ibryr(nnn), indsr(nnn),
     &                inunr(nnn), iavdr(nnn), ihisr(nnn)
!$OMP THREADPRIVATE(/coodre/)
      common /qmdsuc/ isucc
...
*-----------------------------------------------------------------------
               ...
               call packqmdR(massal,msprgr,gambb,ierr)
...
*-----------------------------------------------------------------------
            ...
            if( massta .eq. 1 .and. idnta .ne. 0 ) then ! idnta/idnpr: target/projectile meson number
...
************************************************************************
*                                                                      *
      subroutine packqmdR(massal,msprgr,gambb0,ierr)
*                                                                      *
*                                                                      *
...
*                                                                      *
************************************************************************
      use QMD_COOD2_MOD, only : rhol, dpot, rhoa, rhos, rhoc, phase0,
     & phase, d1r, d1p, rr2,rbij,pp2,rha,rhe,rhc
...
*-----------------------------------------------------------------------
            ...
            ntrya1 = 0
            rabs = sqrt(r(1,i)**2 + r(2,i)**2 + r(3,i)**2)
  ...
  210       do j = 1, i - 1
     ...
     &                  + ( r(3,i) - r(3,j) )**2
               ...
               if( ichg(i) .eq. ichg(j) ) then
                  ...
                  if( rdis2. lt. dmin2 ) then
                   if( ntrya1 .gt. 10) then
                    goto 200 ! rabs is wrong
                   endif
                    thrini = pi * unirn(dummy)        ! rotate r keeping rabs
                    phrini = 2.d0 * pi * unirn(dummy)
                    r(1,i) = rabs * sin(thrini) * cos(phrini)
                    r(2,i) = rabs * sin(thrini) * sin(phrini)
                    r(3,i) = rabs * cos(thrini)
                    ntrya1 = ntrya1 + 1
                    goto 210
                  endif
...
*-----------------------------------------------------------------------
  ...
  700       continue
               ...
               do while( psqr .gt. 1.0 )
                  ...
                  p(2,i) = pfm * py
                  p(3,i) = pfm * pz
                  p(6,i) = sqrt( psqr * pfm**2 + p(5,i)**2 )
*              In a first approximation neglect the potential energy
*              in the total energy
                  p(4,i) = p(6,i)
...
*-----------------------------------------------------------------------
            ...
            do i = 1, massal
               ...
               ekinal = ekinal + p(6,i) - p(5,i)

            end do

               call caldisaR
               call epotallR(epotal)

               ebinal = ( epotal + ekinal ) / dble( massal )
...
*-----------------------------------------------------------------------
         ...
         goto 123  ! 2014/6/6 Adjustment in moving frame is no longer necessary in JQMD-2.0
...
*-----------------------------------------------------------------------
     ...
     &                      + p(5,i)**2 )
               ...
               ettal = ettal + p(4,i) - p(5,i)

            end do

               ebinal = ettal / dble( massal )
...
*-----------------------------------------------------------------------
            ...
            do k = 1, nnmax
               ...
               if( jfrc .eq. ifrc .and.               !FURUTA
     &              ( abs( abs(edif)-abs(edif0)) .gt. !FURUTA
     &              epsilon(1.0)*abs(edif) ) .and.    !FURUTA edif=edif0
     &              abs(edif) .gt. abs(edif0) ) then  !FURUTA
...
*-----------------------------------------------------------------------
         ...
         if(isucc .eq. 0) then    ! 2014/6/16 Ogawa    Boost in target rest frame in stability test.
          imax = massta
         elseif(isucc .eq. 1) then
          imax = masspr
         else
          imax = massta + masspr
         endif
         ...
         do i = 1, imax
            ...
            if((i .le. massta .and. isucc .eq. 2) .or. isucc .le. 1)then ! 2014/6/16 Ogawa
...
*-----------------------------------------------------------------------
  ...
  100    continue
            ...
            pcca   = 1.0 - dble( msprpr * mstapr ) * ccoul
     &             / eccm / rmax - ( b / rmax )**2
...
*-----------------------------------------------------------------------
            ...
            pzta   = pzta / dble( massta )
            pxta   = pxta / dble( massta )

            pzpr   = pzpr / dble( masspr )
            pxpr   = pxpr / dble( masspr )
