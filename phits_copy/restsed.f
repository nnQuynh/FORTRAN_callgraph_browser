...
************************************************************************
*                                                                      *
      subroutine check_tsed(m,iax,jsn,jsi,dsin,idsi,ill,ilf,ierr)
*                                                                      *
...
************************************************************************
      ...
      parameter(icsu=47) ! T.Sato 2022/08/14
...
*-----------------------------------------------------------------------
     ...
     &    'volmat  ','cdiam   ','se-unit ','resfile ','ginfo   ',
     &    'stdcut  ','sangel  ','chmin(1)','chmax(1)','chmin(2)',
     &    'chmax(2)','chmin(3)','chmax(3)','maxangel','model   ',
     &    'samepage','rhomat  '/

      data ( lschn(i), i = 1, icsu ) /
     &     4,         4,         7,         4,         4,
     &     4,         5,         5,         7,         6,
     &     5,         5,         5,         5,         5,
     &     6,         8,         6,         8,         8,
...
*-----------------------------------------------------------------------
               ...
               if( iunt .lt. 1 .or.
     &             iunt .gt. 8 ) goto 988 ! T.Sato 2017/11/21
...
*-----------------------------------------------------------------------
               ...
               if( abs(cdiam) .lt. 0.001d0 .or. abs(cdiam) .gt. 2.d0 )
     &         goto 974
...
*-----------------------------------------------------------------------
  800 continue
     ...
     &                  iztp, zmax, zmin, inz)
      ...
      call check_setype(m,iec,cepn,lepn,
     &                  ietp, emax, emin, ine)

      call check_cdiam(m,cdiam,iec,cepn,lepn,ierr)
...
*-----------------------------------------------------------------------
  ...
  950    write(dkam,'(i9)') mdas
         ...
         ErrCha = ''
         ErrID = 'L:1070/R:check_tsed/F:restsed.f'
         goto 999

  974    m_err = 'cdiam should be 0.001 - 2.0 in tally '//tname
         ErrCha = ''
         ErrID = 'L:1075/R:check_tsed/F:restsed.f'
         goto 999

  975    m_err = 'se-unit should be 0 - 3 in tally '//tname
         ErrCha = ''
         ErrID = 'L:1080/R:check_tsed/F:restsed.f'
         goto 999

  976    m_err = 'SE mesh is Lethergy but emin is zero'
         ErrCha = ''
         ErrID = 'L:1085/R:check_tsed/F:restsed.f'
         goto 999

  978    m_err = 'counter should be from -9999 to 9999'
         ErrCha = ''
         ErrID = 'L:1090/R:check_tsed/F:restsed.f'
         goto 999

  979    m_err = 'Description of material parameter is wrong in tally '
     &            //tname
         ErrCha = ''
         ErrID = 'L:1096/R:check_tsed/F:restsed.f'
         goto 999

  983    m_err = '2D-type should be 1-7 in tally '//tname
         ErrCha = ''
         ErrID = 'L:1101/R:check_tsed/F:restsed.f'
         goto 999

  986    write(dkam,'(i5)') itlmax
         m_err = 'Total tally number exceeds itlmax = '//dkam
         ErrCha = ''
         ErrID = 'L:1107/R:check_tsed/F:restsed.f'
         goto 999

  987    m_err = 'Unknown parameter in tally '//tname
         ErrCha = ''
         ErrID = 'L:1112/R:check_tsed/F:restsed.f'
         goto 999

  988    m_err = 'Unit should be 1 - 3 in tally '//tname
         ErrCha = ''
         ErrID = 'L:1117/R:check_tsed/F:restsed.f'
         goto 999

  989    m_err = 'Double definition of the parameter in tally '//tname
         ErrCha = ''
         ErrID = 'L:1122/R:check_tsed/F:restsed.f'
         goto 999

  990    m_err = 'Too many file name in tally '//tname
         ErrCha = ''
         ErrID = 'L:1127/R:check_tsed/F:restsed.f'
         goto 999

  991    m_err = 'Too many axis in tally '//tname
         ErrCha = ''
         ErrID = 'L:1132/R:check_tsed/F:restsed.f'
         goto 999

  992    m_err = 'Unknown axis name in tally '//tname
         ErrCha = ''
         ErrID = 'L:1137/R:check_tsed/F:restsed.f'
         goto 999

  993    m_err = 'In this line, [ ; ] cannot be used.'
         ErrCha = ''
         ErrID = 'L:1142/R:check_tsed/F:restsed.f'
         goto 999

  994    m_err = 'Name of particle is wrong in tally '//tname
         ErrCha = ''
         ErrID = 'L:1147/R:check_tsed/F:restsed.f'
         goto 999

  995    m_err = 'Number of particles is larger than 6 in tally '//tname
         ErrCha = ''
         ErrID = 'L:1152/R:check_tsed/F:restsed.f'
         goto 999

  996    m_err = 'Description of mesh is wrong in tally '//tname
         ErrCha = ''
         ErrID = 'L:1157/R:check_tsed/F:restsed.f'
         goto 999

  997    m_err = 'Description of parameter is wrong in tally '//tname
         ErrCha = ''
         ErrID = 'L:1162/R:check_tsed/F:restsed.f'
         goto 999

  998    m_err = 'Unknown mesh parameter in tally '//tname
         ErrCha = ''
         ErrID = 'L:1167/R:check_tsed/F:restsed.f'
         goto 999
...
************************************************************************
*                                                                      *
      subroutine read_tsed(m,iax,ierr)
*                                                                      *
*   m: the tally number, index of ital.                                *
************************************************************************
...
************************************************************************
*                                                                      *
      subroutine read_sedreg(m,iax,ioe,jsn,jsi,dsin,idsi,ill,ilf,
...
! sumover
            ie = 1
            call psedreg_sumover_getput(jsi,m,iax,
...
************************************************************************
*                                                                      *
      subroutine read_sedrz(m,iax,ioe,jsn,jsi,dsin,idsi,ill,ilf,
...
! sumover
            ie = 1
            call psedrz_sumover_getput(jsi,m,iax,
...
************************************************************************
*                                                                      *
      subroutine read_sedxyz(m,iax,ioe,jsn,jsi,dsin,idsi,ill,ilf,
...
! sumover
            ie = 1
            call  psedxyz_sumover_getput(jsi,m,iax,
     &      npstepi,nestepi,nxstepi,nystepi,nzstepi,
     &      ip,ie,ix,iy,iz,nsame,tott_sum)
...
! sumover
           ...
           call psedreg_sumover_stdev(1,m,ip,ie,ir,
...
*-----------------------------------------------------------------------
        ...
        vl(ir,iz) = 1.0 ! No unit for (/cm3) (volume is always 1.0 for [t-sed]
...
! sumover
           fact_in = abs(rtfac(m)/facmax(m))
           call psedrz_sumover_stdev(1,m,ip,ie,ir,iz,
...
*-----------------------------------------------------------------------
         ...
         vl(ix,iy,iz) = 1.0 ! no unit for (/cm3) for [t-sed]
...
! sumover
           fact_in = abs(rtfac(m)/facmax(m))
           call psedxyz_sumover_stdev(1,m,ip,ie,ix,iy,iz,
